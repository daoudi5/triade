...
for($j=0;$j<count($eleveT);$j++) {  // premiere ligne de la creation PDF
	...
	for($o=0;$o<count($nbabs);$o++) {
		if (eregi('inconnu',$nbabs[$o][6])) { continue; }
		if ($nbabs[$o][4] > 0) {
			if (ereg('\.5$',$nbabs[$o][4])) { 
	...
	if ($duree_heureT1 >= 3) { $duree_heureT1 = $duree_heureT1 / 3 ;  $duree_heureT1=ereg_replace('\..*$','',$duree_heureT1); $nbabsT1 += $duree_heureT1 ; }
	...
	if ($duree_heureT2 >= 3) { $duree_heureT2 = $duree_heureT2 / 3 ;  $duree_heureT2=ereg_replace('\..*$','',$duree_heureT2);  $nbabsT2 += $duree_heureT2 ; }
	...
	if ($duree_heureT3 >= 3) { $duree_heureT3 = $duree_heureT3 / 3 ;  $duree_heureT3=ereg_replace('\..*$','',$duree_heureT3); $nbabsT3 += $duree_heureT3 ; }
	...
	$nbabsdnjT1=$nbabsTT1;
	if ($duree_heureTT1 >= 3) { $duree_heureTT1 = $duree_heureTT1 / 3 ;  $duree_heureTT1=ereg_replace('\..*$','',$duree_heureTT1); $nbabsdnjT1 += $duree_heureTT1 ; }
	...
	$nbabsdnjT2=$nbabsTT2;
	if ($duree_heureTT2 >= 3) { $duree_heureTT2 = $duree_heureTT2 / 3 ;  $duree_heureTT2=ereg_replace('\..*$','',$duree_heureTT2); $nbabsdnjT2 += $duree_heureTT2 ; }
	...
	$nbabsdnjT3=$nbabsTT3;
	if ($duree_heureTT3 >= 3) { $duree_heureTT3 = $duree_heureTT3 / 3 ; $duree_heureTT3=ereg_replace('\..*$','',$duree_heureTT3); $nbabsdnjT3 += $duree_heureTT3 ; }
	...
	$pdf->SetXY(90,3); // placement du cadre du nom de l eleve
	$pdf->MultiCell(117,25,'',1,'L',1);
	...
	//	$pdf->SetTextColor(255,0,0);  // red
		...
		$pdf->SetFont($police,'',$policeT-1);
		$pdf->WriteHTML(''.trunchaine(strtoupper(sansaccent(strtolower($matiere))),42).'');
		...
		$pdf->WriteHTML(trunchaine($profAff,25));
		...
		if ($noteaff != "") { 
			$NoteCentTotal+=$noteaff*5; 
			$totalMax+=100;$nbNoteP++;
			$NoteMoyen1p+=$noteaff;  
		}
	...
	//	$noteaff=ereg_replace("\...$",'',$noteaff);
		...
		$totalAnnee=$NoteCentTotal/$nbNoteP;
		$mi=$totalAnnee/$totalMax ;
		if ($totalAnnee < $mi ) {
	...
	$pdf->SetXY($xcoor0+50+67,$ycoor0+2);
...
//----------------------------
	...
	//if ($NbCent > 0) { $NoteCentTotal=($NoteCentTotal/$NbCent) * 100; }
	//$NoteCentTotal=number_format($NoteCentTotal,2,'.','');
	$pdf->WriteHTML("<b>$NoteMoyenTotal</b>");
	...
	$pdf->SetXY($Xmat,$Ymat+10);
	$pdf->WriteHTML("<b>A</b>bsences non justifiées (demi-journées) ........... ");
	$pdf->SetXY($Xmat,$Ymat+17);
	$pdf->WriteHTML("<b>R</b>etard ................................................................. ");
	$pdf->SetFont($police,'',$policeT);


	// Case abs,rtd
	$pdf->SetXY($Xmat+$LR8,$Ymat);
	$pdf->MultiCell($LR2,8,'',1,'C',0);
	$pdf->SetXY($Xmat+$LR8+$LR6,$Ymat+1);
	$pdf->WriteHTML("$nbabsT1");

	$pdf->SetXY($Xmat+$LR8+$LR2,$Ymat);
	$pdf->MultiCell($LR2,8,'',1,'C',0);
	$pdf->SetXY($Xmat+$LR8+$LR2,$Ymat+1);
	$pdf->WriteHTML("$nbabsT2");

	$pdf->SetXY($Xmat+$LR8+$LR2+$LR2,$Ymat);
	$pdf->MultiCell($LR2,8,'',1,'C',0);
	$pdf->SetXY($Xmat+$LR8+$LR2+$LR2,$Ymat+1);
	$pdf->WriteHTML("$nbabsT3");

		// abs non justifié
	$pdf->SetXY($Xmat+$LR8,$Ymat+=8);
	$pdf->MultiCell($LR2,8,'',1,'C',0);
	$pdf->SetXY($Xmat+$LR8,$Ymat+1);
	...
	if ((file_exists("./data/image_pers/logo_signature.jpg")) && (($_POST["saisie_trimestre"] == "trimestre1" ) || ($_POST["saisie_trimestre"] == "trimestre2" ) || ($_POST["saisie_trimestre"] == "trimestre3" ) )) {
		$taille = getimagesize("./data/image_pers/logo_signature.jpg");
		$logox=$taille[0]/10;
		$logoy=$taille[1]/20;
		$pdf->Image("./data/image_pers/logo_signature.jpg",$Xmat+1,$Ymat+1,$logox,$logoy);
	}
	$pdf->SetXY(4,$Ymat);
	...
	$pdf->MultiCell($LR9,$HT9,'',1,'C',0);
	if ((file_exists("./data/image_pers/logo_signature.jpg")) && (($_POST["saisie_trimestre"] == "trimestre2" ) || ($_POST["saisie_trimestre"] == "trimestre3" )  )) {
		$taille = getimagesize("./data/image_pers/logo_signature.jpg");
		$logox=$taille[0]/10;
		$logoy=$taille[1]/20;
		$pdf->Image("./data/image_pers/logo_signature.jpg",$Xmat+1,$Ymat+1,$logox,$logoy);
	...
	$pdf->SetXY($Xmat+=$LR9,$Ymat);
	$pdf->MultiCell($LR9,$HT9,'',1,'C',0);
	if ((file_exists("./data/image_pers/logo_signature.jpg")) && (($_POST["saisie_trimestre"] == "trimestre3" ) )) {
		$taille = getimagesize("./data/image_pers/logo_signature.jpg");
		$logox=$taille[0]/10;
		$logoy=$taille[1]/20;
		$pdf->Image("./data/image_pers/logo_signature.jpg",$Xmat+1,$Ymat+1,$logox,$logoy);
