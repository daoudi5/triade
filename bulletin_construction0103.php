...
for($j=$dep;$j<$nbEleveT;$j++) {  // premiere ligne de la creation PDF
	...
	$pdf->SetXY(112-20,36);
	$pdf->MultiCell(10,3,'Rangs',0,'C',0);
	$pdf->SetXY(110-10,36);
	...
	for($i=0;$i<count($ordre);$i++) {
		...
		$largeurMat=45;
		$hauteurMatiere=$haut; // taille du cadre matiere
		...
		$pdf->SetXY($Xnote-11,$Ynote);
		$pdf->MultiCell(106,$hauteurMatiere,'',1,'',0);
		...
		if (($sousmatiere != "0") && ($sousmatiere != "")){
			...
			$pdf->SetFont('Arial','',7);
			$pdf->SetXY($XnotVal-11.5,$YnotVal-3);
			$noteaff1=$noteaff;
			...
			if ( trim($matiereEnCours) != trim($matiereSuivante)) {
				...
				$pdf->SetXY($XnotVal-10,$YnotVal+2);
				$pdf->SetFont('Arial','',12);
...
//			if ($noteaff1 != "") { $noteaff1=arrondiOlive($noteaff1); }
		...
		$pdf->SetFont('Arial','',7);
		$pdf->SetXY($XcoeffVal-10,$YcoeffVal-$top);
		if ($coeffaff == "0") {
			$pdf->WriteHTML("F");
		}else{
...
//	if ($moyeMatGenMaxaff != "") { $moyeMatGenMaxaff=arrondiOlive($moyeMatGenMaxaff); }
	...
	$pdf->SetXY($Xcom-10,$Ycom+2.2+$juste);
...
//	$Ycoeff=$Ycoeff + $hauteurMatiere;
	...
	if ($noteaff != "") { $noteaff=arrondiOlive($noteaff); }
	...
	$coeffaff=$coefBull;
	$pdf->SetFont('Arial','',7);
	$pdf->SetXY($XcoeffVal,$YcoeffVal-$top);
	$coeffaff=number_format($coeffaff,1,'.','');
	$pdf->WriteHTML($coeffaff);
	$YcoeffVal=$YcoeffVal + $hauteurMatiere;
	...
	$Xcom=$XmoyMatGVal + 10;
	$pdf->SetFont('Arial','',8);
	$pdf->SetXY($Xcom+10,$Ycom+2.2);
	$pdf->MultiCell(95-9,$confPolice[1],$commentaireeleve,'','','L',0);
...
$pdf->RoundedRect($XmoyClasseMaxValue+=11, $YmoyClasseMaxValue-=2, 106, 20, 3.5, 'DF');
...
$pdf->SetXY(4,$YcomP2-7);
$pdf->SetFont('Arial','',8);
$pdf->WriteHTML($appreciation2);
$pdf->SetFont('Arial','B',9);
$pdf->SetXY(105,$YcomP2-7);
