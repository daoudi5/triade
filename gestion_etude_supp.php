...
validerequete("2");
if (isset($_POST["supp"])) {
	if (verif_eleve_etude($_POST["id"])) {
		$elevededans=true;
	}else{
	        $cr=etude_supp($_POST["id"]);
        	if($cr){
	           history_cmd($_SESSION[nom],"SUPPRIMER","Etude");
        	}
	}
}
...
?>
<?php
if (isset($elevededans))  {
	print "<script language='JavaScript'>";
	print "alert(\"".LANGETUDE30."\");";
	print "</script>";
...
?>
	...
	<br><br>
        ...
        <form method="POST" action="gestion_etude_supp.php">
        <input type=hidden name=id value="<?php print $data[$i][0]?>"  >
        <input type=submit name="supp" value="<?php print LANGBT50 ?>" class="bouton2">
        </form>
        </td>
        </tr>
