...
<ul>
<font class="T2">
      <?php print "Relevé de notes "?> : <?php print $textTrimestre?><br> <br>
...
for($j=$dep;$j<$nbEleveT;$j++) {  // premiere ligne de la creation PDF
	...
	$pdf->MultiCell(210,5,"Relevé de notes \nTranscript of records",0,'C',0);
...
//			$coef=recupCoefUE($idmatiere,$idClasse,$_POST["saisie_trimestre"]);
			...
			if ($affExam == "oui") {
				...
				if (trim($noteexam) == "") {
					if (($idgroupe == "0") || (trim($idgroupe) == "")) {
                		                $noteexam=moyenneEleveMatiere($idEleve,$idmatiere,$dateDebut,$dateFin,$idprof);
		                        }else{
                                		$noteexam=moyenneEleveMatiereGroupe($idEleve,$idmatiere,$dateDebut,$dateFin,$idgroupe,$idprof);
                        		}
				}
				if ($noteexam == "") {
	...
	// ------------------------------
		...
		$pdf->MultiCell(85,4,"$moyenneUEAff ",0,'R',0); 
...
$pdf->MultiCell(195,15,"Moyenne Générale / General average  $moyenneGeneraleAff  ",1,'C',1);
$Y+=18;
...
$pdf->SetFont('Arial','',12);
$pdf->SetFillColor(255);
$pdf->RoundedRect($X=$Xorigine, $Y+=10, $largeurBulle-5, 27, 3.5, 'DF');
$pdf->SetXY($X+2,$Y+1);
...
$fichier="./data/pdf_bull/$classe_nom/releve_".$nomEleve."_".$prenomEleve."_".$trimfichier.".pdf";
