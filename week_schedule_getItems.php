...
list($annee,$mois,$jours)=split("-",strftime("%Y-%m-%d",$resultat));
...
for($i=0;$i<count($data);$i++) {
	...
	list($year,$month,$day)=split("-",$data[$i][2]);
	list($hour,$minute,$second)=split(":",$data[$i][3]);
	$timestamp=mktime ($hour,$minute,$second,$month,$day,$year) ;
	list($hour,$minute,$second)=split(":",$data[$i][4]);
...
//	if ($information != "") { $texteV.="$information, "; }
	...
	$detail=ereg_replace("'","&acute;",$detail);
	$detail=ereg_replace('"',"&quot;",$detail);
	//$detail=ereg_replace("(","\(",$detail);
	//$detail=ereg_replace(")","\)",$detail);
	$information=ereg_replace("'","&acute;",$information);	
	$information=ereg_replace('"',"&quot;",$information);

	
	$mess=$detail;
	include_once("common/config2.inc.php");
	if ((LAN == "oui") && (AGENTWEB == "oui")) {
		$texteV=ereg_replace("&nbsp;","",$texteV);
		$texteV=ereg_replace("&"," et ",$texteV);
		$texteV=ereg_replace("'","&acute;",$texteV);
		$texteV=ereg_replace("\("," ",$texteV);
		$texteV=ereg_replace("\)"," ",$texteV);
		$texteV=ereg_replace("M. ","monsieur ",$texteV);
		$texteV=ereg_replace("Mme ","madame ",$texteV);
		$texteV=ereg_replace("Mlle ","mademoisel ",$texteV);
		$texteV=ereg_replace("P. ","professeur ",$texteV);
	...
	list($t11,$t22)=split(' ',$inf["eventEndDate"]); //2013-02-05 10:37:00
	list($annee,$mois,$jour)=split('-',$t11);
	list($heure,$minute,$seconde)=split(':',$t22);
	...
	list($t1,$t2)=split(' ',$inf["eventStartDate"]); //2013-02-05 10:37:00
	list($annee,$mois,$jour)=split('-',$t1);
	list($heure,$minute,$seconde)=split(':',$t2);
	$resultat=mktime($heure,$minute,$seconde,$mois,$jour,$annee);
