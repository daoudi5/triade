...
$hauteurConseilBull=$_POST["hauteurConseilBull"];
...
config_param_ajout($hauteurConseilBull,"hauteurConseilBull");
...
for($j=$dep;$j<$nbEleveT;$j++) {  // premiere ligne de la creation PDF
	...
	$pdf->SetFont('Arial','',9);
	$pdf->SetFillColor(220);
	$pdf->SetXY(15,60); //  placement  cadre titre
...
//	$pdf->WriteHTML($soustitre5);
	$pdf->SetX(82);
	$pdf->WriteHTML("$soustitre6 $titrenote3");
	$pdf->SetX(92);
...
//	$pdf->WriteHTML($soustitre8);
	...
	//print $effbordure." $libelleMatiere <br>";
		...
		if ($effbordure == 0 ){
			...
			$H=$hauteurMatiere*$nbs;
			$pdf->MultiCell(107,$H,'',1,'L',0);
		}

		// mise en place du cadre commentaire
		$pdf->SetXY($Xnote-20,$Ynote);
		$pdf->MultiCell(107,$hauteurMatiere,'',$effbordure2,'',0);
		...
		if (($sousmatiere != "0") && ($sousmatiere != "")){
			...
			//$pourcentaff=(($noteaff1/2)*100)/10;
			if ($noteaff1 != "") {
				$noteaff1=number_format($noteaff1/2,2,'.','');
				$pdf->WriteHTML("$noteaff1/10");
				unset($noteaff1);
			}
			...
			if (trim($matiereEnCours) != trim($matiereSuivante)) {
				...
				if ($notesoustotal1 != "") {
					...
					if ($notesousmoyen != "") {$pourcentaff=(($notesousmoyen/2)*100)/10; }
					$notesousmoyen=number_format($notesousmoyen/2,2,'.','');
				...
				$pdf->SetFont('Arial','',8);
				$pdf->SetXY($XnotVal-50,$YnotVal-$ajus);
				
				//$pdf->WriteHTML("Moy : $noteaff1/10");  // --->>>>>>>>>>>>>>>>>>>>>>>>>>>
			...
			if ($noteaff != "") { 
				$pourcentaff=(($noteaff/2)*100)/10; 
				$noteaff1=number_format($noteaff/2, 2, ',', ' ');
				$pdf->WriteHTML("$noteaff1/10");
				unset($noteaff1);
			}
			$YnotVal=$YnotVal + $hauteurMatiere;
		...
		$pdf->SetXY($XcoeffVal,$YcoeffVal+1.5);
		if ($pourcentaff != "") {
			$pourcentaff=number_format($pourcentaff,0,'','');
			$pdf->WriteHTML("$pourcentaff%");
		}
	//	$YcoeffVal=$YcoeffVal + $hauteurMatiere;
		...
		$pdf->SetFont('Arial','',7);
		$pdf->SetXY($XmoyMatGVal-0.5,$YmoyMatGVal-1);
		$moyeMatGenaff=$moyeMatGen;
		if ($moyeMatGenaff != "") {
			$moyeMatGenaff=number_format($moyeMatGenaff/2, 2, ',', ' ');
			$pdf->WriteHTML("$moyeMatGenaff/10");
		}
...
//	$pdf->WriteHTML($moyeMatGenMaxaff);
	...
	$pdf->MultiCell(107,$confPolice[1],$commentaireeleve,'','','L',0);
...
if (MODNAMUR0 == "oui") {
	...
	$pdf->SetXY($Xnote-20,$Ynote);
	$pdf->MultiCell(107,$hauteurMatiere,'',1,'',0);
	$Ynote=$Ynote + $hauteurMatiere;

	// mise en place des notes
	$noteaff=calculNoteVieScolaire($idEleve,$coefProf,$coefVieScol,$_POST["saisie_trimestre"]);
	$pdf->SetFont('Arial','',8);
	$pdf->SetXY($XnotVal-0.5,$YnotVal-1);
	if ($noteaff != "") {
		$noteaff=number_format($noteaff/2, 2, ',', ' ');
		$pdf->WriteHTML("$noteaff/10");
	}
...
// fin du cadre moyenne generale
	...
	$moyenEleveaff=$moyenEleve/2;

	$pdf->WriteHTML("<B>".$moyenEleveaff."/10</B>");
	$noteMoyEleG=0; // pour la moyenne de l'eleve general
	$coefEleG=0; // pour la moyenne de l'eleve general
...
// cadre appréciation
$Ycom=$YMoyGE + 15;
$EpaisCom=$hauteurConseilBull*2;
$YcomP1=$Ycom + 1;
$YcomP2=$YcomP1 + ($EpaisCom/3);
$YcomP3=$YcomP2 + 5;
/*
$Ycom=$YMoyGE + 15;
$EpaisCom=30;
