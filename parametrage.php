...
<html xml:lang="fr" lang="fr" xmlns="http://www.w3.org/1999/xhtml">
	...
	if (isset($_POST["info"])) {
		...
		$val=ereg_replace("{MEMBRE}",$_SESSION["membre"],$val);
		$val=ereg_replace("{IDPERS}",$_SESSION["id_pers"],$val);
	}
	?>
	...
	<!-- // fin  -->
		...
		for ($i=0;$i<count($data);$i++) {
			if (ereg("^sms",$data[$i][1])) { 
				$checkedsms="checked='checked'"; 
				$numero=ereg_replace('sms/','',$data[$i][1]);
			}	
			if ($data[$i][1] == "rss") { 
			...
			if (ereg("@",$data[$i][1])) { $mail=$data[$i][1]; }	
