...
//
if ( (!isset($_GET['iu'])) or (!isset($_GET['v'])) or (!isset($_GET['t'])) or (!isset($_GET['ip'])) ) die();
//
...
if ( ($id_user > 0) and ($version > 18) and ($ip != "") )
{
  ...
  {
    if (f_verif_id_session_id_user($id_user, $id_session) != 'OK')    die ("Session KO.");  // NON! die(">F16#KO#1#");  NON !!!
  }
  //
  //
  ...
  if (_SLOW_NOTIFY == "") // si PAS coché
  {
    ...
    if ($t_share_files != "")
    {
      if ($last_id_sf >= 0)
      {
        ...
        $result = mysql_query($requete);
        if (!$result) error_sql_log("[ERR-22g]", $requete);
        if ( mysql_num_rows($result) == 1 )
        {
          list ($id_sharefiles_max) = mysql_fetch_row ($result);
...
/*    if ( ($t_share_files != "") and ($t_share_files_exchange != "") )
    {
      if ($last_id_ef >= 0)
      {
        ...
        if ( mysql_num_rows($result) == 1 )
        {
          list ($id_sharefiles_exchange_max) = mysql_fetch_row ($result);
...
*/
  ...
  if ( (_SPECIAL_MODE_OPEN_COMMUNITY == "") and (_SPECIAL_MODE_GROUP_COMMUNITY == '') and (_SPECIAL_MODE_OPEN_GROUP_COMMUNITY == '') )
  {
    ...
    $requete .= " and CNT.CNT_STATUS > 0 ";
    $requete .= " and USR.USR_STATUS = 1 ";
    $requete .= " and ( SES.SES_STATUS > 0 and SES.SES_STATUS <= 4 ) "; // 19/05/13
    if ($tri == 'etat')
  ...
  if ( (_SPECIAL_MODE_OPEN_GROUP_COMMUNITY != '')  and  (_SPECIAL_MODE_OPEN_COMMUNITY == '') and (_SPECIAL_MODE_GROUP_COMMUNITY == '') )
  {
    //$username = f_get_username_of_id($id_user);
    $requete  = " select DISTINCT 0, SES.SES_STATUS, USR.ID_USER, SES.SES_AWAY_REASON, GRP.GRP_NAME, USR.ID_ROLE ";
    $requete .= " FROM " . $PREFIX_IM_TABLE . "USG_USERGRP USG, " . $PREFIX_IM_TABLE . "USR_USER USR, " . $PREFIX_IM_TABLE . "SES_SESSION SES, " . $PREFIX_IM_TABLE . "GRP_GROUP GRP ";
    $requete .= " WHERE USR.ID_USER = SES.ID_USER ";
  ...
  {
    $nb_c_send = 0;
    while( list ($id_contact, $eta_num, $usr_id, $away_reason, $group, $id_role_cnt) = mysql_fetch_row ($result) )
    {
      $ok = 'OK';
