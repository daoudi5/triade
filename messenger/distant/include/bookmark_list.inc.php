...
if ( ($id_user > 0) and ($n_version > 0) and ($ip != "") )
{
  ...
  {
    if (f_verif_id_session_id_user($id_user, $id_session) != 'OK')    die(">F55#KO#1#"); // 1:session non ouverte.
  }
  //
  //
  ...
  if ($last_id_m == 0)
  {
    ...
    else
    {
      ...
      if ( $nb_lig > 0 )
      {
        while( list ($id_categ, $categ) = mysql_fetch_row ($result) )
        {
    ...
    {
      echo ">F55#OK#" . $nb_lig . "###|";
      while( list ($id_book, $id_categ, $id_aut, $s_date, $rating, $title, $url, $nb_vote_m, $nb_vote_l) = mysql_fetch_row ($result) )
      {
