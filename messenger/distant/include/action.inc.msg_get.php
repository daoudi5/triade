...
if ( ($id_u_1 > 0) and ($id_session > 0) and ($ip != "") )
{
  ...
  {
    die(">F42#KO#1#"); // la session est déjà fermée (de celui qui consulte, donc presque impossible).
  }
  //
  $user_2  = f_clean_name($user_2); // après functions.inc.php !
  if (f_verif_check_user_only($id_u_1, $check) == 'OK')
  {
    ...
    if ( ($id_conf > 0) and ($id_u_2 <= 0) )
    {
      ...
      if ( mysql_num_rows($result) == 1 ) // normalement pas plus...
      {
        ...
        {
          list ($msg, $heure, $dt_msg, $id_msg, $id_u_2, $cr) = mysql_fetch_row ($result);  // on ne parcours pas pour n'en afficher qu'un à la fois.
          $dt_form = "d/m/Y"; // FR
    ...
    else
    {
      ...
      if ($id_u_2 > 0)
      {
        ...
        if ( mysql_num_rows($result) > 0  )
        {
          list ($msg, $heure, $dt_msg, $id_msg, $cr) = mysql_fetch_row ($result);  // on ne parcours pas pour n'en afficher qu'un à la fois.
          $dt_form = "d/m/Y"; // FR
          if ($dt_f == 'EN') $dt_form = "m-d-Y";
          $dt_msg = date($dt_form, strtotime($dt_msg));
          if ($cr == "")
        ...
        else
        {
          ...
          echo ">F42#OK##h#" . $etat_num . "#" . $etat_away . "####"; // pas de message : attention, renvoyer OK suivi de vide (##) (h pour l'heure, on s'en fou) et surtout l'état.
        }
        //
