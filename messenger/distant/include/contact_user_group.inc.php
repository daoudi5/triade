...
//
if ( (!isset($_GET['u1'])) or (!isset($_GET['u2'])) or (!isset($_GET['ic'])) or (!isset($_GET['gp'])) or (!isset($_GET['ip'])) ) die();
//
...
if ( ($id_u_1 > 0) and ($id_u_2 > 0) and ($contact_id > 0) and ($ip != "") )
{
  ...
  {
    if (f_verif_id_session_id_user($id_u_1, $id_session) != 'OK')    die ("Session KO.");
  }
  //
  //
  ...
  if (!$result) error_sql_log("[ERR-36a]", $requete);
  if ( mysql_num_rows($result) == 1 )
  {
    list ($priv) = mysql_fetch_row ($result);
    //
    $requete  = " update " . $PREFIX_IM_TABLE . "CNT_CONTACT "; 
    $requete .= " set CNT_USER_GROUP = '" . $group . "' ";
    $requete .= " WHERE ID_CONTACT = " . $contact_id;
    $requete .= " LIMIT 1 "; // (to protect)
    $result = mysql_query($requete);
    if (!$result) error_sql_log("[ERR-36b]", $requete);
  }
  //
