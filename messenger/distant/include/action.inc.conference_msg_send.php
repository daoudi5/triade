...
//
if ( (!isset($_GET['u'])) or (!isset($_GET['s'])) or (!isset($_GET['id'])) or (!isset($_GET['m'])) or (!isset($_GET['ip'])) ) die();
//
...
//
if ( ($id_user > 0) and ($id_session > 0) and ($version > 0) and ($msg != "") )
{
  ...
  if ($id_conf > 0)
  {
    ...
    if ( mysql_num_rows($result) == 1 ) // normalement pas plus...
    {
      ...
      {
        list ($nb_p) = mysql_fetch_row ($result);
        if (intval($nb_p) > 0)
        {
          $ok_send = "OK";
        }
