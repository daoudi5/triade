...
function f_share_files_projet_folder($t_project)
{
  ...
  if ($t_project > 0) 
  {
    ...
    {
      list ($folder) = mysql_fetch_row ($result);
    }
  }
	//
	return $folder;
...
{
  global $PREFIX_IM_TABLE, $have_quota_download;
  //
  if ($have_quota_download != "")
  {
    ...
    if ( mysql_num_rows($result) == 1 ) 
    {
      list($nb_share, $nb_exchange, $nb_last_date, $last_date, $nb_last_week, $last_week, $this_week) = mysql_fetch_row ($result);
      //
...
function stats_sharefile_download_inc($id_user, $file_id, $fil_size)
{
  ...
  if ( mysql_num_rows($result) == 1 ) 
  {
    list($last_file_id, $nb_download, $last_date, $nb_last_date, $traffic_last_date,
      ...
      $this_month, $last_month, $nb_last_month, $traffic_last_month) = mysql_fetch_row ($result);
    //
...
function stats_sharefile_add_alert_reject($id_user, $alert_or_reject)
{
  ...
  if ( mysql_num_rows($result) == 1 ) 
  {
    list($nb_alert, $nb_reject) = mysql_fetch_row ($result);
    //
