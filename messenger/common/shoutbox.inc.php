...
{
  GLOBAL $PREFIX_IM_TABLE, $l_date_format_display, $l_time_short_format_display;
  //
  if (_SHOUTBOX_PUBLIC != "")
  {
    ...
    if (flock($fp, 2));
    {
      ...
      if ( mysql_num_rows($result) > 0 )
      {
        ...
        while( list ($id_shout, $s_display, $id_aut, $s_time, $s_date, $rating, $txt) = mysql_fetch_row ($result) )
        {
...
function f_id_group_id_sbx($id_m)
{
  ...
  if ( mysql_num_rows($result) == 1 )
  {
    list ($id_grp) = mysql_fetch_row ($result);
  }
  //
  return $id_grp;
}
...
function shoutbox_remove_old_msg()
{
  ...
  if (intval(_SHOUTBOX_STORE_MAX) > 5)
  {
    ...
    if ( mysql_num_rows($result) == 1 )
    {
      list ($nbtotal) = mysql_fetch_row ($result);
...
function stats_sbx_inc($id_user)
{
  ...
  if ( mysql_num_rows($result) == 1 ) 
  {
    list($nb, $nb_last_date, $last_date, $nb_last_week, $last_week, $this_week) = mysql_fetch_row ($result);
    //
    $nb++;
...
function stats_sbx_update_scores($id_shout, $id_user_aut)
{
  ...
  if ( mysql_num_rows($result) == 1 )
  {
    list ($score_more) = mysql_fetch_row ($result);
  }
  //
  $score_less = 0;
  ...
  if ( mysql_num_rows($result) == 1 )
  {
    list ($score_less) = mysql_fetch_row ($result);
  }
  //
  //
  if ($score_more > 0)
