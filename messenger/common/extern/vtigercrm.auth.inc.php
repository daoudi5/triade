...
define("_EXTERNAL_AUTHENTICATION_NAME", "vtigerCRM");
...
function f_external_authentication($t_user, $t_pass)
{
  ...
  {
    // Si vtigerCRM n'est pas sur le même serveur ou la même base de donnée.
    mysql_close();
    require("extern.sql.inc.php");
  }
   //
  $requete  = " select LOWER(user_name), 	user_password, user_hash FROM " . $extern_prefix . "users ";
  $requete .= " WHERE LOWER(user_name) = '" . $t_user . "' ";
  //$requete .= " and status = 'Active' ";
  $result = mysql_query($requete);
  if (!$result) error_sql_log("[ERR-T21a]", $requete);
...
function f_extern_name_of_user($t_user)
{
  ...
  if ( ($extern_dbhost != '') and ($extern_database != '') and ($extern_dbuname != '') )
  {
   ...
   //
  $requete  = " select LOWER(user_name), last_name, first_name FROM " . $extern_prefix . "users ";
  $requete .= " WHERE LOWER(user_name) = '" . $t_user . "' ";
  $result = mysql_query($requete);
  if (!$result) error_sql_log("[ERR-T21b]", $requete);
  if ( mysql_num_rows($result) == 1 )
  {
