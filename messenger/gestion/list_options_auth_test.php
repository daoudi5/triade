...
if (f_nb_auth_extern() == 1)
{
  ...
  if ($external_authentication_name != "")
  {
    ...
    if ( ($extern_dbhost != '') and ($extern_database != '') and ($extern_dbuname != '') )
    {
      ...
      $id_connect_extern = mysql_connect($extern_dbhost, $extern_dbuname, $extern_dbpass) or die("<font color='red'>" . $l_admin_check_conf_not_ok . "</FONT> <BR/><BR/><SMALL>Unable to connect to MySQL server : " . mysql_error() . "<BR/> ");
      mysql_select_db($extern_database, $id_connect_extern) or die("<font color='red'>" . $l_admin_check_conf_not_ok . "</FONT><BR/>" . $l_admin_check_connect_to_server . " : OK<BR/>" . $l_admin_check_connect_to_database . " : Ko !   <BR/><BR/><SMALL>Unable to select database : " . mysql_error() . "<BR/> ");
      //
      echo "OK.<BR/>";
      echo "<BR/>";
