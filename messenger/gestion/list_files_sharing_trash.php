...
echo "<html><head>";
echo "<title>[IM] " . $l_admin_share_files_trash . "</title>";
display_header();
...
if ( _SHARE_FILES != '' )
{
  ...
  //
  $requete  = " SELECT FIL.ID_FILE, FIL.FIL_NAME, FIL.FIL_SIZE, FIL.FIL_DATE, FIL.FIL_DATE_ADD, FIL.FIL_DATE_TRASH, FIL.FIL_NB_DOWNLOAD, FIL.FIL_HASH, ";
  $requete .= " USR.USR_USERNAME, USR.USR_NICKNAME, FIL.ID_USER_AUT, FPJ.FPJ_NAME, FIL.ID_GROUP_DEST, FMD.FMD_NAME, FIL.FIL_COMPRESS, FIL.FIL_COMMENT ";
  ...
  echo "<TR>";
    ...
    $title = $l_admin_share_files_trash . " </B> ";
  ...
  if ( $nb_row > 0 )
  {
    echo "<TR>";
      ...
      //display_row_table($l_admin_share_files_col_nb_download, '');
    ...
    /*
    echo "<TFOOT>";
    //if ( ( mysql_num_rows($result) > 1 ) and ( (_SHARE_FILES_NEED_APPROVAL != "") or (_SHARE_FILES_EXCHANGE_NEED_APPROVAL != "") ) )
    ...
    */
    ...
    while( list ($id_file, $fil_name, $fil_size, $fil_date, $fil_date_add, $fil_date_trash, $fil_nb_download, $fil_hash, $usrname_auth, $nickname_auth, $id_user_aut, $fpj_name, $id_group_dest, $fmd_name, $fil_compress, $fil_comment) = mysql_fetch_row ($result) )
    {
      $row_num++;
      if (  ($display_start <= 0) or ($display_end <= 0) or ( ($row_num >= $display_start) and ($row_num <= $display_end) )  )
      {
        ...
        */
        ...
        echo "<FORM METHOD='POST' ACTION='files_sharing_delete.php?'>";
          ...
          echo "<INPUT TYPE='hidden' name='source' value = 'share_files_trash' />";
          echo "</TD>";
        echo "</FORM>";
        //
        echo "</TR>";
    ...
    if ( ($nb_row > 15) and ($nb_row_by_page < 1000) )
    {
      ...
      display_nb_row_page(15, $nb_row_by_page, "list_files_sharing_trash_nb_rows");
      echo " | ";
      display_nb_row_page(20, $nb_row_by_page, "list_files_sharing_trash_nb_rows");
      echo " | ";
      display_nb_row_page(25, $nb_row_by_page, "list_files_sharing_trash_nb_rows");
      echo " | ";
      display_nb_row_page(30, $nb_row_by_page, "list_files_sharing_trash_nb_rows");
      echo " | ";
      display_nb_row_page(50, $nb_row_by_page, "list_files_sharing_trash_nb_rows");
    }
