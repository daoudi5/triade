...
$if_prob_const = "OK";
$txt_const = "";
if (!defined("_GROUP_ID_DEFAULT_FOR_NEW_USER"))               $txt_const .= f_add_file_missing("_GROUP_ID_DEFAULT_FOR_NEW_USER", "2.0.5"); // 11/2013
...
//
$id_connect = mysql_connect($dbhost, $dbuname, $dbpass);
$txt = $l_admin_check_connect_server . " : ";
if (!$id_connect) 
{
	$if_prob = "KO";
	$txt .= "<FONT COLOR='RED'><B>" . $l_admin_check_failed . "</B> : " . mysql_error();
}
else
	$txt .= $c_OK;
//
...
{
	while( list ($table1) = mysql_fetch_row ($result) )
	{
		$table_exists .= $table1 . "#";
	}
...
{
  GLOBAL $lang, $c_not_found, $PREFIX_IM_TABLE, $l_admin_check_table, $l_admin_check_col, $l_admin_check_use, $l_admin_check_in_admin, 
...
if ( (_SHARE_FILES != "") and (_SHARE_FILES_FTP_ADDRESS != "") and (_SHARE_FILES_FTP_LOGIN != "") and (_SHARE_FILES_FTP_PASSWORD_CRYPT != "") )
{
  if ( (_SHARE_FILES_FTP_PASSWORD != "") and (_SHARE_FILES_FOLDER == "") )
  {
    ...
    $nb_files_to_delete = mysql_num_rows($result);
    if ($nb_files_to_delete > 0)
    {
      ...
      {
        while( list ($id_file, $fil_name) = mysql_fetch_row ($result) )
        {
