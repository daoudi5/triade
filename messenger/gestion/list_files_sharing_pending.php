...
echo "<html><head>";
echo "<title>[IM] " . $l_admin_share_file_pending . "</title>";
display_header();
...
if ( _SHARE_FILES != '' )
{
  ...
  //
  $requete  = " SELECT FIL.ID_FILE, FIL.FIL_NAME, FIL.FIL_SIZE, FIL.FIL_DATE, FIL.FIL_DATE_ADD, FIL.FIL_HASH, USR.USR_USERNAME, USR.USR_NICKNAME, ";
  $requete .= " FIL.ID_USER_AUT, FMD.FMD_NAME, FPJ.FPJ_NAME, FIL.FIL_COMPRESS, FIL.FIL_COMMENT ";
  ...
  echo "<TR>";
    ...
    $title = $l_admin_share_file_pending . " </B> ";
  ...
  if ( $nb_row > 0 )
  {
    ...
    //{
      ...
      echo "<FORM METHOD='POST' ACTION='files_sharing_pending_validate.php?'>";
      echo "<INPUT TYPE='submit' VALUE = '" . $l_admin_share_file_valid_pending_files . "' class='liteoption' />";
      echo "<INPUT TYPE='hidden' name='from' value = 'list_files_sharing_pending' />";
    ...
    while( list ($id_file, $fil_name, $fil_size, $fil_date, $fil_date_add, $fil_hash, $usrname_auth, $nickname_auth, $id_user_aut, $fmd_name, $fpj_name, $fil_compress, $fil_comment) = mysql_fetch_row ($result) )
    {
      $row_num++;
      if (  ($display_start <= 0) or ($display_end <= 0) or ( ($row_num >= $display_start) and ($row_num <= $display_end) )  )
      {
        ...
        if ($option_show_col_comment != "")
        {
          ...
          echo "<INPUT TYPE='hidden' name='source' value = 'share_files_pending' />";
          echo "</TD>";
          echo "</FORM>";
        }
        //
    ...
    if ( ($nb_row > 15) and ($nb_row_by_page < 1000) )
    {
      ...
      display_nb_row_page(15, $nb_row_by_page, "list_files_sharing_pending_nb_rows");
      echo " | ";
      display_nb_row_page(20, $nb_row_by_page, "list_files_sharing_pending_nb_rows");
      echo " | ";
      display_nb_row_page(25, $nb_row_by_page, "list_files_sharing_pending_nb_rows");
      echo " | ";
      display_nb_row_page(30, $nb_row_by_page, "list_files_sharing_pending_nb_rows");
      echo " | ";
      display_nb_row_page(50, $nb_row_by_page, "list_files_sharing_pending_nb_rows");
    }
