...
if (isset($_POST['id_level'])) $id_level = intval($_POST['id_level']);  else $id_level = 0;
...
{
  if ( ($id_user > 0) and (!preg_match("#[^0-9]#", $id_user)) and ($id_level >= 0) and ($id_level <= 10) )
  {
    define('INTRAMESSENGER',true);
    require ("../common/sql.inc.php");
    //
    $requete  = " update " . $PREFIX_IM_TABLE . "USR_USER ";
    $requete .= " set USR_LEVEL = " . $id_level;
    $requete .= " WHERE ID_USER = " . $id_user;
    $requete .= " LIMIT 1 "; // (to protect)
    $result = mysql_query($requete);
    if (!$result) error_sql_log("[ERR-B8]", $requete);
    //
