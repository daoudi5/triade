...
if ( _ROLES_TO_OVERRIDE_PERMISSIONS != '' )
{
  ...
  {
    list ($role, $default) = mysql_fetch_row ($result);
    echo "<TR>";
    ...
    if ( mysql_num_rows($result) > 0 )
    {
      ...
      //
      //$coul = "row2";
      while( list ($mdl_name, $id_module, $state, $max_value, $rlm_value, $mdl_is_role) = mysql_fetch_row ($result) )
      {
  ...
  //
    ...
    if ( mysql_num_rows($result) > 0 )
    {
      ...
      $coul = "row2";
      while( list ($mdl_name, $id_module, $max_value, $mdl_is_role, $mdl_other) = mysql_fetch_row ($result) )
      {
        $display_this_row = "X";
