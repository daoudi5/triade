...
if ( mysql_num_rows($result) == 1 ) list($nb_users) = mysql_fetch_row($result);
//
...
//$nb_user_uniq_contact = mysql_num_rows($result);
list ($nb_user_uniq_contact) = mysql_fetch_row ($result);
//
...
if ($only_one > 0)
{
  ...
  {
    list ($name_only_one, $nickname, $country_code, $language_code, $avatar, $time_shift) = mysql_fetch_row ($result);
    /*
...
if ( ($nb_row > 0) and (_SPECIAL_MODE_GROUP_COMMUNITY == '') and (_SPECIAL_MODE_GROUP_COMMUNITY == '') )
{
  ...
  if ($nb_row > $nb_row_by_page)
  {
	...
	while( list ($priv, $id_contact, $username_1, $nickname_1, $username_2, $nickname_2, $pseudo, $id_user_1, $id_user_2, $country_code_1, $country_code_2, $language_code_1, $language_code_2) = mysql_fetch_row ($result) )
	{
    $row_num++;
