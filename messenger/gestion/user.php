...
if ( mysql_num_rows($result) == 1 )
{
  list ($username, $nickname, $nom, $usr_level, $usr_check, $usr_status, $usr_datcreat, $usr_datlast, $passcr, $version, $country_code, 
        ...
        $screen_size, $mailclient, $browser, $ooo, $usr_rating, $usr_datpwd, $usr_datactivity, $usr_date_backup, $usr_id_role) = mysql_fetch_row ($result);
...
if (_SHOUTBOX != "")
{
  ...
  list ($sbx_nb_msg_ok, $sbx_nb_msg_ko, $sbx_nb_votes_p, $sbx_nb_votes_c, $sbx_nb_votes_max_tot_p, $sbx_nb_votes_max_tot_c, $sbx_last_vote) = mysql_fetch_row ($result);
  //
  if (_SHOUTBOX_VOTE != "")
  {
    ...
    list ($sbx_nb_give_votes_p) = mysql_fetch_row ($result);
    //
    // Nbre de votes - (donnés)
    $requete  = " SELECT count(*) ";
    $requete .= " FROM " . $PREFIX_IM_TABLE . "SBV_SHOUTVOTE ";
    ...
    list ($sbx_nb_give_votes_c) = mysql_fetch_row ($result);
  }
}
//
if ( _SHARE_FILES != '' )
{
  ...
  list ($share_file_nb) = mysql_fetch_row ($result);
  //
  ...
  if (_SHARE_FILES_VOTE != "")
  {
    ...
    list ($share_file_nb_give_votes_p) = mysql_fetch_row ($result);
    //
    // Nbre de votes - (donnés)
    $requete  = " SELECT count(*) ";
    $requete .= " FROM " . $PREFIX_IM_TABLE . "FLV_FILEVOTE ";
    ...
    list ($share_file_nb_give_votes_c) = mysql_fetch_row ($result);
  }
}
//
if (_BACKUP_FILES != "")
...
//
//
//mysql_close($id_connect); non plus bas !
//
//
if ($action == "wait")
{
  ...
  echo "<TR>";
      ...
      echo "<TD valign='center' VALIGN='MIDDLE' class='row1'>";
        ...
        if ( mysql_num_rows($result) > 0 )
        {
          ...
          //
          while( list ($role, $id_role) = mysql_fetch_row ($result) )
          {
  ...
  echo "<TR>";
    ...
    echo "</TD>";
      echo "<TD class='row1' colspan='2'>";
        ...
        {
          $nb = 0;
          while( list ($grp, $id_grp) = mysql_fetch_row ($result) )
          {
            $nb++;
            if ( $nb % 3 == 0) echo "<br/>";
