 ...
 *******************************************************/
//
if (isset($_POST['srv_ip'])) $srv_ip = $_POST['srv_ip'];  else $srv_ip = "";
...
{
  if ( ( (strlen($srv_ip) > 6) or ($srv_ip == "") ) and ($id_srv > 0) and (!preg_match("#[^0-9]#", $id_srv)) )
  {
    define('INTRAMESSENGER',true);
    require ("../common/sql.inc.php");
    //
    $requete  = " update " . $PREFIX_IM_TABLE . "SRV_SERVERSTATE ";
    $requete .= " set SRV_IP_ADDRESS = '" . $srv_ip . "' ";
    $requete .= " where ID_SERVER = " . $id_srv;
    $requete .= " LIMIT 1 "; // (to protect)
    $result = mysql_query($requete);
    if (!$result) error_sql_log("[ERR-W5a]", $requete);
    //
