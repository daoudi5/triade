...
if (!defined("_ALLOW_CONTACT_RATING"))                      $txt_const .= "_ALLOW_CONTACT_RATING";
if (!defined("_ALLOW_MANAGE_PROFILE"))                      $txt_const .= "_ALLOW_MANAGE_PROFILE";
if (!defined("_ALLOW_MANAGE_OPTIONS"))                      $txt_const .= "_ALLOW_MANAGE_OPTIONS";
if (!defined("_ALLOW_MANAGE_CONTACT_LIST"))                 $txt_const .= "_ALLOW_MANAGE_CONTACT_LIST";
if (!defined("_ALLOW_HISTORY_MESSAGES"))                    $txt_const .= "_ALLOW_HISTORY_MESSAGES";
if (!defined("_FLAG_COUNTRY_FROM_IP"))                      $txt_const .= "_FLAG_COUNTRY_FROM_IP";
if (!defined("_HISTORY_MESSAGES_ON_ACP"))                   $txt_const .= "_HISTORY_MESSAGES_ON_ACP";
if (!defined("_ALLOW_HIDDEN_TO_CONTACTS"))                  $txt_const .= "_ALLOW_HIDDEN_TO_CONTACTS";
if (!defined("_ALLOW_HISTORY_MESSAGES_EXPORT"))             $txt_const .= "_ALLOW_HISTORY_MESSAGES_EXPORT";
if (!defined("_ACP_ALLOW_MEMORY_AUTH"))                     $txt_const .= "_ACP_ALLOW_MEMORY_AUTH";
if (!defined("_ACP_PROTECT_BY_HTACCESS"))                   $txt_const .= "_ACP_PROTECT_BY_HTACCESS";
if (!defined("_ROLE_ID_DEFAULT_FOR_NEW_USER"))              $txt_const .= "_ROLE_ID_DEFAULT_FOR_NEW_USER";
if (!defined("_ALLOW_HIDDEN_STATUS"))                       $txt_const .= "_ALLOW_HIDDEN_STATUS";
if (!defined("_STOP_USE_THIS_SERVER_ADDRESS_NOW_USE_THIS_URL_AFTER_LOGIN")) $txt_const .= "_STOP_USE_THIS_SERVER_ADDRESS_NOW_USE_THIS_URL_AFTER_LOGIN";
if (!defined("_SHARE_FILES_ALLOW_UPPERCASE"))               $txt_const .= "_SHARE_FILES_ALLOW_UPPERCASE";
if (!defined("_SHARE_FILES_PROTECT"))                       $txt_const .= "_SHARE_FILES_PROTECT";
if (!defined("_SHARE_FILES"))                               $txt_const .= "_SHARE_FILES";
if ($txt_const != "")
{
  echo "<html><head>";
...
#  require ("../common/sql.inc.php");
  ...
  list ($time_server_mysql) = mysql_fetch_row ($result);
  ...
  if (intval($nb_row_stats) > 60)
  {
    ...
    if ( (_ROLES_TO_OVERRIDE_PERMISSIONS != "") and ( (intval(_LOCK_AFTER_NO_ACTIVITY_DURATION) >= 10) or (intval(_LOCK_AFTER_NO_CONTACT_DURATION) >= 10) ) )
    {
      ...
      {
        while( list ($tid) = mysql_fetch_row ($result) )
        {
  ...
  if (_SHARE_FILES != "")
  {
    ...
    list ($nb_file_share) = mysql_fetch_row ($result);
    //
    $nb_file_download = 0;
    if ($nb_file_share > 9)
    {
      ...
      list ($nb_file_download) = mysql_fetch_row ($result);
    }
    //
    $requete  = " select count(*) ";
    $requete .= " FROM " . $PREFIX_IM_TABLE . "FIL_FILE ";
    ...
    list ($nb_file_share_trash) = mysql_fetch_row ($result);
    //
    //$nb_file_pending = 0;
    ...
    list ($nb_file_pending) = mysql_fetch_row ($result);
    //
    $requete  = " select count(*) ";
    $requete .= " FROM " . $PREFIX_IM_TABLE . "FIL_FILE ";
    ...
    list ($nb_file_alert) = mysql_fetch_row ($result);
    //
    ...
    list ($t_size_ko) = mysql_fetch_row ($result);
    ...
    if (_SHARE_FILES_VOTE != "")
    {
      ...
      list ($share_files_nb_votes_p) = mysql_fetch_row ($result);
      //
      // Nbre de vote -
      $requete  = " SELECT count(*) ";
      $requete .= " FROM " . $PREFIX_IM_TABLE . "FLV_FILEVOTE ";
      ...
      list ($share_files_nb_votes_c) = mysql_fetch_row ($result);
    }
  }
  //
  //
  if (_BACKUP_FILES != "")
  {
    ...
    list ($nb_file_backup) = mysql_fetch_row ($result);
    //
    ...
    list ($nb_file_user_backup) = mysql_fetch_row ($result);
    //
