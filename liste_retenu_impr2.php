...
if ($texte ==  "{FICHIERRTFRETENU}") {
	...
	for($i=0;$i<$nb;$i++) {
		...
		list($idEleve,$dateretenue,$heureretenue,$duree,$idcategory,$motif,$attribuerpar,$devoirafaire,$faits) = split("#",$_POST["liste"][$i]);
		if ($idEleve == "")  { continue; }
		...
		$nomEleve=ereg_replace("'"," \\rquote ",$nomEleve);
		$nomEleve=ereg_replace('"'," \\ldblquote ",$nomEleve);
		$prenomEleve=ereg_replace("'"," \\rquote ",$prenomEleve);
		$prenomEleve=ereg_replace('"'," \\ldblquote ",$prenomEleve);
		$commune_adr1=ereg_replace("'"," \\rquote ",$commune_adr1);
		$commune_adr1=ereg_replace('"'," \\ldblquote ",$commune_adr1);
		$category=ereg_replace("'"," \\rquote ",$category);
		$category=ereg_replace('"'," \\ldblquote ",$category);
		$motif=ereg_replace("'"," \\rquote ",$motif);
		$motif=ereg_replace('"'," \\ldblquote ",$motif);
		$attribuerpar=ereg_replace("'"," \\rquote ",$attribuerpar);
		$attribuerpar=ereg_replace('"'," \\ldblquote ",$attribuerpar);
		$devoirafaire=ereg_replace("'"," \\rquote ",$devoirafaire);
		$devoirafaire=ereg_replace('"'," \\ldblquote ",$devoirafaire);
		$faits=ereg_replace("'"," \\rquote ",$faits);
		$faits=ereg_replace('"'," \\ldblquote ",$faits);

		$texte=ereg_replace("NomEleve","$nomEleve",$texte);
		$texte=ereg_replace("PrenomEleve","$prenomEleve",$texte);
		$texte=ereg_replace("ClasseEleve","$classe_nom",$texte);
		$texte=ereg_replace("AdresseEleve","$adr1",$texte);
		$texte=ereg_replace("CodePostalEleve","$code_post_adr1",$texte);
		$texte=ereg_replace("VilleEleve","$commune_adr1",$texte);
		

		$texte=ereg_replace("RETENUCATEGORY",$category,$texte); 
		$texte=ereg_replace("DATERETENU",$dateretenue,$texte); 
		$texte=ereg_replace("HEURERETENU",$heureretenue,$texte);
		$texte=ereg_replace("RETENUDUREE",$duree,$texte);
		$texte=ereg_replace("RETENUMOTIF",$motif,$texte);
		$texte=ereg_replace("ATTRIBUEPAR",$attribuerpar,$texte);
		$texte=ereg_replace("DEVOIRAFAIRE",$devoirafaire,$texte);
		$texte=ereg_replace("FAITS",$faits,$texte);

		$texte=ereg_replace("DATEDUJOUR",$date,$texte);
		$texte=ereg_replace("PRENOMRESP1",$prenomtuteur1,$texte);
		$texte=ereg_replace("NOMRESP1",$nomtuteur1,$texte);
		$texte=ereg_replace("CIVILITETUTEUR1",$civtuteur1,$texte);
