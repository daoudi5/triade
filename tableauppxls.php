...
if (count($valeur)) {
	...
	while(count($ordre)) {
		...
		$jo=0;
		$worksheet1->write(0, $jo, "Nom Prénom", $header);
		$worksheet2->write(0, $jo, "Classe", $header);
		$jo++;
		$worksheet1->write(0, $jo, "Période", $header);
		$jo++;
		...
		while (list($key, $nom_matiere) = each($tabmatiere4)) {
			...
			if (trim($nomSousMatiere) != "") {
				$nom_matiere=ucfirst($nom_matiere)." ".$nomSousMatiere;
			}
			$nomSousMatiere="";
			$codeMatiere=chercheCodeMatiere($idMatiere);
			if ($codeMatiere != "") $nom_matiere=$codeMatiere;
			$worksheet1->write(0, $jo, "$nom_matiere ($coeffaff)", $header);
			$jo++;
			$nbmat0++;
		}
		$nbmat0+=3;
		...
		for($j=0;$j<count($eleveT);$j++) {  // premiere ligne de la creation PDF
			...
			$worksheet2->write($ligne2, 0, "$nomEleve $prenomEleve", $center);
	
			$worksheet1->write($ligne, 0, "$nomEleve $prenomEleve", $center);
