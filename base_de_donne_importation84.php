...
if ( (!empty($fichier)) && (($type == "application/octet-stream" ) || ($type == "application/vnd.ms-excel" ))) {
	...
	for ($i = 2; $i <= $data->sheets[0]['numRows']; $i++) {
		...
		if ($nb == 0)  { 
			...
			if ($nb == 0) {
				import_affectation_IPAC($idMatiere,$idClasse,$coef,$trim,$ects,$idDetailUE,$ordre_affichage,$specif_etudeDeCas,$anneeScolaire);
			}else{ 		
				import_affectation_IPAC_UPDATE($idMatiere,$idClasse,$coef,$trim,$ects,$idDetailUE,$ordre_affichage,$specif_etudeDeCas,$anneeScolaire);
			}
		}
