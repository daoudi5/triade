...
for($j=$dep;$j<$nbEleveT;$j++) {  // premiere ligne de la creation PDF
	...
	*/
	...
	$pdf->SetFillColor(230,230,255);
	$pdf->RoundedRect($xcoor0, $ycoor0, 102, 23, 3.5, 'DF');
	$pdf->SetFillColor(255);
	$pdf->SetXY($xcoor0,$ycoor0+2);
	$textTrimestre=ucfirst(strtolower($textTrimestre));
	$pdf->MultiCell(100,3,"RELEVE DE NOTES \n\n $textTrimestre",0,'C',0);	
	...
	$pdf->SetXY($xcoor0=3,$ycoor0+=25);
	$pdf->SetFillColor(230,230,255);
	$pdf->RoundedRect($xcoor0, $ycoor0, 170, 22, 3.5, 'DF');
	$pdf->SetFillColor(255);
...
//print $effbordure." $libelleMatiere <br>";
		...
		$libelleMatiere=ereg_replace('&#8217;',"'",$libelleMatiere);
		...
		$listeExamen=array("CC","DST","Dad","Soutenance","Rapport","Fiche de lecture","Exposé","Partiel");
		...
		if ($effbordure == 0 ){
			...
			$H=$hauteurMatiere*$nbs;
			$pdf->MultiCell(80,$H,'',1,'L',0);  //commentaire
		}
...
$pdf->SetXY($xcoor0+10,$ycoor0+4);
$pdf->SetFont('Arial','',10);
$pdf->MultiCell(100,3,"Moyenne académique : $noteMoyEleG / 20",0,'L',0);

$noteMoyEleG=preg_replace('/,/','.',$noteMoyEleG);
$noteMoyEleG=$noteMoyEleG-$ptenmoinsMoyen; // point en moins par assuidité
$noteMoyEleG=number_format($noteMoyEleG,2,',','');
$ptenmoinsMoyen=number_format($ptenmoinsMoyen,2,',','');


$pdf->SetXY($xcoor0+10,$ycoor0+10);
$pdf->SetFont('Arial','',10);
$pdf->MultiCell(100,3,"Pénalité pour absences : $ptenmoinsMoyen ",0,'L',0);



$pdf->SetXY($xcoor0,$ycoor0+18);
$pdf->SetFont('Arial','',12);
$pdf->MultiCell(100,3,"MOYENNE : $noteMoyEleG / 20",0,'C',0);
