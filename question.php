...
if (isset($_POST["saisie_question"])) {
	if ( (!empty($_POST["saisie_question"])) && (!empty($_POST["email"])) && (!empty($_POST["nom"])) && (!empty($_POST["prenom"]) ) ) {
		...
		$prenom=$_POST["prenom"];
      		$today=dateDMY();
      		$fichier=fopen("./data/fic_question_faq.txt","a+");
      		...
      		if (MAILMESSSYS == "oui") {
  			mailAdmin("Une question vous est posée.");
      		}
      		print "<script>alert(\"".LANGTPROBL11."\")</script>";
...
print "<script type='text/javascript'>var vocalmess='questionacces';</script>\n";
...
<tr id='coulBar0' ><td height="2"><b><font   id='menumodule1' ><?php print LANGTTITRE6 ?></FONT></td></tr>
<tr id='cadreCentral0' >
<td >
<?php
$cnx=cnx();
