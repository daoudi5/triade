...
//tinymce.activeEditor.setContent("textarea content in here"); 
	...
	&nbsp;&nbsp;<input type='button' value='<?php print LANGBT50 ?>'  class='bouton2' onclick="supprDevoir(document.form11.iddevoir.value,'retourenr0',document.form11.devoirvisu.value)" title="<?php print LANGMESS107 ?>" />
...
//	tinyMCE.triggerSave();
//        var commentaire = escape(document.getElementById('elm1').value);
	var commentaire = tinyMCE.get('elm1').getContent() ;
        enrDevoir(iddevoir,commentaire,'retourenr0',devoirvisu);
}

function visacahiertexte(idclasse,idmatiere,datedebut,datefin,classorgrp) {
	enrvisacahiertexte(idclasse,idmatiere,datedebut,datefin,'afficheVisa',classorgrp);
}
...
for($i=0;$i<=$nb;$i++) {
	...
	for($j=0;$j<count($data);$j++) {
	        $id_class_or_grp=$data[$j][0];
                ...
                if ($classorgrp == 0) {
                        $nomclasse=chercheClasse($id_class_or_grp);
		...
		print "&nbsp;<font color=blue title=\"$nomclasse\" >".ucfirst(chercheMatiereNom($data[$j][1]))."</font> ";
	        print " <font class='T1'><i>(".ucwords(LANGPROFK)." ".dateForm($data[$j][2]).")</i></font>";
		if ($tempsestime == "00:00:00") $tempsestime="";
		print "$tempsestime";
		print "&nbsp;&nbsp;".$contenu;
