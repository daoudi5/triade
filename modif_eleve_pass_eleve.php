...
$affiche=1;
$email=trim(recupEmail("menueleve",$ideleve,0));
$emailVatel=trim(recupEmailVatel("menueleve",$ideleve,0));
if ((trim($email) != "") && (trim($emailVatel) != "")) { $email.=",";  }
$email.=$emailVatel;
if(isset($_POST["create"])) {
	$cr=modif_eleve_passe_2($ideleve,$_POST["pass"],$_POST["envoimail"],$_POST["email"]);
	if ($cr) {
	$affiche=0;
?>
	<br><font class=T2><?php print PASSG9 ?> <b><?php print ucwords(strtolower($prenomeleve));?>
	<?php print ucwords($nomeleve);?></b> <?php print PASSG9bis ?><br>
	</font><br /><br />
	<table align=center border=0><tr><td><script language=JavaScript>buttonMagicFermeture(); </script>
	</td></tr></table>
<?php
	}else{
...
<font class=T2><?php print LANGNA1 ?> : <b><?php print ucwords($nomeleve);?></b><br>
<br>
...
<script language=JavaScript>buttonMagicFermeture(); //bouton de fermeture</script>&nbsp;&nbsp;</td></tr></table>
