...
if (isset($_POST["create"])) {
	...
	if($cr == 0){
		...
		if($cr == 1){
			...
			$enr="pasok";
			$messageresa="<font color=red>"."Salle déjà réservée à l'une des dates récurrentes.";
...
<td>
<?php if ($enr == "pasok") { ?>
<form method=post action="resr_salle_fen.php" name=formulaire  onsubmit="return validresa2()" >
...
<tr><td align=right width=50%><font class="T2"><?php print LANGRESA70 ?> :</font> </td>
<?php
if (isset($_GET["date"])) {
		$date=dateForm($_GET["date"]);
}
?>
<td><input type=text name=saisie_date value="<?php print $date?>"  size=12 class=bouton2 readonly>
<?php
...
<tr><td align=right><font class="T2"><?php print LANGTE3 ?> :</font> </td>
<td><input type=text name=saisie_heure1 value="<?php print $heure1?>"  onclick="this.value=''" size=5></td></tr>


<tr><td align=right><font class="T2"><?php print LANGTE13 ?> :<font> </td>
<td><input type=text name=saisie_heure2 value="<?php print $heure2?>"  onclick="this.value=''" size=5></td></tr>

<tr><td align=right valign=top><font class="T2"><?php print RESA75 ?> :</font> </td>
...
<?php
	...
	if (RESERV == "oui") {
		print "<center><font class=T2>Enregistrement terminé</font></center>";
	}else{
