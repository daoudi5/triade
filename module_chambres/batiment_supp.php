...
if(autorisation_module()) {
	...
	if($operation == "enregistrer") {
		if($id > 0) {
			$sql= "DELETE FROM ".CHA_TAB_BATIMENT." ";
	...
	// Rechercher la liste des batiments deja utilises dans les chambres (=> ils ne pourront pas etre supprimes)
	$sql ="SELECT batiment_id ";
	$sql.="FROM ".CHA_TAB_CHAMBRE." ";
	$sql.="GROUP BY batiment_id ";
	$res_batiments_utilises=execSql($sql);
	
	/*
			// Verifier si la requete sql a reussi ou non
			if(is_object($res_batiments_utilises) || (is_numeric($res_batiments_utilises) && $res_batiments_utilises > 0))
			{
				echo 'batiments_utilises_OK';
			} else {
				echo 'batiments_utilises_KO : ' .$sql;
			}
	*/
	
	$batiments_utilises = array();
	for($i=0;$i<$res_batiments_utilises->numRows();$i++) {
		$ligne_tmp = $res_batiments_utilises->fetchRow();
		$batiments_utilises[$i] = $ligne_tmp[0];
	}
	...
	if($res->numRows() == 0) {
		msg_util_ajout(LANG_CHA_BATI_010, 'avertissement');
	}
	//***************************************************************************
		...
		<table border="0" cellpadding="3" cellspacing="1" width="100%" bgcolor="#0B3A0C" height="85">
			<tr id="coulBar0">
				<td height="2" align="left">
					<b><font id="menumodule1" ><?php echo LANG_CHA_BATI_007; ?></font></b>
				</td>
			</tr>
			<tr id="cadreCentral0">
				<td valign="top" align="center">
								...
								<td valign=top align="center">
									...
									<table border="0" cellpadding="0" cellspacing="0" align="center">							
										<tr>
											...
											<td valign="top" align="left">
												...
												<?php
													for($i=0; $i<$res->numRows(); $i++) {
														...
														$classe = '';
														if(in_array($ligne[0], $batiments_utilises)) {
											...
											<td>&nbsp;</td>
											<td valign="top">
												<a href='javascript:;'  onMouseOver="AffBulle3('<?php echo LANG_CHA_GENE_002; ?>','./image/commun/info.jpg','<?php echo LANG_CHA_BATI_100; ?>', '');"  onMouseOut="HideBulle()";><img src="./image/help.gif" border=0 align=center></a>
											</td>
										</tr>
									</table>
								...
								<td align="center">
									<table border="0" align="center" cellpadding="4" cellspacing="0">
										<tr>
											...
											?>
											<td align="center">
												<script language="javascript">buttonMagicSubmit("<?php print LANG_CHA_BATI_011?>","create"); //text,nomInput</script>
											</td>
											<?php
											}
											?>
					...
					<script language="javascript">
						...
						function valider_le_formulaire() {
							...
							if(obj.options[obj.selectedIndex].className == 'element_liste_grise') {
								alert("<?php echo LANG_CHA_BATI_101; ?>");
								if(valide) {
									obj.focus();
								}
								valide = false;
					...
					<form name="formulaire_annuler" id="formulaire_annuler" action="<?php echo $g_chemin_relatif_module; ?>batiment_ajout.php" method="post">
					</form>
