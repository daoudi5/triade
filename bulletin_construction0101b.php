...
for($i=0;$i<count($data);$i++) {
       $nom_etablissement=trim(sansaccent($data[$i][0]));
...
//$ordre=ordre_matiere_visubull_btsblanc($_POST["saisie_classe"],$_POST["saisie_trimestre"],$anneeScolaire); // recup ordre matiere
...
$typeexamen="Examen B.T.S. Blanc";
$typecommentaire="btsblanc";     
$examen="BTS Blanc"; // BTS Blanc

// pour le calcul de moyenne classe

$moyenClasseGen=calculMoyenClasseBlancAFTEC($idClasse,$eleveT,$dateDebut,$dateFin,$ordre,$examen);
...
for($j=$dep;$j<$nbEleveT;$j++) {  // premiere ligne de la creation PDF
	...
	$titre="<B><U>".LANGBULL30." BTS Blanc </u></B>";
	...
	$Pdate="Année de Formation ".$anneeScolaire;
	...
	for($i=0;$i<count($ordre);$i++) {
		...
		$coeff=recup_coef_bulletin($_POST["examen"],$idClasse,$idMatiere,$i);
...
//print $effbordure." $libelleMatiere <br>";
		...
		if ($notaffmoyclass != "oui") { 
			...
			$pdf->MultiCell(87+20,$hauteurMatiere,'',$effbordure2,'',0);
		}	
...
//			$coefsous=recupCoeff($idMatiere,$idClasse,$ordre[$i][2]);
			$coefsous=recup_coef_bulletin($_POST["examen"],$idClasse,$idMatiere,$ordre[$i][2]);
...
//				$coeffaff=recupCoeff($ordre[$i][0],$idClasse,$ordre[$i][2]);
				$coeffaff=recup_coef_bulletin($_POST["examen"],$idClasse,$idMatiere,$ordre[$i][2]);
				if ($coeffaff > 0) {	
			       	 	$noteMoyEleGTempo = $noteaff * $coeffaff;
		               	 	$noteMoyEleG=$noteMoyEleG + $noteMoyEleGTempo;
		       			$totalEleve+=$noteaff * $coeffaff;
                			$coefEleG=$coefEleG + $coeffaff;
				}else{
					$noteaff = $noteaff - 10;
...
//print $idMatiere." ".$coeffaff."<br>";
		...
		if ($idgroupe == "0") {   // non matiere affectée à un groupe
			...
			for($g=0;$g<count($eleveTg);$g++) {
				...
				if ($idgroupe == "0") {
		                        $valeur=moyenneEleveMatiereExamen($idEleve,$ordre[$i][0],$dateDebut,$dateFin,$idprof,$examen);
                		}else{
                        		$valeur=moyenneEleveMatiereGroupeExamen($idEleve,$ordre[$i][0],$dateDebut,$dateFin,$idgroupe,$idprof,$examen);
                		}
				if (trim($valeur) != "") {	
					if ($valeur >= $max) { $max=$valeur; }
					if ($valeur <= $min) { $min=$valeur; }
...
//		$pdf->WriteHTML($moyeMatGenMaxaff);
	...
	if ($notaffmoyclass != "oui") { 
		...
		$pdf->MultiCell(87+32,$confPolice[1]+0.3,$commentaireeleve,'','','L',0);
	}else{
		$pdf->SetXY($Xcom-20,$Ycom);
		$pdf->MultiCell(87+20,$confPolice[1]+0.3,$commentaireeleve,'','','L',0);
	}
...
// fin du cadre moyenne generale
	...
	if (($moyenadmis != 0) && (trim($moyenadmis) != "")) {
		if (($moyenEleveaff != 0) && (trim($moyenEleveaff) != "")) {
		        $moyenEleve=preg_replace('/,/','.',$moyenEleveaff);
		        ...
		        $pdf->SetFont('Arial','',12);
			if ($notaffmoyclass == "oui") {
				$pdf->SetXY($XmoyElValue+25,$YmoyElGenValue);
			}else{
				$pdf->SetXY($XmoyElValue+15,$YmoyElGenValue);
			}
			$total=$coefEleG*20;
	        	$pdf->WriteHTML("<FONT COLOR='$color' ><B>".$admis."</B></FONT>   $totalEleve/$total ");
		}
	}
	$coefEleG=0; // pour la moyenne de l'eleve general
	$total=0;
...
$pdf->close();
if ($examen != "") {
	$examenAff=preg_replace('/ /','_',$examen);
	bulletin_archivage($examenAff."_".$_POST["saisie_trimestre"],$anneeScolaire,$fichier,$idEleve,$classe_nom,$nomEleve,$prenomEleve);
}else{
