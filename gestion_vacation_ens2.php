...
$pe=ereg_replace('"',"'",rechercheInfoPerso($idpers,0));
$lieuenseignant=ereg_replace('"',"'",rechercheInfoPerso($idpers,1));
...
$listeMatiere=ereg_replace(" Et "," et ",$listeMatiere);
$listeSousMatiere=ereg_replace(" Et "," et ",$listeSousMatiere);
?>
...
//	$pdf->SetTextColor($tabcoul1[$ii],$tabcoul2[$ii],$tabcoul3[$ii]);
	$info="$presta - $nommatiere ($classe)";
	$pdf->MultiCell(60,3,"$info",0,'L',0);
	$pdf->SetXY($X,$Y);
	$pdf->MultiCell(60,15,"",1,'L',0);
//	$pdf->SetTextColor(0);
	
	$pdf->SetFont('Arial','',10);
	$X+=60;
	$pdf->SetXY($X,$Y);
	$pdf->MultiCell(20,15,"$nbheure",1,'C',0);
	$X+=20;
	$pdf->SetXY($X,$Y);
	$pdf->MultiCell(20,15,"$nbforfait",1,'C',0);
	$X+=20;
	$pdf->SetXY($X,$Y);
	$base=affichageFormatMonnaie($base);
	$pdf->MultiCell(30,15,$base,1,'C',0);
	$X+=30;
	$pdf->SetXY($X,$Y);
	$pdf->MultiCell(40,15,$nbTotalBrut,1,'C',0);
	$X+=40;
	$Y+=15;
	$X=20;
	$ii++;
...
for($i=0;$i<count($liste);$i++) {
	...
	$duree=timeForm($liste[$i][5]);$duree=ereg_replace(":","h",$duree);
	$annule=$liste[$i][11];
...
$nomprenom=ereg_replace("/","_",$nomprenom);
...
<font class=T2>&nbsp;&nbsp;Imprimer la demande de vacation : </font> <input type=button  name="liste" class="bouton2"  value="Imprimer" onClick="open('visu_pdf_admin.php?id=<?php print $fichier?>','_blank','');" />
<?php } ?>
<br /><br /><br />
