...
<?php print LANGASS38 ?></font></b></td>
</tr>
<tr id='cadreCentral0'>
<td>
<!-- // fin  -->
<?php
if (TROMBIALLCLASSEPROF == "oui") {
	print "<form method='post' name='formulaire' action='trombi-prof2.php'>";
}else{ 
	print "<form method='post' onsubmit='return verifAccesFiche()' name='formulaire' action='trombi-prof2.php'>";
} 
?>
<br />
<ul>
<font class="T2"><?php print LANGPROFG ?> :</font>
				 ...
				 <select name="sClasseGrp" size="1" >
					 <option value="0" id='select0'> <?php print LANGCHOIX3 ?> </option>
				 <?php
				 if (TROMBIALLCLASSEPROF == "oui") {
						select_classe();					
					 ...
					 for($i=1;$i<count($data);$i++){
					 	if( $i>1 && ($data[$i][4]==$gtmp) && ($data[$i][0]==$ctmp) ){
							...
							print "<option id='select1' value=\"".$data[$i][0]."\">".$libelle."</option>\n";
						}
						$gtmp=$data[$i][4];
						$ctmp=$data[$i][0];
				 	}
