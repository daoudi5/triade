...
<?php print LANGMODIF13 ?> <?php print LANGASS5 ?></font></b></td></tr>
<tr id='cadreCentral0'>
<td >
<?php
...
if(isset($_POST["create"])) {
	// requete ? prenom2 ?
	$cr=modif_personnel_prof($_POST["id_pers"],$_POST["saisie_creat_nom"],$_POST["saisie_creat_prenom"],$_POST["saisie_intitule"],$_POST["saisie_creat_adr"],$_POST["saisie_creat_code"],$_POST["saisie_creat_tel"],$_POST["saisie_creat_mail"],$_POST["saisie_creat_commune"],$_POST["saisie_creat_tel_port"],$_POST["saisie_identifiant_prof"],$_POST["saisie_indice_salaire"]);
	if($cr == 1){
		alertJs(LANGMODIF14);
	...
	$saisie_id=$_GET["saisie_id"];
	  ...
	  $passage_argument="oui"; // pour le JavaScript
          ...
          // pers_id,nom,prenom,mdp,civ,email,adr,code_post,commune,tel,tel_port,identifiant,offline
...
<tr><td align=right width=40%><font class="T2"><?php print nb2space(LANGNA3)?> : </font></td><td><input type=button onclick="open('modif_pers_pass.php?id=<?php print $saisie_id;?>&type=ENS','pass','width=400,height=300')" value='<?php print LANGPER30 ?>'  class="bouton2" > </td></tr>
</table>
</fieldset>
<br><br><br>
...
<tr><td align=right><font class="T2"><?php print LANGMESS57 ?> : </font></td><td><input type=text name="saisie_identifiant_prof" size=10 maxlength=40 value="<?php print $identifiant ?>" onblur="verifChamps('personnel','identifiant',this.value,document.formulaire.create)" > <i>(Harpege, VT, ...)</i> </td></tr>
...
<script language=JavaScript>buttonMagicSubmit("<?php print LANGMODIF13 ?>","create"); //text,nomInput</script>
	  <script language=JavaScript>buttonMagic("<?php print LANGBT10?>","list_enseignant.php","_parent","","");</script><br><br>
<?php if ($offline == 0) { ?>
