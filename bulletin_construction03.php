...
$retenuarras=$_POST["affretenu"];
config_param_ajout($retenuarras,"retenuarras");
...
for($j=$dep;$j<$nbEleveT;$j++) {  // premiere ligne de la creation PDF
	...
	//---------------------------------//
	$nbretenue="0";
	$tabretenue=affRetenue_via_date($idEleve,$dateDebut,$dateFin);
	$nbretenue=count($tabretenue);
	...
	if ($retenuarras == "oui") {
		$appreciationbis2="$nbretenue retenue(s)";
	}else{
		$appreciationbis2="$nbheureabs heure(s) d'absence(s)";
	}
	$appreciationbis="($nbretard retard(s) / $nbabs demi-journée(s) d'absence(s) / $appreciationbis2 ) " ;
	...
	// $signature2="T.R.I.A.D.E. / http://www.triade-educ.com";
	// FIN variables
	...
	for($ik=0;$ik<count($dataadresse);$ik++) {
		...
		$civ_1=civ($dataadresse[$ik][13]);
		$civ_2=$dataadresse[$ik][14];
		...
		$pdf->SetXY($Xv1+ 33,44);
		$class_ant=trunchaine($class_ant,12);
		$pdf->WriteHTML("Classe ant.: $class_ant ");

		$pdf->SetFont('Arial','',10);
		$pdf->SetXY($Xv11,36);
		$chaine=$civ_1." ".trim(strtoupper($nomtuteur))." ".trim(ucwords(strtolower($prenomtuteur)));
	...
	for($i=0;$i<count($ordre);$i++) {
		...
		// $pdf->WriteHTML('<B>'.trunchaine(sansaccentmajuscule(strtoupper($matiere)),19).'</B>');
...
$commentairedirection=recherche_com_scolaire($idEleve,$_POST["saisie_trimestre"]);
...
$pdf->SetFont('Arial','',10);
$pdf->SetXY(16,$YcomP2+5);
$pdf->WriteHTML($barre);
$pdf->SetXY(16,$YcomP3+5);
$pdf->WriteHTML($appreciation2);
$pdf->SetXY(16+74,$YcomP3+5);
$pdf->SetFont('Arial','',8);
...
if ((file_exists("./data/image_pers/logo_signature.jpg")) && ($_POST["ajsignature"] == "oui")){
	...
	$logox=$taille[0]/15;
	$logoy=$taille[1]/15;
	$pdf->Image("./data/image_pers/logo_signature.jpg","120",$YduplicaSign+3.5,$logox,$logoy);
}
