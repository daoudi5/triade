...
$CIE_FORM_version_bulletin_triade = "1.14";
$CIE_FORM_date_bulletin_triade = "02/10/2013"; // Par l'équipe Triade 
...
alert(\"Le bulletin édité par la Compagnie de Formation fonctionne uniquement avec les paramètres suivants : mode semestriel, tous les élèves.\");
...
if ($YmoyenneGeneral > 207) { // Valeur réduite pour générer un saut de page + haut car cadre commentaires + grand
	$pdf->AddPage();
	$YmoyenneGeneral=20;
}
...
if ($_POST["saisie_trimestre"] == "trimestre2" ) { // Si on est en bulletin de P2			
	...
	$pdf->SetFont('Arial','',9);
	$pdf->SetXY(17,$YmoyenneGeneralT+7.6);
...
$pdf->SetFont('Arial','',6);
$pdf->SetXY(16,$YmoyClasseGValue+14);
if ($_POST["saisie_trimestre"] == "trimestre2" ) { // Si on est en bulletin de P2			
...
if ($_POST["type_pdf"] == "mail"){
	...
	$classe_nom=TextNoCarac($classe_nom);
	$classe_nom=ereg_replace("/","_",$classe_nom);
	if (!is_dir("./data/pdf_bull/mail_$classe_nom")) { mkdir("./data/pdf_bull/mail_$classe_nom"); }
	$fichier=urlencode($fichier);
	$fichier="./data/pdf_bull/mail_$classe_nom/bulletin_".$nomEleve."_".$prenomEleve."_".$_POST["saisie_trimestre"].".pdf";
	@unlink($fichier); // destruction avant creation
	$pdf->output($fichier);
	$pdf->close();
	$pdf=new PDF();
...
if ($_POST["type_pdf"] == "mail"){
	...
	$texte_email.="<td><input type=button onclick=\"open('visu_pdf_admin.php?id=$fichier','_blank','');\" value=\"Visualiser\"  STYLE=\"font-family: Arial;font-size:10px;color:#CC0000;background-color:#CCCCFF;font-weight:bold;\"></td>";
	$texte_email.="</tr>";
...
if ($_POST["type_pdf"] == "mail"){
	print $texte_email;
	print "</table><br /><br />";
	print "<input type=submit value='Envoyer Email' class=BUTTON>";
	print "</form></ul>";
}
...
if ($_POST["type_pdf"] == "global"){
	...
	$classe_nom=TextNoCarac($classe_nom);
	$classe_nom=ereg_replace("/","_",$classe_nom);
	$fichier="./data/pdf_bull/bulletin_".$classe_nom."_".$_POST["saisie_trimestre"].".pdf";
	@unlink($fichier); // destruction avant creation
	$pdf->output($fichier);
	$bttexte=LANGPARAM33;
}
...
if ($_POST["type_pdf"] == "pers"){
	...
	$archive->create('./data/pdf_bull/'.$classe_nom);
...
echo "- Création des bulletins : ok</br>"; // affichage d'informations
*/
