...
if ($liste_matiere != "") {
	...
	$res=execSql($sql);
	$tabMat=chargeMat($res);
	for($i=0;$i<count($tabMat);$i++) {
		$tabMatFacul[$i]=$tabMat[$i][0];
	}
}
...
$moyenClasseGen=calculMoyenClasse2($idClasse,$eleveT,$dateDebut,$dateFin,$ordre,$tabMatFacul);
...
//-------------------------
	...
	for($g=0;$g<count($eleveT);$g++) {
		...
		for($t=0;$t<count($ordre);$t++) {
			...
			if ( $noteaff != "" ) {
				$cr=verifMatFacul($tabMatFacul,$idMatiere,$noteaff);
				if ($cr == "1") { 
					$noteaff=10 - $noteaff;
					$noteMoyEleGTempo=$noteMoyEleGTempo+ $noteaff;
...
for($j=$dep;$j<$nbEleveT;$j++) {  // premiere ligne de la creation PDF
	...
	$appreciation2="Appréciation du Président du Conseil de Classe";
	...
	$pdf->SetXY(15,30); // placement du cadre du nom de l eleve
	...
	$pdf->SetXY($Xv1,31); // placement du nom de l'eleve
	...
	$pdf->SetFont('Arial','',9);
	$pdf->SetFillColor(220);
	$pdf->SetXY(15,50); //  placement  cadre titre
	$pdf->MultiCell(184,11,'',1,'C',1);
	$pdf->SetXY(25,52); // placement contenu titre
...
//print $effbordure." $libelleMatiere <br>";
		...
		if ($sousmatiere != "") {
			$pdf->SetXY($Xmat+($largeurMat/2)+3,$Ymat+2.5);
			$pdf->SetFont('Arial','',7);
	...
	//	print "($cr)";
		if ($cr == "1") { 
			$noteaff=$noteaff - 10;
		//print $noteaff." ";
			$noteMoyEleG=$noteMoyEleG + $noteaff;
	...
	//		print " ini ";
		...
		//print $noteaff." ";
		        $noteMoyEleGTempo = $noteaff * $coeffaff;
		//print " ($noteMoyEleGTempo) / " ;
       	       		$noteMoyEleG=$noteMoyEleG + $noteMoyEleGTempo;
       		        $coefEleG=$coefEleG + $coeffaff;
		}
	//	print " $noteMoyEleG<br> ";
	}
}

//print "<br> <b>$coefEleG  $noteMoyEleG</b> <br>";
...
if ((file_exists("./data/image_pers/logo_signature.jpg")) && ($_POST["ajsignature"] == "oui")){
	...
	$logox=$taille[0]/15;
	$logoy=$taille[1]/15;
	$pdf->Image("./data/image_pers/logo_signature.jpg","150",$YmoyenneGeneralT-4,$logox,$logoy);
}
...
// fin affichage
// ($nbretard retard(s) / $nbabs demi-journée d'absence(s) / $nbheureabs heure(s) d'absence(s) ) " ;
$pdf->SetXY($XmoyClasseMaxValue+=15,$YmoyClasseMaxValue-=2);
$pdf->MultiCell($LargeurMG+20,10,'',1,'L',0);
$pdf->SetXY($XmoyClasseMaxValue+=1,$YmoyClasseMaxValue);
$pdf->SetFont('Arial','',8);
$pdf->WriteHTML("Absences (1/2 journée) : $nbabs ");
...
$pdf->SetFillColor(220);
$pdf->SetXY(15,$Ycom);
$pdf->MultiCell(155,$EpaisCom,'',1,'C',0);
$pdf->SetXY(155+15,$Ycom);
$pdf->MultiCell(29,$EpaisCom,'',1,'C',0);
$pdf->SetXY(155+15,$Ycom+1);
...
$pdf->SetFont('Arial','',$confPolice[0]);
$pdf->MultiCell(150,$confPolice[1],$commentaireprofp,'','','L',0); // commentaire de la direction (visa)
