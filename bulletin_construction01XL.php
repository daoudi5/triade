...
//	$commentaireDirection->set_valign('top');
	...
	for($j=$dep;$j<$nbEleveT;$j++) {  // premiere ligne de la creation PDF
		...
		$worksheet->write('A2',"$nom_etablissement",$titre1);
		$worksheet->write('A3',"$adresse",$normal);
		$worksheet->write('A4',"$postal $ville",$normal);
		$worksheet->write('A5',"$tel $ville",$normal);
	...
	// -------------------------------------------------------------------------------------------
		...
		$worksheet->write(8,0,"Matières",$matiere1);
		$worksheet->write_blank(8,1,$matiere1);

		$worksheet->write(8,2,"Appréciations",$matiere1); // ESAD
		$worksheet->write_blank(8,3,$matiere1);
		$worksheet->write_blank(8,4,$matiere1);
		...
		$worksheet->write('A10',"Intitulés",$colonne2);
		$worksheet->write('B10',"Enseignants",$colonne2); // ESAD coef
		
		$worksheet->write('H9',"Etudiant",$matiere1);
		$worksheet->write_blank(8,7,$matiere1);
		$worksheet->write_blank(8,8,$matiere1);

		$worksheet->write('H10',"MOYENNES",$colonne2);
		$worksheet->write('I10',"ECTS",$colonne2);
		 ...
		 */
			...
			for($i=0;$i<count($listeMatiere);$i++) {
				...
				if (($noteaffP11 < 10) && ($noteaffP11 != "")) { 
					$worksheet->write("H$ligne","$noteaffP11",$matiere44);
					$ects=0;	
				}else{
					if ($noteaffP11 != "") {
						$ects=recupECTS($idmatiere,$idClasse,$_POST["saisie_trimestre"]);
						$worksheet->write("H$ligne","$noteaffP11",$matiere4);
						if ($noteaffP11 >= 10) {
							$ectsTOTAL+=$ects;
						        $ectsUE+=$ects;	
						}
...
//				$worksheet->write("G$ligne","$moyeMatGenMaxaff",$matiere4);
				...
				$worksheet->write_blank($ligneN,6,$commentaire2);
...
//			$worksheet->write("G$ligneUE","$maxClassEU",$matiere2);
		...
		if ($affmoyengeneralexls == "oui") {	
			$worksheet->write("F$ligne","Moyenne générale : ",$M1);
			$worksheet->write("H$ligne","$moyEleveG",$M2);
