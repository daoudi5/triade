...
if(autorisation_module()) {
	...
	$groupe_frais_id = lire_parametre('groupe_frais_id', 0, 'POST');
	//***************************************************************************


	//*************** TRAITER L'OPERATION DEMANDEE ****************
	if($operation == "enregistrer") {
		if($groupe_frais_id > 0) {
			$sql= "DELETE FROM ".FIN_TAB_GROUPE_FRAIS." ";
			$sql.="WHERE groupe_id = $groupe_frais_id ";
			$res=execSql($sql);
			//echo $sql;
	...
	// Rechercher la liste des groupes de frais deja utilise dans les type de frais(=> ils ne pourront pas etre supprimes)
	$sql ="SELECT gf.groupe_id ";
	$sql.="FROM ".FIN_TAB_GROUPE_FRAIS." gf ";
	$sql.="INNER JOIN ".FIN_TAB_TYPE_FRAIS." tf ON tf.groupe_id = gf.groupe_id ";
	$sql.="ORDER BY gf.groupe_id ";
	
	
	$res_groupe_frais_utilises=execSql($sql);
	
	$groupe_frais_utilises = array();
	for($i=0;$i<$res_groupe_frais_utilises->numRows();$i++) {
		$ligne_tmp = $res_groupe_frais_utilises->fetchRow();
		$groupe_frais_utilises[$i] = $ligne_tmp[0];
	}
		...
		<table border="0" cellpadding="3" cellspacing="1" width="100%" bgcolor="#0B3A0C" height="85">
			<tr id="coulBar0">
				<td height="2" align="left">
					<b><font id="menumodule1" ><?php echo LANG_FIN_GROUPE_010; ?></font></b>
				</td>
			</tr>
			<tr id="cadreCentral0">
								...
								<td valign=top align="center">
									...
									<table border="0" cellpadding="0" cellspacing="0" align="center">							
										<tr>
											...
											<td valign="top" align="left">
												...
												<?php
													for($i=0; $i<$res->numRows(); $i++) {
														...
														$classe = '';
														if(in_array($ligne[0], $groupe_frais_utilises)) {
											...
											<td>&nbsp;</td>
											<td valign="top">
												<a href='javascript:;'  onMouseOver="AffBulle3('<?php echo LANG_FIN_GENE_002; ?>','./image/commun/info.jpg','<?php echo LANG_FIN_GROUPE_011; ?>', '');"  onMouseOut="HideBulle()";><img src="./image/help.gif" border=0 align=center></a>
											</td>
										</tr>
									</table>
							...
							<tr>
								<td align="center">
									<table border="0" align="center" cellpadding="4" cellspacing="0">
										<tr>
											<td align="center">
												<script language="javascript">buttonMagicSubmit("<?php print LANG_FIN_GROUPE_007?>","create"); //text,nomInput</script>
											</td>
											<td align="center">
					...
					<script language="javascript">
						...
						function valider_le_formulaire() {
							...
							if(obj.options[obj.selectedIndex].className == 'element_liste_grise') {
								alert("<?php echo LANG_FIN_GROUPE_012; ?>");
								if(valide) {
									obj.focus();
								}
								valide = false;
