...
// Set time rajouté le 02 Aout 2012 par ED
set_time_limit(0);
...
if(autorisation_module()) {
	...
	$date_reglement = lire_parametre('date_reglement', '', 'GET');
	$ordre_tri = lire_parametre('ordre_tri', 'nom_eleve', 'POST');
	//***************************************************************************
	
	// Initialiser la date limite si elle est vide
	if($date_reglement == '') {
		echo 'Vous devez donner une date de règlement';
		exit;
	}
	...
	if($operation == 'rechercher') {
		...
		$sql  = "SELECT el.elev_id, el.nom, el.prenom, ec.echeancier_id, ec.date_echeance, ec.montant, ec.numero_rib, cl.code_class, cl.libelle, i.annee_scolaire, i.date_depart, reg.montant ";
		...
		$sql .= "INNER JOIN ".FIN_TAB_REGLEMENT." reg ON reg.echeancier_id = ec.echeancier_id ";
		$sql .= "WHERE reg.date_reglement = '" . $date_reglement . "' ";
