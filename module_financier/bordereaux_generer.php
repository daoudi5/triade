...
if(autorisation_module()) {
	...
	$liste_reglement_id = lire_parametre('liste_reglement_id', '', 'POST');
	//***************************************************************************

	$nb ="SELECT max(numero_bordereau) ";
	$nb.="FROM ".FIN_TAB_REGLEMENT." ";
	$res = execSql($nb);
	$ligne_nb = $res->fetchRow();
	$numero_bordereau = $ligne_nb[0] + 1;
	
	
	
	
	// Verifier que l'on a bien toutes les donnees
	if($date_remise != '' && $type_reglement_id != 0 && ($type_reglement_id == $g_tab_type_reglement_id['cheque'] || $type_reglement_id == $g_tab_type_reglement_id['espece']) && $numero_bordereau != '' && $liste_reglement_id != '') {
		...
		$total = 0.0;
		$lignes_tableau = '';
		// $sql  = "SELECT montant, numero_cheque ";
		// $sql .= "FROM ".FIN_TAB_REGLEMENT." ";
		// $sql .= "WHERE reglement_id IN (" . $liste_reglement_id . ") ";
		// $sql .= "ORDER BY reglement_id ASC";
		
		//echo $sql;
		
		
		$sql  = "SELECT el.nom, el.prenom, cl.libelle, i.annee_scolaire, r.montant, r.numero_cheque ";
		...
		$sql .= "WHERE r.reglement_id IN (" . $liste_reglement_id . ") ";
		...
		$sql .= "SET numero_bordereau = '" . esc($numero_bordereau) . "' ";
		$sql .= ", date_remise_bordereau = '" . date_vers_bdd($date_remise) . "' ";
