...
if(autorisation_module()) {
	...
	$groupes_id = lire_parametre('groupes_id', array(), 'REQUEST');
	$groupes_id1 = lire_parametre('groupes_id1', array(), 'REQUEST');
	if($groupes_id1 != array())
	{
		$groupes_id = explode(',',$groupes_id1);
	}
					...
					<td valign="top" align="center">
										...
										<form name="formulaire_criteres" id="formulaire_criteres" action="<?php echo url_script(); ?>" method="post" onSubmit="">
											...
											<fieldset>
												...
												<table cellpadding="0" cellspacing="2" align="center">
													...
													<tr>
														<td align="right" valign="top"><?php echo LANG_FIN_GROUPE_018; ?>&nbsp;<small>(<?php echo count($groupes_id); ?>)</small>&nbsp;:</td>
														<td align="left">
															<div>
																<?php
																if($groupes_frais->numRows() > 0) {
																?>
																<table cellspacing="1" cellpadding="3" border="0" bgcolor="#cccccc">
																<?php
																	for($i=0; $i<$groupes_frais->numRows(); $i++) {
																		$res = $groupes_frais->fetchInto($ligne, DB_FETCHMODE_DEFAULT, $i);
																?>
																	<tr bgcolor="#ffffff">
																		<td align="left">
																			<?php
																			$checked = '';
																			if(in_array($ligne[0], $groupes_id)) {
																				$checked = 'checked';
																			}
																			?>
																			<input type="checkbox" name="groupes_id[]" id="groupes_id[]" value="<?php echo $ligne[0]; ?>" <?php echo $checked; ?> <?php echo $disabled_cadre; ?>>
																		</td>
																...
																{
																?>
																<input type="hidden" name="groupes_id" id="groupes_id" value="">
																<?php
																}
																?>
															</div>
						...
						<script language="javascript">
							...
							function onclick_imprimer() {							
									...
									liste_fenetre[liste_fenetre.length] = open('<?php echo site_url_racine(FIN_REP_MODULE); ?>module_financier/editions_inscriptions_rechercher_par_groupes.php?mode_affichage=impression&operation=rechercher&code_class=<?php echo urlencode($code_class); ?>&annee_scolaire=<?php echo urlencode($annee_scolaire); ?>&groupes_id1=<?php echo implode(',',$groupes_id); ?>','fenetre_editer_' + liste_fenetre.length,'width=800,height=600,resizable=yes,scrollbars=yes');
						...
						<form name="formulaire_modif" id="formulaire_modif" action="<?php echo $g_chemin_relatif_module; ?>type_frais_modif.php" method="post">
							<input type="hidden" name="groupes_id" id="groupes_id" value="0">
						</form>
						<form name="formulaire_annuler" id="formulaire_annuler" action="<?php echo $g_chemin_relatif_module; ?>editions.php" method="post">
						</form>
						<form name="formulaire_export_excel" id="formulaire_export_excel" action="<?php echo $g_chemin_relatif_module; ?>editions_inscriptions_rechercher_par_groupes_excel.php" method="post" target="">
							<input type="hidden" name="annee_scolaire" id="annee_scolaire" value="<?php echo $annee_scolaire?>">
							<input type="hidden" name="groupes_id" id="groupes_id" value="<?php echo implode(',',$groupes_id)?>">
