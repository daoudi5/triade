...
for($g=0;$g<count($eleveT);$g++) {
	for($t=0;$t<count($ordre);$t++) {
		...
		$verifGroupe=verifMatiereAvecGroupe($ordre[$t][0],$idEleve,$idClasse,$ordre[$t][2]);
		if ($verifGroupe) {  continue; } // verif pour l'eleve de l'affichage de la matiere
		$idgroupe=verifMatierAvecGroupeRecupId($idMatiere,$idEleve,$idClasse,$ordre[$t][2]);
                if ($idgroupe == "0") {
                        $noteaff=moyenneEleveMatiereSansExam($idEleve,$ordre[$t][0],$dateDebut,$dateFin,$idprof);
                }else{
			$noteaff=moyenneEleveMatiereGroupeSansExam($idEleve,$ordre[$t][0],$dateDebut,$dateFin,$idgroupe,$idprof);
                }
		$noteExam=moyenneEleveMatiereExamen($idEleve,$idMatiere,$dateDebut,$dateFin,$idprof,"BTS Blanc");
                if ($noteaff != "") {
                        $moyenneMatiere=$noteaff*1;
                        $nbnote=1;
                }
		$coeffaff=recupCoeff($ordre[$t][0],$idClasse,$ordre[$t][2]);
                if (($noteExam != "") && ($coeffaff != 0)){
                        $moyenneMatiere+=$noteExam*2;
                        $nbnote+=2;
                }
                if ($nbnote != 0) {
                        $moyenneMatiere=$moyenneMatiere/$nbnote;
                }
		if ($moyenneMatiere != "" ) {
		       	$noteMoyEleG=$moyenneMatiere + $noteMoyEleG;
			$coefEleG=$coefEleG + 1;
		}
		unset($noteaff);
		unset($moyenneMatiere);
		unset($nbnote);
	}
        if ($noteMoyEleG != "") { $moyenEleve2=moyGenEleve($noteMoyEleG,$coefEleG); }
	unset($coefEleG);
        if (trim($moyenEleve2) != "") {
...
for($j=$dep;$j<$nbEleveT;$j++) {  // premiere ligne de la creation PDF
	...
	$barre="______________________________________________________________________________________________________________________";
	...
	for($i=0;$i<count($ordre);$i++) {
		...
		//mise en place Note CC
		$pdf->SetFont('Arial','',8);
		$pdf->SetXY($Xcoeff,$Ycoeff);
		if ($idgroupe == "0") {
			$noteaff=moyenneEleveMatiereSansExam($idEleve,$ordre[$i][0],$dateDebut,$dateFin,$idprof);
			$noterang=moyenneEleveMatiere($idEleve,$ordre[$i][0],$dateDebut,$dateFin,$idprof);
		}else{
			$noterang=moyenneEleveMatiereGroupe($idEleve,$ordre[$i][0],$dateDebut,$dateFin,$idgroupe,$idprof);
		...
		$pdf->MultiCell(10,$hauteurMatiere,"$noteaff1",1,'C',0);


		//mise en place Note BTS blanc
		$pdf->SetFont('Arial','',8);
		$pdf->SetXY($Xcoeff+10,$Ycoeff);
		$noteExam=moyenneEleveMatiereExamen($idEleve,$idMatiere,$dateDebut,$dateFin,$idprof,"BTS Blanc");	
		if (($noteExam < 10) && ($noteExam != "")) { $noteExam="0".$noteExam; }
		$pdf->MultiCell(10,$hauteurMatiere,"$noteExam",1,'C',0);

		// mise en place de la colonne coeff
		$pdf->SetFont('Arial','',8);
		$pdf->SetXY($Xcoeff+20,$Ycoeff);
		$coeffaff=recupCoeff($ordre[$i][0],$idClasse,$ordre[$i][2]);
		$pdf->MultiCell(10,$hauteurMatiere,"$coeffaff",1,'C',0);

		// mise en place de la colonne Total
		$pdf->SetFont('Arial','',8);
		$pdf->SetXY($Xcoeff+30,$Ycoeff);
		if ($coeffaff != 0) { $total=$coeffaff*$noteExam; }
		$pdf->MultiCell(10,$hauteurMatiere,"$total",1,'C',0);
		unset($total);

		// mise en place moyenne eleve
		$pdf->SetFont('Arial','',8);
		$pdf->SetXY($Xcoeff+40,$Ycoeff);
		...
		if ($nbnote != 0) {
			$moyenneMatiere=$moyenneMatiere/$nbnote;
			$moyenneMatiereG=$moyenneMatiere;
			if (($moyenneMatiere < 10) && ($moyenneMatiere != "")) { $moyenneMatiere="0".$moyenneMatiere; }
			$moyenneMatiere=number_format($moyenneMatiere,'2','.','');
		}
		$pdf->MultiCell(15,$hauteurMatiere,"$moyenneMatiere",1,'C',1);
		unset($nbnote);
		unset($moyenneMatiere);
		...
		$pdf->SetFont('Arial','',8);
		$pdf->SetXY($Xcoeff+55,$Ycoeff);
		$pdf->MultiCell(10,$hauteurMatiere,"$moyeMatGenaff",1,'C',0);
		...
		$pdf->SetFont('Arial','',8);
		$pdf->SetXY($Xcoeff+65,$Ycoeff);
		$pdf->MultiCell(12,$hauteurMatiere,"$rang/$nbtotalRang",1,'C',0);	
		$rang="";
		$noterang="";
		//----------------------------------------------------------------------
	...
	$pdf->SetXY($Xcoeff+77,$Ymoyclasse);
	$pdf->MultiCell(77,$hauteurMatiere,'','1','L',0);
	$pdf->SetXY($Xcoeff+77,$Ymoyclasse);
	...
	$Ymoyclasse=$Ymoyclasse + $hauteurMatiere;
        ...
        if ($moyenneMatiereG != "" ) {
        	$noteMoyEleG=$moyenneMatiereG + $noteMoyEleG;
                $coefEleG=$coefEleG + 1;
        }
...
if ($affichemoyengeneral == "oui") {
	...
	$pdf->SetFont('Arial','',9);
	$pdf->SetXY(15-10,$YmoyenneGeneral);
	$pdf->MultiCell($LargeurMG,10,'',1,'L',0);
	$pdf->SetXY(17-10,$YmoyenneGeneralT);
	...
	$moyenEleveaff=ereg_replace(',','.',$moyenEleveaff);
	...
	$pdf->SetFont('Arial','',9);
	$pdf->SetXY($XmoyClasseGValue-1,$YmoyClasseGValue);
...
$pdf->SetFont('Arial','',$confPolice[0]);
$pdf->MultiCell(190,$confPolice[1],$commentairedirection,'','','L',0); // commentaire de la direction (visa)
...
$pdf->SetFont('Arial','',$confPolice[0]);
$pdf->MultiCell(190,$confPolice[1],$commentaireprofp,'','','L',0); // commentaire de la prof P (visa)
