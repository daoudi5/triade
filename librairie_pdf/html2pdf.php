...
function PDF($orientation='P',$unit='mm',$format='A4',$_title,$_url,$_debug=false)
	{
		...
		$this->fontlist=array("Times","Courier","ae_AlHor", "ae_AlBattar");
...
function Footer() {
                global $afficheNbPage;
		global $footer;
		global $footerlien;
		global $nofooterPDF;
		//Go to 1.5 cm from bottom
		if ($nofooterPDF == "OUI") return ;
                $this->SetY(-5);
                ...
                if (($afficheNbPage == "oui") || ($afficheNbPage == 1)) {
                        $info.="Page ".$this->PageNo().'/{nb}';
                }
		if ($info != "") { $info=" - $info"; }
		if ($footer != "") {
			$footer=strip_tags($footer);
	                $this->Cell(0,4,"$footer $info",0,0,'C',0,"$footerlien");
		}elseif($nofooterPDF == "ISMAPP") {
			$this->SetY(-17);
	                $this->SetFont('Arial','B',6);
	                $this->Cell(0,3,"Institut Supérieur du Management Public et Politique",0,0,'C',0,'');
	                $this->SetFont('Arial','',6);
			$this->SetY(-15);
	                $this->Cell(0,3,"Etablissement privé d'enseignement supérieur technique, reconnu par l'Etat",0,0,'C',0,'');
			$this->SetY(-13);
	                $this->Cell(0,3,"Organisme de formation déclaré sous le n° 11 75 54276 75",0,0,'C',0,'');
			$this->SetY(-9);
	                $this->Cell(0,3,"80, rue Taitbout - 75009 PARIS",0,0,'C',0,'');
			$this->SetY(-7);
                	$this->Cell(0,3,"Tel. +33(0)1 55 50 12 40 - Fax. +33(0)1 55 50 12 49",0,0,'C',0,'');
			$this->SetY(-5);
                	$this->Cell(0,3,"Courriel : direction@ismapp.com - Web : www.ismapp.com",0,0,'C',0,'');
