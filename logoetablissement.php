...
if (isset($_POST["create"])) {
    ...
    if ( (!empty($photo)) &&  ($size <= 2000000)) {
		...
		if (verifImageJpg($type))  {
			$nomphoto="logo_bulletin$idsite.".$type;
			move_uploaded_file($tmp_name,"data/image_pers/$nomphoto");
			enr_photo_bulletin($nomphoto,$idsite);
			history_cmd($_SESSION["nom"],"PHOTO","AJOUT Etablissement");
...
<?php
	...
	if (count($photo) > 0) {
		if (file_exists("./data/image_pers/".$photo[0][0])) {
			$logo="<img src='image.php?id=./data/image_pers/".$photo[0][0]."' width='96' height='96' ><br>[<a href='logoetablissement.php?supp=logo'>".LANGacce21."</a>]";
...
<td width=65% align=left><br><?php print LANGLOGO1?> :<br> <input type="file" name="photo" size=30 ><?php print $alert ?></td></tr>
<tr><td colspan=2 align=center><br>
<table align=center><tr><td>
<script language=JavaScript>buttonMagicSubmit('<?php print LANGLOGO2?>','create'); //text,nomInput</script>
