...
$partiel = array(0=> array ('min'=>'','max'=>''),1=> array ('min'=>'','max'=>''),2=> array ('min'=>'','max'=>''),3=> array ('min'=>'','max'=>''),4=> array ('min'=>'','max'=>''),5=> array ('min'=>'','max'=>''),6=> array ('min'=>'','max'=>''),7=> array ('min'=>'','max'=>''),8=> array ('min'=>'','max'=>''),9=> array ('min'=>'','max'=>''),10=> array ('min'=>'','max'=>''),11=> array ('min'=>'','max'=>''),12=> array ('min'=>'','max'=>''),13=> array ('min'=>'','max'=>''),14=> array ('min'=>'','max'=>''),15=> array ('min'=>'','max'=>''));
	...
	//					if ( $moyenne_gen_partiel > -1 && $moyenne_gen_partiel!='') {$somme_coef_gen_partiel+=$somme_coef;}
						$moyenne_ue=0;
						$somme_coef=0;	
					...
					if ($moyenne_periode_ue> -1 && $moyenne_periode_ue!='') {
		...
		//			$nom_ue=$ue[$nb_ue][2];
				...
				$somme_coef+=$notepartiel[0][6];
				 // recup chaque note de partiel pour min max et moyene / matiere
								if ($notepartiel[0][0]>-1) {$moyenne_matiere_part[$idMatiere][]=$notepartiel[0][0];}
				}
...
// modif ambis 9/12/08				$moyenne_periode=$moyenne_periode/$nb_note_periode;
		...
		//		$pdf->MultiCell(20,6,''.number_format($moyenne_periode_ue,2,'.','').'',1,'C',0);
...
//		$moy_mat= vatel_moyenne ($moyenne_matiere_per[4]);
//		print ('<br>min: '.$moy_mat['min']);
//		print (' - max: '.$moy_mat['max']);
//		print (' - moy : '.$moy_mat['moy']);
		?>
		</pre>-->
		<?php
...
for ($j=0;$j<count($eleveT);$j++) {  // premiere ligne de la creation PDF
	...
	$nomprenom= trunchaine($nomprenom,19);
...
for ($nb_ue=0;$nb_ue<count($ue);$nb_ue++) { // boucle pour chaque UE
	...
	//		if ($noteperiode[0][0]!=0 && $noteperiode[0][0]!='') {$nb_mat+=1;}
		...
		if ($ecrit!='') {
			...
			if ($moyenne_ue>-1 && $moyenne_ue!='') {$moyenne_gen_partiel+=$moyenne_ue;$moyenne_ue=$moyenne_ue/$somme_coef;$somme_coef_gen_partiel+=$somme_coef;}
			
			// Affiche la moy. partiel UE de l'etudiant
			if (format_moyenne($moyenne_ue) > -1 ) {
			...
			if ($moyenne_periode_ue > -1 && $moyenne_periode_ue!='') {$nb_mat_gen++;}
			$moyenne_periode_ue=0;
...
//		print_r($notepartiel);
			...
			if ($notepartiel[0][0] > -1 && $notepartiel[0][0]!='') {$nb_mat+=1;
					$moyenne_ue+=$notepartiel[0][6]*$notepartiel[0][0];
		$somme_coef+=$notepartiel[0][6];
			}
		$pdf->SetFont('Arial','',6);
...
//		$pdf->MultiCell($lMat_mid2,$hauteurMatiere,$liste_note,1,'C',0);
		...
		if ($moyenne_periode >-1 && $nb_note_periode>0) { 
...
// traitement de la derniere UE
	...
	// print ('nbre mat: '.$nb_mat_per. '<br>');
			...
			$moyenne_periode_ue=$moyenne_periode_ue/($nb_mat_per);			$moyenne_periode_gen+=$moyenne_periode_ue;
			$som_coef_periode_ue_total+= $som_coef_periode_ue;
			$som_coef_periode_ue=0;
			$moy_per_tot+=$moyenne_periode_ue;
			$nb_ue_per++;
			if ($moyenne_periode_ue>-1) { 
			...
			$pdf->MultiCell($lMat_mid,4,format_moyenne($affiche_note),1,'C',0);$pdf->SetLineWidth(0.2);
