...
//	$pdf->SetTitle("RELEVE DES NOTES - $nomEleve $prenomEleve");
	...
	$nomprenom=trunchaine($nomprenom,40);
...
// premiere ligne
	...
	$Xmat1=$Xmat+$largeurMat+30;
	$pdf->SetXY($Xmat1,$Ymat);
	$pdf->MultiCell(90,6,'',1,'C',0);
...
// deuxieme ligne
	...
	$pdf->SetXY($Xmat1,$Ymat);
	$pdf->MultiCell(15,6,'Note',1,'C',1);
//	$Xmat1=$Xmat1+20;
	//$pdf->SetXY($Xmat1,$Ymat);
//	$pdf->MultiCell(20,6,'Coef',1,'C',0);
	$Xmat1=$Xmat1+15;
	$pdf->SetXY($Xmat1,$Ymat);
	$pdf->MultiCell(90,6,'Commentaire des enseignants',1,'C',0);
	$Xmat1=$Xmat1+80;
	$pdf->SetXY($Xmat1,$Ymat);
...
for ($nb_ue=0;$nb_ue<count($ue);$nb_ue++) { // boucle pour chaque UE
		...
		// mise en place des matieres
		$largeurMat=60;
		$hauteurMatiere=15; // taille du cadre matiere
	...
	//		if ($noteperiode[0][0]!=0 && $noteperiode[0][0]!='') {$nb_mat+=1;}
		...
		if ($ecrit!='') {
			...
			$pdf->SetFont('Arial','',7);
			$pdf->SetXY($Xmat,$Ymat);
			$pdf->MultiCell(60,6,' Moyenne min.:'.format_moyenne($partiel[$id_ue]['min']).' - max:'.format_moyenne($partiel[$id_ue]['max']),'T','L',1);
			$pdf->SetFont('Arial','',7);
			$pdf->SetXY($Xmat+60,$Ymat);
			$pdf->MultiCell(30,6,''.format_moyenne($moyenne_ue).'',1,'C',1);
...
//		$pdf->WriteHTML(''.$nom_ue.'');
	...
	////	$moyenne_ue=$moyenne_ue/$somme_coef;
		...
		$pdf->WriteHTML(''.trunchaine(strtoupper(sansaccent(strtolower($matiere))),35).'');
...
//		$pdf->MultiCell(20,$hauteurMatiere,$noteperiode[0][6],'LR','C',0);
//print('coef_per'.$noteperiode[$nb_eu][6].'; ');		
		// note
		$liste_note='';
		$moyenne_periode='';
		$nb_note_periode=0;
...
// modif ambis 12/12/08		$moyenne_periode=$moyenne_periode/$nb_note_periode;
		...
		if ($nb_note_periode>0) {$nb_note_periode_ue++;$nb_mat_per++;}
		$pdf->SetFont('Arial','',7);
		$pdf->SetXY($Xmat+$largeurMat+30,$Ymat);
		...
		$pdf->MultiCell(90,$hauteurMatiere,"",'LR','L',0);
		$pdf->SetXY($Xmat+$largeurMat+30,$Ymat);
		$pdf->MultiCell(90,3,"$commentaireeleve",'0','L',0);
