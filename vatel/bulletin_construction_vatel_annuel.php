...
//			if ($notepartiel[0][0]!=0 && $notepartiel[0][0]!='') {$nb_mat+=1;}
		...
		//	print ($moyenne_ue.' // ' );	
		/*	if ($moyenne_ueS2 >= 0) {
				$moyenne_ueS2=$moy_partiel_euS2/$som_coef_partiel_euS2;
				$moyenne_ueS2=number_format($moyenne_ueS2,'2','.','');
				$moyenne_gen_partielS2+=$moyenne_ueS2*$som_coef_partiel_euS2;
				$nbMoyenPartielS2+=$som_coef_partiel_euS2;
			}
			if ($moyenne_ueS1 >= 0) {
				$nbMoyenPartielS1+=$som_coef_partiel_euS1;
			}
...
//			print ('nbre2 : '.$nb_mat);
		...
		//	$pdf->MultiCell(40,6,' Moyenne min.:'.format_moyenne($periode[$id_ue]['min']).'- max:'.format_moyenne($periode[$id_ue]['max']),'T','C',0);
			...
			if (($moyenne_periode1 != "") && ($moyenne_periode1 >= 0 )) {
			...
			if (($moyenne_periode2 >= 0) && ($moyenne_periode2 != "" )) {
...
//	print ('som coef'.$som_coef_periode_ue);
		...
		//	$pdf->MultiCell(40,6,' Moyenne min.:'.format_moyenne($periode[$id_ue]['min']).' - max:'.format_moyenne($periode[$id_ue]['max']),'T','C',0);
			...
			$pdf->SetXY($Xmat+120,$Ymat);
                        $pdf->MultiCell(20,6,''.format_moyenne($moyenne_periode2).'',1,'C',0);
			if (($moyenne_periode1 != '') && ($moyenne_periode2 >= 0)) {
				$moyenne_annuelle_periode2+=$moyenne_periode2;
                                $nbcoef_annuelle_periode2++;
				$moyenne_periode_annuelle_ue+=$moyenne_periode2;
                                $nbcoef_periode_annuelle_ue++;
...
//print_r ($periode);
			...
			$pdf->SetFont('Arial','B',7);
                        $pdf->MultiCell(45,6,' Moyenne Générale Annuelle /20 :','1','R',1);
                        // sam 
						$pdf->SetFont('Arial','B',7);
			$pdf->SetXY($Xmat,$Ymat+6);
			if ($nbcoef_annuelle_partiel1 > 0) { $moyenne_annuelle_partiel1=$moyenne_annuelle_partiel1/$nbcoef_annuelle_partiel1; $nbMoyPartielG++; }
			if ($nbcoef_annuelle_partiel2 > 0) { $moyenne_annuelle_partiel2=$moyenne_annuelle_partiel2/$nbcoef_annuelle_partiel2; $nbMoyPartielG++; } 
			//$moyenne_gen_partiel=$moyenne_gen_partiel/$somme_coef_gen_partiel;
			$moyenne_annuelle_ue=($moyenne_annuelle_partiel1+$moyenne_annuelle_partiel2)/ $nbMoyPartielG;
			$moyenne_annuelle_ue=number_format($moyenne_annuelle_ue,'2','.','');
			unset($nbMoyPartielG);
...
//			$pdf->WriteHTML('(min.:'.format_moyenne($moyenne_per[0]).' - max:'.format_moyenne(end($moyenne_per)).')</b>');
			...
			$Ymat=242; // pour decaller le bloc en bas de la page 230
			$moyenne_gen_partiel='';
			$somme_coef_gen_partiel=0;
//			$pdf->SetFont('Arial','',7);
