...
$appliPratique=$_POST["appliPratique"];
$sautPage=$_POST["sautPage"];

config_param_ajout($appliPratique,"appliPratique");
...
//	$pdf->SetTitle("RELEVE DES NOTES - $nomEleve $prenomEleve");
	$pdf->SetTitle("Transcript - $classe_nom");
...
//		$pdf->Image($logo); // modif ambis 12/02/09 pour resoudre prb erreur defichier PDF pour ecole bordeaux classe L2B  prob de marge
		$pdf->Image($logo,20,0); // modif ambis 06/12/11
	}
	// fin du logo
	//
...
//	$pdf->MultiCell(25,10,"$Trimestre Année scolaire $anneeScolaire",1,'C',0);
	...
	$pdf->MultiCell(25,24,"",1,'C',0);
			...
			if ($Trimestre  == "1er  SEMESTRE") $Trimestre="1st Period" ;
			if ($Trimestre  == "2eme SEMESTRE") $Trimestre="2d  Period" ;
			$pdf->SetXY(168,0);
			$pdf->WriteHTML($Trimestre);
			
			$pdf->SetXY(167,8);
			$pdf->WriteHTML("Teaching year");
...
////	$pdf->WriteHTML();
	...
	for ($ik=0;$ik<=count($dataadresse);$ik++) {
		...
		$pdf->SetXY($Xv1,$Yv1); 
		$pdf->WriteHTML("<b>Name first name</b> : $nomprenom");
		$pdf->SetXY($Xv1,$Yv1+4);
...
// 		$pdf->WriteHTML("<b>Classe </b>: $lib_classe - ".strtoupper($classe_nom));
		$pdf->WriteHTML("<b>Class </b>: ".Vatel_Classe_desc($idClasse)." - ".strtoupper($classe_nom)); // modif pour integrer le descriptif long de la classe
	...
	$pdf->SetXY($Xmat,$Ymat);
	$pdf->MultiCell(180,7,'TRANSCRIPT ',1,'C',0);
	$pdf->SetFont('Arial','B',8);
	$pdf->SetXY($Xmat+117,$Ymat+1);
	$pdf->WriteHTML('From '.$dateDebut.' to '.$dateFin);
...
// premiere ligne
	$pdf->SetFont('Arial','',8);
	$pdf->SetXY($Xmat,$Ymat);
	$pdf->MultiCell($largeurMat,6,"Teaching departement",1,'L',0);
	$pdf->SetXY($Xmat+$largeurMat,$Ymat);
	$pdf->SetFillColor(198,209,229);  // couleur du cadre de l'eleve
	$pdf->MultiCell(40,6,'EXAMS',1,'C',1);
	$pdf->SetFillColor(255);  // fond blanc
	$Xmat1=$Xmat+$largeurMat+40;
	$pdf->SetXY($Xmat1,$Ymat);
	$pdf->MultiCell(100,6,'TESTS',1,'C',0);
...
// deuxieme ligne
	$pdf->SetFont('Arial','',8);
	$pdf->SetXY($Xmat,$Ymat);
	$pdf->MultiCell($largeurMat,6,'Subject',1,'C',0);
	$pdf->SetXY($Xmat+$largeurMat,$Ymat);
	$pdf->SetFillColor(198,209,229);  // couleur du cadre de l'eleve
	$pdf->MultiCell(20,6,'Coef.',1,'C',1);
	$pdf->SetFillColor(198,209,229);  // fond blanc
	$Xmat1=$Xmat+$largeurMat+20;
	$pdf->SetXY($Xmat1,$Ymat);
	$pdf->MultiCell(20,6,'Mark',1,'C',1);
//	$Xmat1=$Xmat1+20;
	//$pdf->SetXY($Xmat1,$Ymat);
//	$pdf->MultiCell(20,6,'Coef',1,'C',0);
	$Xmat1=$Xmat1+20;
	$pdf->SetXY($Xmat1,$Ymat);
	$pdf->MultiCell(80,6,'Mark ( Coef. 1 )',1,'C',0);
	$Xmat1=$Xmat1+80;
	$pdf->SetXY($Xmat1,$Ymat);
	$pdf->MultiCell(20,6,'Average',1,'C',0);
...
for ($nb_ue=0;$nb_ue<count($ue);$nb_ue++) { // boucle pour chaque UE
	...
	//		if ($noteperiode[0][0]!=0 && $noteperiode[0][0]!='') {$nb_mat+=1;}
		...
		if ($ecrit!='') {
			...
			$pdf->SetFont('Arial','',7);
			$pdf->SetXY($Xmat,$Ymat);
			//$pdf->MultiCell(40,6,' Average lower.:'.format_moyenne($partiel[$id_ue]['min']).' - higher:'.format_moyenne($partiel[$id_ue]['max']),'T','C',1);
			$pdf->MultiCell(45,6,'Average lower:'.format_moyenne($partiel[$id_ue]['min']).'-higher:'.format_moyenne($partiel[$id_ue]['max']),'0','L',0);
			$pdf->SetXY($Xmat,$Ymat);
			$pdf->MultiCell(40,6,'','T','L',1);
			//$pdf->MultiCell(40,6,' Average lower.:'.format_moyenne($partiel[$id_ue]['min']).' - higher:'.format_moyenne($partiel[$id_ue]['max']),'T','L',1);
			$pdf->SetFont('Arial','',7);
			$pdf->SetXY($Xmat+40,$Ymat);
			$pdf->MultiCell(40,6,''.format_moyenne($moyenne_ue).'',1,'C',1);
...
//			print ('nbre2 : '.$nb_mat);
			...
			//$pdf->MultiCell(40,6,' Average lower.:'.format_moyenne($periode[$id_ue]['min']).'- higher:'.format_moyenne($periode[$id_ue]['max']),'T','C',0);
			//$pdf->MultiCell(40,6,' Average lower.:'.format_moyenne($partiel[$id_ue]['min']).' - higher:'.format_moyenne($partiel[$id_ue]['max']),'T','L',1);
			$pdf->MultiCell(45,6,'Average lower:'.format_moyenne($partiel[$id_ue]['min']).'-higher:'.format_moyenne($partiel[$id_ue]['max']),'0','L',1);
			$pdf->SetXY($Xmat+120,$Ymat);
			$pdf->MultiCell(40,6,'','T','L',1);
			$pdf->SetFont('Arial','',7);
			$pdf->SetXY($Xmat+160,$Ymat);
			$pdf->MultiCell(20,6,''.format_moyenne($moyenne_periode_ue).'',1,'C',0);
...
//		$pdf->WriteHTML(''.$nom_ue.'');
	...
	////	$moyenne_ue=$moyenne_ue/$somme_coef;
		...
		$matiereVerif=strtoupper(sansaccent(strtolower($matiere)));
		
	
//-----------------------------------------------------------------------
                // Periode
		if ($appliPratique == "oui") {
			if (trim($matiereVerif) == "APPLICATIONS PRATIQUES") {
		                $noteperiode= recupNoteperiode($idEleve,$idMatiere,$dateDebut,$dateFin);
		                $liste_note='';
		                $moyenne_periode='';
		                $nb_note_periode=0;
		                ...
		                for ($nb_note=0;$nb_note<count($noteperiode);$nb_note++) {
		                        ...
		                        } elseif ($noteperiode[$nb_note][0] == -1){
		                                $liste_note = $liste_note." Abs. - ";
		                        }
		                }
				...
				$moyenne_application_pratique=$moyenne_periode;
			}
		}
//---------------------------------------------------------------------
		...
		if ($appliPratique == "oui") {
			if (trim($matiereVerif) == "APPLICATIONS PRATIQUES") {
				...
				$moy_coef_partiel=number_format($moy_coef_partiel,'0','','');
			}
		}	
		//print ($idEleve.'**'.$idMatiere.'**'.count($notepartiel).'**'.$moy_coef_partiel.'<br>');
		$pdf->SetFont('Arial','',7);
		$pdf->SetXY($Xmat+$largeurMat,$Ymat);
		$pdf->MultiCell(20,$hauteurMatiere,$moy_coef_partiel  ,'LR','C',1);
		$affich_coef_partiel='';
//print($nb_ue.'coef_pa'.$notepartiel[0][6].' ;');
		...
		if ($appliPratique == "oui") {
			if (trim($matiereVerif) == "APPLICATIONS PRATIQUES") {
				$affiche_note = $moyenne_application_pratique;		
			}
		}	
		$pdf->SetFont('Arial','',7);
		$pdf->SetXY($Xmat+$largeurMat+20,$Ymat);
		$pdf->MultiCell(20,$hauteurMatiere,$affiche_note,'LR','C',1);
...
*/
		...
		$som_coef_periode=0;
		$moyenne_periode_ue+=$moyenne_periode;
//	print ("total: ".$moyenne_periode." - nb :".$nb_note_periode."<br>");
		if ($nb_note_periode>0) {$nb_note_periode_ue++;$nb_mat_per++;}
		$pdf->SetFont('Arial','',7);
		$pdf->SetXY($Xmat+$largeurMat+40,$Ymat);
		$pdf->MultiCell(80,$hauteurMatiere,$liste_note,'LR','C',0);
//print('note_per'.$liste_note.'; ');	
		//$commentaireeleve=cherche_com_eleve($idEleve,$idMatiere,$idClasse,$_POST["saisie_trimestre"],$idprof,$idgroupe);
		//$commentaireeleve=ereg_replace("\n"," ",$commentaireeleve);
		...
		if ($moyenne_periode >=0 && is_numeric($moyenne_periode) && $nb_note_periode>0) { 
			$moyenne_periode=number_format($moyenne_periode,2,'.','');
			$pdf->SetFont('Arial','',7);
			$pdf->SetXY($Xmat+$largeurMat+120,$Ymat);
...
//			$moyenne_gen_partiel+=($moyenne_ue*$somme_coef);
			if ($moyenne_ue >=0 && is_numeric($moyenne_ue)){
				...
				$moyenne_gen_partiel+=$moyenne_ue;
				$moyenne_ue=$moyenne_ue/$somme_coef;						$somme_coef_gen_partiel+=$somme_coef;
			}
			$pdf->Line($Xmat,$Ymat,$Xmat+180,$Ymat);
			$pdf->SetFont('Arial','',7);
			$pdf->SetXY($Xmat,$Ymat);
...
if ($sautPage == "oui") {
	...
	if ($Ymat >= 230) { 
		$pdf->AddPage(); $Ymat=10; 
			...
			$pdf->WriteHTML('<b>EXAM AVERAGE / 20</b>');
			$pdf->SetFont('Arial','',8);
			$pdf->SetXY($Xmat+15,$Ymat+5);
			$pdf->WriteHTML('<b> EXAMS :                              '.format_moyenne($moyenne_gen_partiel).'</b>');
			$pdf->SetFont('Arial','',7);
			$pdf->SetXY($Xmat+30,$Ymat+8);
			$pdf->WriteHTML('Rank  : '.((array_search($moyenne_gen_partiel, $moyenne_par))+1).'/'.count($eleveT).'  - (lower.:'.format_moyenne(end($moyenne_par)).' - higher:'.format_moyenne($moyenne_par[0]).')');
			$moyenne_gen_partiel='';
			$somme_coef_gen_partiel=0;
//			$pdf->SetFont('Arial','',7);
...
// periode
		// print ("moy: ".$moyenne_periode_gen."nb: ".$nb_mat_gen."<br>");
			...
			$pdf->WriteHTML('<b> TESTS AVERAGE :         '.format_moyenne($moyenne_periode_gen).'</b>');
			$pdf->SetFont('Arial','',7);
			$pdf->SetXY($Xmat+140,$Ymat+8);
			$pdf->WriteHTML('(lower.:'.format_moyenne($moyenne_per[0]).' - higher:'.format_moyenne(end($moyenne_per)).')</b>');
			$moyenne_periode_gen='';
			$nb_mat_gen=0;
//			$pdf->SetFont('Arial','',7);
...
// FIN Mise en place des moyenens general partiel et periode
	...
	$pdf->SetFont('Arial','',8);
	$pdf->WriteHTML("<b>Number of half days absence : </b>".$nbabs .' - <b>Number of late arrival :</b> '.$nbretard);
...
$pdf->SetFont('Arial','B',8);
$pdf->WriteHTML("COMMENTS FROM HEAD OF STUDIES :");
...
?>
<br><ul><ul>
<input type=button onClick="open('visu_pdf_bulletin.php?id=<?php print $fichier?>&idclasse=<?php print $_POST["saisie_classe"] ?>','_blank','');" value="<?php print $bttexte ?>"  class='btn btn-primary btn-sm  vat-btn-footer'  >
</ul></ul>
<?php // ----------------------------------------------------------------------------------------------------------------------------   ?>
<br /><br />
<?php
