...
function _topNav() {
	...
	function topMain() {
		...
		if(window.topNavSmall === false && _scrollTop > 0) {
			...
			jQuery('#topNav').stop().animate({ 'height':'145px'},400);
		...
		if(window.topNavSmall === true && _scrollTop < 3) {
			...
			jQuery('#topNav').stop().animate({ 'height':'125px'},400);
...
function affichage_popup(nom_de_la_page, nom_interne_de_la_fenetre) {
	window.open (nom_de_la_page, nom_interne_de_la_fenetre, config='height=800, width=400, toolbar=no, menubar=no, scrollbars=no, resizable=yes, location=no, directories=no, status=no')
}
