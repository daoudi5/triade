...
if (isset($_POST["eid"])) {
	...
	$laclasse=0;
	$erreur=LANGMESS49;
	list($choixEntrepriseId,$num_stagePost)=split("#",$_POST["choix_stage"]);
}
...
if ($texte ==  "{FICHIERRTFSTAGE}") {
	...
	if ($laclasse == 1) {
		...
		for($j=0;$j<count($eleveT);$j++) {  // premiere ligne de la creation PDF
			...
			$responsable2="";
 			...
 			for($h=0;$h<count($dataEnt);$h++) {
				...
				$texte=ereg_replace("classe_eleve","$classe_nom",$texte);
				$texte=ereg_replace("naissance_eleve","$naissance_eleve",$texte);	
				$texte=ereg_replace("ent_nom","$ent_nom",$texte);
				$texte=ereg_replace("ent_rs","$ent_rs",$texte);
				$texte=ereg_replace("ent_adr","$ent_adr",$texte);
				$texte=ereg_replace("ent_cp","$ent_cp",$texte);
				$texte=ereg_replace("ent_directeur","$ent_directeur",$texte);
				$texte=ereg_replace("ent_localite","$ent_localite",$texte);
				$texte=ereg_replace("ent_tel","$ent_tel",$texte);
				$texte=ereg_replace("lieu_naissance","$lieu_naissance_eleve",$texte);
				$texte=ereg_replace("directeur","$directeur",$texte);
				$texte=ereg_replace("nom_stage","$nom_stage",$texte);
				$texte=ereg_replace("ent_dir_fonction","$ent_fonction",$texte);
				$texte=ereg_replace("adresse_eleve","$adresseEleve",$texte);
				$texte=ereg_replace("ccp_eleve","$codePostalEleve",$texte);
				$texte=ereg_replace("ville_eleve","$villeEleve",$texte);
				$texte=ereg_replace("tel_eleve","$telEleve",$texte);
				$texte=ereg_replace("tel_parent","$telParent",$texte);
				$dateJour=dateDMY();
				$texte=ereg_replace("date_du_jour","$dateJour",$texte);
				$texte=ereg_replace("ent_tuteur","$ent_tuteur",$texte);
				$texte=ereg_replace("enseignant_suivi_1","$enseignant_1",$texte);
				$texte=ereg_replace("enseignant_suivi_2","$enseignant_2",$texte);
				$texte=ereg_replace("date_suivi_2","$date_suivi_2",$texte);
				$texte=ereg_replace("date_suivi_1","$date_suivi_1",$texte);
				$texte=ereg_replace("ent_fax","$ent_fax",$texte);	

				$texte=ereg_replace("ent_web","$ent_web",$texte);	
				$texte=ereg_replace("ent_etoile","$ent_etoile",$texte);	
				$texte=ereg_replace("ent_chambre","$ent_chambre",$texte);	
				$texte=ereg_replace("ent_grp_hotelier","$ent_grp_hotelier",$texte);	
				$texte=ereg_replace("ent_logement","$ent_logement",$texte);
				$texte=ereg_replace("ent_service","$ent_service",$texte);
				$texte=ereg_replace("ent_indemnitestage","$ent_indemnitestage",$texte);
				$texte=ereg_replace("ent_pays","$ent_pays",$texte);
				$texte=ereg_replace("anneescolaire","$anneescolaire",$texte);
				$texte=ereg_replace("&#8364;","euro",$texte);
			...
			for($h=0;$h<count($dataEnt);$h++) {
				...
				$texte=ereg_replace("nb_jour","$nbjourentredeuxdate",$texte); 
				if ($num_stage == 1) $texte=ereg_replace("periode_1","$periode",$texte); 
				if ($num_stage == 2) $texte=ereg_replace("periode_2","$periode",$texte); 
				if ($num_stage == 3) $texte=ereg_replace("periode_3","$periode",$texte); 
				if ($num_stage == 4) $texte=ereg_replace("periode_4","$periode",$texte); 
				if ($num_stage == 5) $texte=ereg_replace("periode_5","$periode",$texte); 
				if ($num_stage == 6) $texte=ereg_replace("periode_6","$periode",$texte); 
				if ($num_stage == 7) $texte=ereg_replace("periode_7","$periode",$texte); 
				if ($num_stage == 8) $texte=ereg_replace("periode_8","$periode",$texte); 
				if ($num_stage == 9) $texte=ereg_replace("periode_9","$periode",$texte); 
				if ($num_stage == 0) $texte=ereg_replace("periode_0","$periode",$texte); 
				$texte=ereg_replace("periode_x","$periode",$texte); 

 
				if ($num_stage == 1) $texte=ereg_replace("debperio_1","$date_debut",$texte); 
				if ($num_stage == 2) $texte=ereg_replace("debperio_2","$date_debut",$texte); 
				if ($num_stage == 3) $texte=ereg_replace("debperio_3","$date_debut",$texte); 
				if ($num_stage == 4) $texte=ereg_replace("debperio_4","$date_debut",$texte); 
				if ($num_stage == 5) $texte=ereg_replace("debperio_5","$date_debut",$texte); 
				if ($num_stage == 6) $texte=ereg_replace("debperio_6","$date_debut",$texte); 
				if ($num_stage == 7) $texte=ereg_replace("debperio_7","$date_debut",$texte); 
				if ($num_stage == 8) $texte=ereg_replace("debperio_8","$date_debut",$texte); 
				if ($num_stage == 9) $texte=ereg_replace("debperio_9","$date_debut",$texte); 
				if ($num_stage == 0) $texte=ereg_replace("debperio_0","$date_debut",$texte); 
				$texte=ereg_replace("debperio_x","$date_debut",$texte); 

				if ($num_stage == 1) $texte=ereg_replace("finperio_1","$date_fin",$texte); 
				if ($num_stage == 2) $texte=ereg_replace("finperio_2","$date_fin",$texte); 
				if ($num_stage == 3) $texte=ereg_replace("finperio_3","$date_fin",$texte); 
				if ($num_stage == 4) $texte=ereg_replace("finperio_4","$date_fin",$texte); 
				if ($num_stage == 5) $texte=ereg_replace("finperio_5","$date_fin",$texte); 
				if ($num_stage == 6) $texte=ereg_replace("finperio_6","$date_fin",$texte); 
				if ($num_stage == 7) $texte=ereg_replace("finperio_7","$date_fin",$texte); 
				if ($num_stage == 8) $texte=ereg_replace("finperio_8","$date_fin",$texte); 
				if ($num_stage == 9) $texte=ereg_replace("finperio_9","$date_fin",$texte); 
				if ($num_stage == 0) $texte=ereg_replace("finperio_0","$date_fin",$texte); 
				$texte=ereg_replace("finperio_x","$date_fin",$texte); 
		...
		$bouton="<input type=button onclick=\"open('telecharger.php?fichier=../data/pdf_certif/courrierstageconvention/$classeNom.zip','_blank','');\" value=\"".LANGTMESS530."\"   class='btn btn-primary btn-sm  vat-btn-footer'  >";
	}else{
		$idEleve=$ideleve;
	...
	//35	//  e.autre_responsable
		for($h=0;$h<count($dataEnt);$h++) {
			...
			$texte=ereg_replace("ent_localite","$ent_localite",$texte);
			$texte=ereg_replace("ent_tel","$ent_tel",$texte);
			$texte=ereg_replace("ent_mail","$ent_mail",$texte);
			$texte=ereg_replace("lieu_naissance","$lieu_naissance_eleve",$texte);
			$texte=ereg_replace("directeur","$directeur",$texte);
			$texte=ereg_replace("nom_stage","$nom_stage",$texte);
			$texte=ereg_replace("ent_dir_fonction","$ent_fonction",$texte);
		...
		$bouton="<input type=button onclick=\"open('telecharger.php?fichier=$fic','_blank','');\" value=\""."Récuperation de la convention de stage"."\"  class='btn btn-primary btn-sm  vat-btn-footer' >";
	}
...
<?php if ($okconv == 1) { ?>
	...
	<?php if ($_SESSION["membre"] == "menuprof") { ?>
		<input type=button onclick="open('visu_pdf_prof.php?id=<?php print $fichierpdf?>','_blank','');" value="<?php print LANGPER5?>"  class='btn btn-primary btn-sm  vat-btn-footer' >
	<?php }else{ ?>	
