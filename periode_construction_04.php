...
<ul><font class="T2">
      ...
      <?php print LANGPER4?> : <?php print ucwords($classe_nom) ?><br> <br>
    </font>
</ul>
...
$pdf->SetXY(3,35); // placement du cadre du nom de l eleve
$pdf->MultiCell(200,20,'',1,'L',1);
...
for($i=0;$i<count($ordre);$i++) {
	...
	for($b=0;$b<count($note);$b++) {
		...
		if ($ordre[$i][0] == $note[$b][2]) {
			...
			$font='';
			$fontF='';
			$pdf->SetXY($XsujetNote-40,$YsujetNote);
			$pdf->SetFont('Arial','',7);
	...
	$pdf->SetXY($Xmat,$Ymat);
        ...
        $pdf->SetXY($Xmat,$Ymat);
        $nommatiere=trunchaine($matiere,60);
        $pdf->MultiCell(37,3,"$nommatiere",0,'L',0);
...
//	$pdf->WriteHTML(ucwords(trunchaine($nomprof,20)));
	...
	$pdf->SetXY($Xnote-40,$Ynote);
	$pdf->MultiCell(160,$hauteurMatiere,'',1,'',0);
...
for($ja=0;$ja<count($data_3);$ja++) {
	$nbminute=ereg_replace('mn','',$data_3[$ja][5]);
	if (ereg("[0-9]h",$data_3[$ja][5])) {
		$minute=0;
		list($heure,$minute) =split( "h", $data_3[$ja][5], 2 );
