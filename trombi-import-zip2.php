...
if ((!empty($fichier)) && ($size <= 20000000)) {
	...
	if ((trim($type) == "application/x-zip-compressed") || (trim($type) == "application/octet-stream") || (trim($type) == "application/x-download")  || (trim($type) == "application/x-stuffit")  ||  (trim($type) == "application/zip") || (eregi('zip',$type)) || (trim($type) == "application/force-download") ) {
		$erreur_fichier="non";
