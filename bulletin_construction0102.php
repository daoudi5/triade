...
for($j=$dep;$j<$nbEleveT;$j++) {  // premiere ligne de la creation PDF
	...
	//	$Ycoeff=$Ycoeff + $hauteurMatiere;
		...
		$pdf->SetFont('Arial','',8);
		$pdf->SetXY($Xmoyeleve-17,$Ymoyeleve);
		...
		$pdf->SetFont('Arial','',8);
		$pdf->SetXY($Xmoyclasse-14,$Ymoyclasse);
		...
		if ($effbordure == 0 ){
		//	$pdf->SetXY($Xnote-24,$Ynote);
		//	$H=$hauteurMatiere;
		//	$pdf->MultiCell(119,$H,'',0,'L',0);
		}

		// mise en place du cadre commentaire
		$pdf->SetXY($Xnote-24,$Ynote);
		$pdf->MultiCell(119,$hauteurMatiere,'',1,'',0);
		...
		if (($sousmatiere != "0") && ($sousmatiere != "")){
			$pdf->SetFont('Arial','',8);
			$pdf->SetXY($XnotVal-19,$YnotVal-3);
			$noteaff1=$noteaff;
	...
	$pdf->SetXY($XmoyMatGenMaxVal-22,$YmoyMatGVal-$top);
	...
	$pdf->SetXY($XmoyMatGVal-15,$YmoyMatGVal-$top);
	...
	$pdf->SetXY($XmoyMatGenMinVal-19,$YmoyMatGVal-$top);
	...
	$commentaireeleve=ereg_replace("^,","",$commentaireeleve);
	...
	$pdf->SetXY($Xcom-22,$Ycom+2.2+$juste);
...
/*
	...
	$pdf->SetXY($Xmat,$Ymat);
	$pdf->MultiCell($largeurMat-1,$hauteurMatiere,'',1,'L',0);
	$pdf->SetXY($Xmatcont,$Ymatcont);
...
$moyenEleveaff=arrondiaudixieme($moyenEleveaff);
...
$pdf->SetXY($XmoyClasseMaxValue-3,$YmoyClasseMaxValue);
...
$pdf->RoundedRect($XmoyClasseMaxValue+5, $YmoyClasseMaxValue-=2, 119, 20, 3.5, 'DF');

$pdf->SetXY($XmoyClasseMaxValue+3+5,$YmoyClasseMaxValue+1);
$pdf->SetFont('Arial','B',9);
$pdf->SetFont('Arial','U',9);
$pdf->MultiCell(40,3,'Vie Scolaire : ',0,'L',0);
$pdf->SetFont('Arial','',8);
$pdf->SetXY($XmoyClasseMaxValue+26+5,$YmoyClasseMaxValue+1);
$pdf->MultiCell(40,3,"$persVieScolaire",0,'L',0);

$pdf->SetXY($XmoyClasseMaxValue+5,$YmoyClasseMaxValue+5);
$commentaireeleve=cherche_com_scolaire_eleve_cpe($idEleve,"-10",$idClasse,$_POST["saisie_trimestre"],"");
