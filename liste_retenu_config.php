...
<tr id='coulBar0' ><td height="2"><b><font   id='menumodule1' ><?php print LANGCOUR2 ?></font></b></td></tr>
<tr id='cadreCentral0' >
<td ><br>
...
if (isset($_POST["create"])) {
	...
	if ($_FILES['rtf']['name'] != "") {
		...
		if ( (!empty($fichier)) &&  ($size <= 2000000)) {
			if  (($type == "application/octet-stream") || ($type == "application/msword"))  {
				@unlink("./data/parametrage/courrier_retenu.rtf");
				move_uploaded_file($tmp_name,"./data/parametrage/courrier_retenu.rtf");
				print "<BR><center><font class='T2'>"."Fichier enregistré"."</font></center>";
				config_param_ajout("{FICHIERRTFRETENU}",'param_retenu');
				$texte="{FICHIERRTFRETENU}";
		...
		$texte=ereg_replace("\\\'","'",$texte);
		//--------------------
		config_param_ajout($_POST["suite"],'param_retenu');
	}
...
<?php
	...
	print LANGPARAM43;
	if (trim($data[0][0]) != "{FICHIERRTFRETENU}") {
		$textes=$data[0][0];
	}else{
