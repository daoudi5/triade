...
if (count($valeur)) {
	...
	//	print "$matiere##$num_ordre##$idMatiere<br>";
	//	}
		$nbMat++;
	}
...
while(count($ordre)) {
	...
	//	if (verifNoteDansMatiere($idMatiere,$idClasse,$idgroupe,$dateDebut,$dateFin)) { $nbaffichematiere++ ;  continue; }
		if (trim($matiere) != "") { $tabmatiere[$num_ordre]="$matiere##$num_ordre##$idMatiere##$nomSousMatiere"; }
		if (trim($matiere) != "") { $tabmatiere2[$num_ordre]="$matiere##$num_ordre##$idMatiere"; }
		$nomSousMatiere="";
	}
	...
	while (list($key, $nom_matiere) = each($tabmatiere)) {
		list($nom_matiere,$num_ordre,$idMatiere,$nomSousMatiere)=split("##",$nom_matiere);
		...
		if (trim($nomSousMatiere) != "") {	
			if ($affmatiere != "non") {
				$nom_matiere2=trunchaine($nom_matiere,5);
			}
			if ($affsousmatiere != "non") {
				$nbc=13;
				if ($affmatiere != "non") { $nbc=5;} 
				$nom_matiere2.=" ".trunchaine($nomSousMatiere,$nbc);
			}
		}else{
			if ($affmatiere != "non") {
				$nom_matiere2=eregi_replace('education','educ',$nom_matiere);
				$nom_matiere2=trunchaine($nom_matiere,13);
			}
		}
		$nomSousMatiere="";
		$nom_matiere2=trim($nom_matiere2);
		$codeMatiere=chercheCodeMatiere($idMatiere);
		if ($codeMatiere != "") $nom_matiere2=$codeMatiere;
		$pdf->MultiCell($largeurnote,2.7,"$nom_matiere2",0,'L',0);
		$nom_matiere2="";
	...
	if (($afficheMoyen) && ($nbaffichematiere >= count($ordre))) {
		...
		if ((EXAMENJTC != "oui") && (MODNAMUR0 == "oui")) {
			$pdf->SetFont('Arial','B',6);
			$pdf->SetXY($xmatiere,$ymatiere-3); // placement du cadre notes
		  	$pdf->MultiCell($largeurnote,8,'Vie Scol.',1,'L',1);	
			$pdf->SetFont('Arial','',6);

			$pdf->SetFillColor(220);
			$pdf->SetFont('Arial','B',6);
			$pdf->SetXY($xmatiere+$largeurnote,$ymatiere-3); // placement du cadre notes
		  	$pdf->MultiCell($largeurnote,8,'Moyen.',1,'L',1);	
			$pdf->SetFont('Arial','',6);
	...
	for($j=0;$j<count($eleveT);$j++) {  // premiere ligne de la creation PDF
		...
		if ($ju > 32) { $pdf->AddPage();$ju=0; $ynomeleve=20;$YnotVal=20;$ymatiere=$YnotVal-5;}
		...
		foreach ($tabmatiere as $key => $value)  {
			$nbmat++;
			list($nom_matiere,$num_ordre,$idMatiere)=split("##",$value);
			$matiere=$nom_matiere;
			//$idMatiere=$key;
			unset($verifGroupe);
	...
	foreach ($tabmatiere as $key => $value)  {
		list($nom_matiere,$num_ordre,$idMatiere,$sousMatiere)=split("##",$value);
		$matiere=$nom_matiere;
		//$idMatiere=$key;
	...
	if (($afficheMoyen) && ($nbaffichematiere >= count($ordre))) {
		if ((EXAMENJTC != "oui") && (MODNAMUR0 == "oui")) {
			$moyenEleve="";
			if ($notescolaireG != "") {
				$moyenEleve=moyGenEleve($notescolaireG,$coefscolaireG);
			}
			$moyenEleve=ereg_replace(',','.',$moyenEleve);
			if ($moyenEleve < 10) { 
				$pdf->SetTextColor(255,0,0);
...
?>
	<input type=button onclick="open('visu_pdf_admin.php?id=<?php print $fichier?>','_blank','');" value="R&eacute;cup&eacute;rer le fichier PDF"   class="bouton2" >
<?php
}
...
?>
	<input type=button onclick="open('visu_pdf_scolaire.php?id=<?php print $fichier?>','_blank','');" value="R&eacute;cup&eacute;rer le fichier PDF"  class="bouton2" >
<?php
}
...
?>
	<input type=button onclick="open('visu_pdf_prof.php?id=<?php print $fichier?>','_blank','');" value="R&eacute;cup&eacute;rer le fichier PDF"  class="bouton2" >
<?php } ?>
</ul></ul>
</form>
<br /><br />
<i>Information : les moyennes g&eacute;n&eacute;rales sont r&eacute;alis&eacute;es via les moyennes des sous-mati&egrave;res et non la moyenne des moyennes des mati&egrave;res.</i>
<?php
// gestion d'historie
