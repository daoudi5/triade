...
for($j=0;$j<count($eleveT);$j++) {  // premiere ligne de la creation PDF
	...
	$pdf->SetXY($X+104,$Y+1); //125
	$pdf->MultiCell(33,3,"Points d'appui. \n Progrès. Efforts.",0,'C',0);
	$pdf->SetXY($X+137,$Y+1); //125
	$pdf->MultiCell(33,3,"Ecarts par rapport \n aux objectifs attendu",0,'C',0);
	$pdf->SetXY($X+170,$Y+1); //125
	$pdf->MultiCell(33,3,"Conseils pour \n progresser.",0,'C',0);
	$pdf->SetFont('Arial','',9);
	// fin des titres
	...
	//	print $effbordure2." $libelleMatiere <br>";
		...
		if ($effbordure == 0 ){
			...
			$H=$hauteurMatiere*$nbs;
			$pdf->MultiCell(33,$H,'',1,'L',0);
			$pdf->SetXY($Xnote+33,$Ynote);
			$pdf->MultiCell(33,$H,'',1,'',0);
			$pdf->SetXY($Xnote+66,$Ynote);
			$pdf->MultiCell(34,$H,'',1,'',0);
		}

		// mise en place du cadre commentaire
		$pdf->SetXY($Xnote,$Ynote);
		$pdf->MultiCell(33,$hauteurMatiere,'',$effbordure2,'',0);
		$pdf->SetXY($Xnote+33,$Ynote);
		$pdf->MultiCell(33,$hauteurMatiere,'',$effbordure2,'',0);
		$pdf->SetXY($Xnote+66,$Ynote);
		$pdf->MultiCell(34,$hauteurMatiere,'',$effbordure2,'',0);
	...
	$commentaireeleve=cherche_com_eleve2($idEleve,$idMatiere,$idClasse,$_POST["saisie_trimestre"],$idprof,$idgroupe,"1");
	$commentaireeleve=ereg_replace("\n"," ",$commentaireeleve);
	$confPolice=confPoliceSeminaire($commentaireeleve);  // $confPolice[0] -> Policy  ; $confPolice[1] -> Cadre

	$Xcom=$XmoyMatGenMaxVal + 10;
	$pdf->SetFont('Arial','',$confPolice[0]);
	$pdf->SetXY($Xcom+1,$Ycom);
	$pdf->MultiCell(33,$confPolice[1],$commentaireeleve,0,'',0);

	/*--------------------------*/
	$commentaireeleve=cherche_com_eleve2($idEleve,$idMatiere,$idClasse,$_POST["saisie_trimestre"],$idprof,$idgroupe,"2");
	$commentaireeleve=ereg_replace("\n"," ",$commentaireeleve);
	$confPolice=confPoliceSeminaire($commentaireeleve);  // $confPolice[0] -> Policy  ; $confPolice[1] -> Cadre

	$pdf->SetFont('Arial','',$confPolice[0]);
	$pdf->SetXY($Xcom+34,$Ycom);
	$pdf->MultiCell(33,$confPolice[1],$commentaireeleve,0,'',0);
	
	/*--------------------------*/
	$commentaireeleve=cherche_com_eleve2($idEleve,$idMatiere,$idClasse,$_POST["saisie_trimestre"],$idprof,$idgroupe,"3");
	$commentaireeleve=ereg_replace("\n"," ",$commentaireeleve);
	$confPolice=confPoliceSeminaire($commentaireeleve);  // $confPolice[0] -> Policy  ; $confPolice[1] -> Cadre

	$pdf->SetFont('Arial','',$confPolice[0]);
	$pdf->SetXY($Xcom+67,$Ycom);
	$pdf->MultiCell(34,$confPolice[1],$commentaireeleve,0,'',0);
...
$pdf->SetXY(3,$Ycom+=10);
$pdf->WriteHTML("Niveau A2 Langue :"); // Niveau A2
$pdf->SetXY(43,$Ycom+1);
$pdf->MultiCell(4,4,'',1,'L',0); // case oui
$pdf->SetXY(49,$Ycom);
$pdf->WriteHTML("oui");
