...
config_param_ajout($adressebulletin,"adressebulletin");
...
for($j=$dep;$j<$nbEleveT;$j++) {  // premiere ligne de la creation PDF
	...
	$pdf->SetXY(15,$Y); // placement du cadre du nom de l eleve
	if ($afficherang == "oui") {
...
//	$pdf->WriteHTML($infoeleveclasse);
	...
	for($ik=0;$ik<=count($dataadresse);$ik++) {
		...
		if (($adressebulletin == "etudiant") && ($adr_eleve != "")) { 
			$pdf->SetFont('Arial','',10);
			$pdf->SetXY($Xv11,36);
			$chaine=LANGBULL44." ".trim(strtoupper($nomEleve))." ".trim(ucwords(strtolower($prenomEleve)));
			$pdf->WriteHTML(trunchaine($chaine,40));
			$pdf->SetXY($Xv11,42);
			$chaine=trim($adr_eleve);
			$pdf->WriteHTML(trunchaine($chaine,40));;
			$pdf->SetXY($Xv11,48);
			$chaine=trim($ccp_eleve)." ".trim($commune_eleve);
			$pdf->WriteHTML(trunchaine($chaine,40));
		}
	...
	$pdf->SetFont('Arial','',9);
	$pdf->SetFillColor(220);
	$pdf->SetXY(15,$Y); //  placement  cadre titre
	if ($afficherang == "oui") {
		$pdf->MultiCell(194,11,'',1,'C',1);
	}else{
		$pdf->MultiCell(184,11,'',1,'C',1);
	}
	$pdf->SetXY(25,$Y+2); // placement contenu titre
	...
	for($i=0;$i<count($ordre);$i++) {
		...
		if ($afficherang == "oui") {
			$pdf->SetFont('Arial','',8);
			$pdf->SetXY($Xmoyclasse+32,$Ymoyclasse);
			$pdf->MultiCell(10,$hauteurMatiere,'',1,'L',0);	
		}
...
// --------------------------------------------------------------------------------------------------------------------------------	
	...
	// rangs
 	if ($afficherang == "oui") {
		...
		$pdf->SetFont('Arial','',8);
		$pdf->SetXY($XmoyMatGVal+32,$YcoeffVal-7);
		if ($rang != "") {
