...
if ((empty($_SESSION["nom"])) && (empty($_SESSION["membre"]))) {
    ...
    print "alert(\"Vous devez vous connecter avec votre compte TRIADE.\\n \\n L'Equipe TRIADE.\");\n";
    print "location.href='../../../index1.php'";
    print "</script></html>";
    exit;
}
...
$cnx=cnx();
if ($membre == "menueleve") { $mail=cherchemail($idpers); }else{ $mail=cherchemailpersonnel($idpers); }
$mdp="PASSTRIADE";
$login="$nom $prenom";
...
if ($display_all_form === true) {
	...
	if (api_is_western_name_order()) {
		...
		$form->addElement('text', 'firstname', get_lang('FirstName'), array('size' => 40,'value'=>"$prenom",'readonly'=>'readonly'));
		$form->addElement('text', 'lastname',  get_lang('LastName'),  array('size' => 40,'value'=>"$nom",'readonly'=>'readonly'));
	} else {
	...
	//	EMAIL
	$form->addElement('text', 'email', get_lang('Email'), array('size' => 40,'value'=>"$mail"));
	...
	*/
	//	USERNAME
	$form->addElement('text', 'username', get_lang('UserName'), array('size' => USERNAME_MAX_LENGTH,'value'=>"$login",'readonly'=>'readonly','style'=>'display:block'));
	...
	//	PASSWORD
	$form->addElement('password', 'pass1', get_lang('Pass'),         array('size' => 20, 'id'=> 'pass1','value'=>"$mdp",'readonly'=>'readonly','style'=>'display:block'));
	$form->addElement('password', 'pass2', get_lang('Confirmation'), array('size' => 20,'value'=>"$mdp",'readonly'=>'readonly','style'=>'display:block'));
	...
	//	STUDENT/TEACHER
		if (($membre == "menuparent") || ($membre == "menueleve")){
			$form->addElement('radio', 'status', get_lang('Status'), get_lang('RegStudent'), true);
		}else{
			$form->addElement('radio', 'status', get_lang('Status'), get_lang('RegAdmin'), true);
		}
...
/*	if (api_get_setting('allow_registration_as_teacher') == 'false') {
	...
	if ($membre == "menueleve")  $values['status']=STUDENT;
	if ($membre == "menuparent")  $values['status']=STUDENT;
	...
	if ($user_id) {
        ...
        if ($user_enrolled) { // Student was enrolled to trainings
          echo '<script>window.location.href = "/user_portal.php";</script>';
          exit;
        }
	}
