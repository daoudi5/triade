...
if (isset($_GET["logout"])) {
	session_start();
	$_SESSION['name_dokeos']="";
	$_SESSION['passwd_dokeos']="";
	unset($_SESSION['name_dokeos']);
	unset($_SESSION['passwd_dokeos']);
...
function display_login_form() {
	...
	if (isset($_GET["admin"]))  {
		...
		$form->addElement('text', 'login', get_lang('UserName'),array('size'=>15,'value'=>$login,'readonly'=>'readonly'));
		$form->addElement('password', 'password', get_lang('Pass'),array('size'=>15,'value'=>$mdp));
		...
		$renderer->setElementTemplate('<div ><label>{label}</label></div><div>{element}</div>');
              ...
              /*  $form = new FormValidator('formLogin');
                ...
                $idpers=$_COOKIE["id_pers"];
                $mail="";
                $mdp="PASSTRIADE";
                $login="$nom $prenom";$login=ereg_replace(" ","_",$login);
			...
			$form = new FormValidator('formLogin');
			$form->addElement('text', 'login', get_lang('UserName'),array('size'=>15,'value'=>$login,'readonly'=>'readonly'));
			$form->addElement('password', 'password', get_lang('Pass'),array('size'=>15,'value'=>$mdp,'readonly'=>'readonly'));
			$form->addElement('style_submit_button','submitAuth', get_lang('langEnter'), array('class' => 'login'));
			$renderer =& $form->defaultRenderer();
			$renderer->setElementTemplate('<div ><label>{label}</label></div><div>{element}</div>');
...
//                        echo "<script>document.formLogin.submit();</script>";
 ...
 /*                     $form = new FormValidator('formLogin');
                        ...
                        echo "&nbsp;&nbsp;<font color=red>Merci de valider votre compte.</font>";
                }
    ...
    if (api_get_setting('allow_lostpassword') == 'true' || api_get_setting('allow_registration') == 'true') {
        ...
        if (api_get_setting('allow_registration') <> 'false') {
           if (count($data) == 0) {
           	$html_ul.= '<br><li><a href="main/auth/inscription.php"><b><font color=red>Enregistrer votre compte --> [ici]</font></b></a></li>';
	   }else{
		$html_ul.= '<script>document.formLogin.submit();</script>';	
	   }
        }
