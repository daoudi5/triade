...
$barre="";
$duplicata="ATTENTION: Ce bulletin est l'original, il doit être conservé par la famille  - suivi d'un élève sur http://www.mfrvieux-habitants.c.la";
// FIN variables
	...
	$logo="./image/banniere/banniere-mfreo.jpg";
	if (file_exists($logo)) {
		$xlogo=11;
		$ylogo=16;
		$xcoor0=30;
		$ycoor0=3;
		$pdf->Image($logo,10,10,$xlogo,$ylogo);
	}
	// fin du logo
	...
	$pdf->SetXY(21,10);
	$pdf->WriteHTML("<B>$nom_etablissement</B>");
	$pdf->SetFont('Arial','',8);
	$pdf->SetXY(21,14);
	$pdf->WriteHTML("$adresse");
	$pdf->SetXY(21,17);
	$pdf->WriteHTML("$postal $ville");
	$pdf->SetXY(21,20);
	$pdf->WriteHTML("$urlsite");
	$pdf->SetXY(21,23);
	$pdf->WriteHTML("E-mail : $mail");
	//fin coordonnees
	...
	// mise en place du logo
	$logo="./image/banniere/banniere-mfreo2.jpg";
	if (file_exists($logo)) {
		$xlogo=25;
		$ylogo=10;
		$xcoor0=30;
		$ycoor0=3;
		$pdf->Image($logo,90,10,$xlogo,$ylogo);
	}
	...
	$pdf->MultiCell(45,4,"Classe",1,'C',1);
	...
	$pdf->MultiCell(15,4,"Moy",1,'C',1);
	$pdf->SetXY($largeurMat+$x+15+15,$y+4);
	$pdf->MultiCell(15,4,"Mini",1,'C',1);
	$pdf->SetXY($largeurMat+$x+15+15+15,$y+4);
	$pdf->MultiCell(15,4,"Maxi",1,'C',1);
	// fin des sous-titres
...
$liste_ens_generaux=ereg_replace(',$','',$liste_ens_generaux);
$liste_spec_prof=ereg_replace(',$','',$liste_spec_prof);
$liste_sect_prof=ereg_replace(',$','',$liste_sect_prof);
...
$pdf->SetFont('Arial','',6);
$pdf->MultiCell(190,3,"* moy.des matières suivantes: ".$liste_ens_generaux." / ** moy.des matières suivantes: ".$liste_sect_prof." / *** moy.des matières suivantes: ".$liste_spec_prof,0,'L',0);
$pdf->SetFont('Arial','BI',8);
$pdf->SetXY(10,$Ycom+$EpaisCom+9);
