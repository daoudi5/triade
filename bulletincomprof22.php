...
print "<center><font class=T2><b>$titrecom</b></font></center>";
...
if ($choix_tri_text == "trimestre1") {
	$choix_tri_text=LANGPROJ3. " ".LANGOU." ".LANGPROJ19;
}
if ($choix_tri_text == "trimestre2") {
    $choix_tri_text=LANGPROJ4. " ".LANGOU." ".LANGPROJ20;
}
...
<select name="choix_trimestre">
	...
	<option value='trimestre1' STYLE='color:#000066;background-color:#CCCCFF'><?php print LANGPROJ3. " ".LANGOU." ".LANGPROJ19?></option>
	<option value='trimestre2' STYLE='color:#000066;background-color:#CCCCFF'><?php print LANGPROJ4. " ".LANGOU." ".LANGPROJ20?></option>
...
$texte=LANGMESS135." : $nomclasse ";
$pdf->SetFont('Arial','',12);
$pdf->SetXY($xcoor,$ycoor);
$pdf->Write(5,$texte);
?>

&nbsp;&nbsp;<?php print LANGMESS109 ?> <b><?php print dateForm($dateDebut) ?></b> <?php print LANGMESS110 ?> <b><?php print dateForm($dateFin) ?></b>
/ <?php print LANGMESS123 ?> : <font color=blue><b><?php print $moyClass ?></b></font>
<br /><br />
<?php 
if ( (defined("NOTEELEVEVISU")) && (NOTEELEVEVISU == "oui")) { 
?>
<table><tr><td><?php print LANGMESS137 ?> :</td><td><script language=JavaScript>buttonMagic("<?php print LANGMESS136 ?>","profpprojo.php?fiche=1&idClasse=<?php print $cid?>","video","width=800,height=700,resizable=yes,personalbar=no,toolbar=no,statusbar=no,locationbar=no,menubar=no,scrollbars=yes","");</script>
</td></tr></table>
<?php } ?>
...
$pdf->SetXY($xcoor,$ycoor);
$pdf->Write(5,LANGMESS109." ".dateForm($dateDebut)." ".LANGMESS110." ".dateForm($dateFin)."  -  ".ucfirst($choix_tri_text));
...
<form method=post name=formulaire action="bulletincomprof44.php"  onsubmit="return envoi();">
...
SQL;
	...
	";
	   ...
	   for($i=0;$i<count($mat);$i++){
				...
				print "<tr>\n";
				print "<td id='bordure' colspan='2' valign='top' >&nbsp;<b>".$mat[$i][1]."</b>";
				print "</tr>";
				print "<tr>\n";
				print "<td id='bordure' valign=top title=\"".$mat[$i][1]."\" ><img src='$photoeleve' >";?>
			...
			?>
			<td id='bordure'>
			<button onMouseOver="AffBulle('<u><strong> <?php print LANGMESS125 ?> :</strong> </u><br> 1º: <?php print LANGMESS131 ?>. <br> 2º: <?php print LANGMESS127 ?> +.')" onMouseOut="HideBulle()" onClick="GP_AdvOpenWindow('bulletin_com_bdd_ajout.php?com=<?php print $commentaireeleve ?>','<?php print LANGMESS128 ?>','fullscreen=no,toolbar=no,location=no,status=no,menubar=no,scrollbars=no,resizable=no,channelmode=no,directories=no',200,100,'center','ignoreLink','alwaysOnTop',3,'',0,1,5,'');return document.MM_returnValue">+</button>&nbsp;
...
<?php 
		...
		for($o=0;$o<count($tabliste);$o++) {
			...
			print "<td>";
			print "<i>En $nommat</i><br><br><textarea cols='68' rows='5' disabled='disabled' >$com</textarea>";
			

			if ($groupe == 1) {
				$moyEleve1=moyenneEleveMatiereGroupe($idEleve,$idmatiere1,dateForm($dateDebut),dateForm($dateFin),$idgroupe,$_SESSION["id_pers"]);
				$moyElevePrecedent1=moyenneEleveMatiereGroupe($idEleve,$idmatiere1,dateForm($dateDebutP),dateForm($dateFinP),$idgroupe,$_SESSION["id_pers"]);
				$notetype=recherchetypenotegroupe($idmatiere1,dateForm($dateDebut),dateForm($dateFin),$idgroupe);
			}else {
				$moyEleve1=moyenneEleveMatiere($idEleve,$idmatiere1,dateForm($dateDebut),dateForm($dateFin),$_SESSION["id_pers"]);
				$moyElevePrecedent1=moyenneEleveMatiere($idEleve,$idmatiere1,dateForm($dateDebutP),dateForm($dateFinP),$_SESSION["id_pers"]);
				$notetype=recherchetypenote($idmatiere1,dateForm($dateDebut),dateForm($dateFin),$idclasse);
			}
				
			if ($notetype == "en") { 
				$moyEleve1=recherche_note_en($moyEleve1); 
				$color="blue";
			}else{
				...
				if ($moyElevePrecedent1 < 10) {
					$colorp="red";	
				}else{
					$colorp="orange";
				}
			}
			print "<br><br> &nbsp<u>".LANGMESS82."</u> : <font color='$color'><b>$moyEleve1</b></font>";
			if ($choix_tri != "trimestre1")	print "&nbsp;&nbsp;/&nbsp;&nbsp<u>".LANGMESS130."</u> : <font color='$colorp'>$moyElevePrecedent1</font>";
			print "</td>";
...
<?php include("./librairie_php/lib_licence.php"); ?>
<br><br><b><center><font class='T2'><?php print LANGPROFP36 ?>.</font></center></b>
<?php
}
Pgclose() ?>
</BODY>
