...
//	$duplicata=LANGBULL41 . " - $urlsite - $mail";
	...
	$pdf->SetXY(15,25); // placement du cadre du nom de l eleve
	$pdf->MultiCell(191,20,'',1,'L',1);
	...
	$pdf->SetFont('Arial','',9);
	$pdf->SetFillColor(220);
	$pdf->SetXY(15,46); //  placement  cadre titre
	$pdf->MultiCell(191,8,'',1,'C',1);
	$pdf->SetXY(25,47); // placement contenu titre
...
//				$coefEleGMat=$coefEleGMat + $coefsous;
			...
			if (trim($matiereEnCours) != trim($matiereSuivante)) {
				...
				$pdf->WriteHTML($noteaff1);
				$coefmatiere=$coefsoustotal1;
				if (( $noteaff1 != "" ) && ($moyensousmatiere == "non")) {
					$coefEleGMat=$coefEleGMat + $coefmatiere;
					$noteMoyEleGTempo = $noteaff1 * $coefEleGMat;
					$noteMoyEleG=$noteMoyEleG + $noteMoyEleGTempo;
					$coefEleG=$coefEleG + $coefmatiere;
				//print "$noteaff1 * $coefmatiere<br>";
				}
				unset($coefEleGMat);
				unset($noteMoyEleGTempo);
				unset($notesoustotal1);
				unset($coefsoustotal1);
				$pdf->SetXY(15,$Ymat-($hauteurMatiere*2));
				$pdf->MultiCell($largeurMat,$hauteurMatiere*$posiNoteSous,'','1','L',0);
		
		if ($moyensousmatiere == "non") {

		foreach($tabnomdugroupe1 as $value) {
	//	 print "Note : $noteaff ($notesousmoyen) - Idmatiere : $idMatiere - Coef : $coeffaff ($coefmatiere) - Tabscient : $value <br>";
			if (trim($idMatiere) == trim($value)) {
				if (trim($notesousmoyen) != "") {
					$moyennomdugroupe1+= $notesousmoyen * $coefmatiere ;
					//$nbnotenomdugroupe1++ ;
					$nbnotenomdugroupe1+= $coefmatiere ;
		...
		// print "Note : $noteaff - Idmatiere : $idMatiere - Coef : $coeffaff - Tabscient : $value <br>";
			if (trim($idMatiere) == trim($value)) {
				if (trim($noteaff1) != "") {
					$moyennomdugroupe2+= $noteaff1  * $coefmatiere ;
					//$nbnotenomdugroupe2++ ;
					$nbnotenomdugroupe2+= $coefmatiere ;
				}
			}
		}
		foreach($tabnomdugroupe3 as $value) {
		// print "Note : $noteaff - Idmatiere : $idMatiere - Coef : $coeffaff - Tabscient : $value <br>";
			if (trim($idMatiere) == trim($value)) {
				if (trim($noteaff1) != "") {
					$moyennomdugroupe3+= $noteaff1  * $coefmatiere ;
					//$nbnotenomdugroupe3++ ;
					$nbnotenomdugroupe3+= $coefmatiere ;
				}
			}
		}
		// ----------------------------------------------------------------------------------------------
...
// fin notes
// --------
$pdf->SetXY($Xmat,$YmatDepart);
$pdf->MultiCell($largeurMat,$HauteurG,'','1','L',0);
$HauteurG='0';
...
if ($affichemoyengeneral == "oui") {
	...
	$pdf->SetXY($largeurMat+15,$YmoyenneGeneral);
	$pdf->MultiCell(15,7,"$moyenEleveaff",1,'C',1);
	$pdf->SetXY($largeurMat+15+15,$YmoyenneGeneral);
	$pdf->SetFont('Arial','',6);
	$pdf->MultiCell(8,7,"$moyenClasseGenaff",1,'C',1);
	$pdf->SetXY($largeurMat+8+15+15,$YmoyenneGeneral);
	$pdf->MultiCell(8,7,"$moyenClasseMinaff",1,'C',1);
	$pdf->SetXY($largeurMat+8+8+15+15,$YmoyenneGeneral);
	$pdf->MultiCell(8,7,"$moyenClasseMaxaff",1,'C',1);
...
if ($moyennomdugroupe1 != ""){
	...
	$notenomdugroupe1=ereg_replace('\.',',',$notenomdugroupe1);
}
if ($moyennomdugroupe2 != ""){
	...
	$notenomdugroupe2=ereg_replace('\.',',',$notenomdugroupe2);
}
if ($moyennomdugroupe3 != ""){
	...
	$notenomdugroupe3=ereg_replace('\.',',',$notenomdugroupe3);
}
...
if ($nomdugroupe1 != "") {
	$pdf->SetFont('Arial','',7);
	$pdf->SetFillColor(214,227,188);
	$pdf->SetXY(3+15+$LargeurMG/2,$YmoyenneGeneral);
	$pdf->MultiCell(35,7,'',1,'L',1);  
	$pdf->SetXY(3+15+$LargeurMG/2,$YmoyenneGeneral+2);
	$pdf->MultiCell(35,3,"$nomdugroupe1",0,'L',0);  
	$pdf->SetXY(35+3+15+$LargeurMG/2,$YmoyenneGeneral);
...
if ($nomdugroupe2 != "") {
	$pdf->SetFont('Arial','',7);
	$pdf->SetFillColor(198,217,241);
	$pdf->SetXY(15+5+35+3+15+$LargeurMG/2,$YmoyenneGeneral);
	$pdf->MultiCell(35,7,'',1,'L',1);  
	$pdf->SetXY(15+5+35+3+15+$LargeurMG/2,$YmoyenneGeneral+2);
	$pdf->MultiCell(35,3,"$nomdugroupe2",0,'L',0);  
	$pdf->SetXY(35+15+5+35+3+15+$LargeurMG/2,$YmoyenneGeneral);
	$pdf->SetFont('Arial','B',7);
...
if ($nomdugroupe3 != "") {
	$pdf->SetFont('Arial','',7);
	$pdf->SetFillColor(253,233,217);
	$pdf->SetXY(15+5+35+15+5+35+3+15+$LargeurMG/2,$YmoyenneGeneral);
	$pdf->MultiCell(35,7,'',1,'L',1);  
	$pdf->SetXY(15+5+35+15+5+35+3+15+$LargeurMG/2,$YmoyenneGeneral+2);
	$pdf->MultiCell(35,3,"$nomdugroupe3",0,'L',0);  
	$pdf->SetXY(35+15+5+35+15+5+35+3+15+$LargeurMG/2,$YmoyenneGeneral);
	$pdf->SetFont('Arial','B',7);
...
$listenomdugroupe1=ereg_replace(',$','',$listenomdugroupe1);
$listenomdugroupe2=ereg_replace(',$','',$listenomdugroupe2);
$listenomdugroupe3=ereg_replace(',$','',$listenomdugroupe3);
