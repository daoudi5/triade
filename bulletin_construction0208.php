...
config_param_ajout($_POST["hauteurmatiere"],"hauteurMatBull208");
$debut=deb_prog();
...
$uneseuledecimale=$_POST["uneseuledecimale"];
config_param_ajout($afficheRemplacant,"uneseuledecimale");
...
for($j=$dep;$j<$nbEleveT;$j++) {  // premiere ligne de la creation PDF
	...
	$pdf->RoundedRect($Xv11+5, 37, 80, 24, 3.5, 'DF');
...
//	$pdf->WriteHTML($soustitre5);
	...
	$YprofVal=$Ymat + 5; // y en nom du prof
	...
	for($i=0;$i<count($ordre);$i++) {
		...
		if(verifMatiereSuivanteCommeSousmatiere($ordre[$ii][0])) {
		   ...
		   while(true) {
			   ...
			   if (verifMatiereSuivanteCommeSousmatiere($ordre[$ii][0])) {
				...
				if ( trim($libelleMatiere) == trim($matiereSuivante)) {	
					...
					if ($commentaire != "") { $commentaireeleve=$commentaire; }
		   ...
		   for($g=0;$g<count($eleveT);$g++) {
	  		   $idEleveMoyen=$eleveT[$g][4];
			   foreach($tmatiere as $ordre1=>$idmatiere1) {
				      	$idprof=recherche_prof($idmatiere1,$idClasse,$ordre1);
					$idgroupe=verifMatierAvecGroupeRecupId($idmatiere1,$idEleveMoyen,$idClasse,$ordre1);
				   	if ($idgroupe == "0") {
						$valeur=moyenneEleveMatiere($idEleveMoyen,$idmatiere1,$dateDebut,$dateFin,$idprof);
					}else{
						$valeur=moyenneEleveMatiereGroupe($idEleveMoyen,$idmatiere1,$dateDebut,$dateFin,$idgroupe,$idprof);
					}
					$valeurT+=$valeur;
					if ($valeur != "") { 
						$nbVal++; 
	...
	//	if ($verifGroupe) {  continue; } // verif pour l'eleve de l'affichage de la matiere
		...
		if ($iii == 19) {
			...
			if ($uneseuledecimale == "oui") $notesousmoyen=number_format($notesousmoyen,1,'.','');
			...
			if ($uneseuledecimale == "oui") $notesousmoyenG=number_format($notesousmoyenG,1,'.','');
		...
		$pdf->SetXY($largeurMatiere+$Xmat,$Ymoyeleve);
		$pdf->MultiCell(15,$hauteurMatiere,'',1,'L',0);
		$Ymoyeleve=$Ymoyeleve + $hauteurMatiere;


		// mise en place moyenne classe
		$pdf->SetXY($largeurMatiere+$Xmat+15,$Ymoyclasse);
		$pdf->MultiCell(33,$hauteurMatiere,'',1,'L',0);
		$Ymoyclasse=$Ymoyclasse + $hauteurMatiere;

            	$pdf->SetFont('Arial','',$policeT+1);
		...
		if ($uneseuledecimale == "oui") $moyeMatGenMinaff=number_format($moyeMatGenMinaff,1,'.','');
		...
		if ($uneseuledecimale == "oui") $moyeMatGenMaxaff=number_format($moyeMatGenMaxaff,1,'.','');
		...
		$pdf->SetXY($largeurMatiere+$Xmat+15+33,$Ynote);
		$pdf->MultiCell(86,$hauteurMatiere,'',1,'',0);
		...
		if (trim($notetype) == "en") { 
			if ($noteaff1 != "") {
				...
				if ($uneseuledecimale == "oui") $noteaff1=number_format($noteaff1,1,'.','');
			}else{
				$noteaff1="";
			}
		}
		...
		if (trim($notetype) == "en") { 
			...
			if ($uneseuledecimale == "oui") $moyeMatGenaff=number_format($moyeMatGenaff,1,'.','');
		}else{
		...
		$pdf->SetXY($Xcom-1,$Ycom+0.5);
		$pdf->MultiCell(85,$confPolice[1],$commentaireeleve,'0','L',0);
		$commentaireeleve="";
		$pdf->SetFont($police,'',$policeT+2);
		$Ycom=$YmoyMatGVal - 3;
		...
		if ( $noteaff1 != "" ) {
		        $noteMoyEleGTempo = $noteaff1 * $coeffGen;
	                $noteMoyEleG=$noteMoyEleG + $noteMoyEleGTempo;
                	$coefEleG=$coefEleG + $coeffGen;
		}
		$noteaff1="";
	}
...
//	$pdf->MultiCell(10,$hauteurMatiere,'',1,'L',0);
	...
	$pdf->SetFont('Arial','',12);
	$pdf->SetXY($XnotVal,$YnotVal);
	if ($uneseuledecimale == "oui") $noteaff=number_format($noteaff,1,'.','');
...
if ($uneseuledecimale == "oui") {
	if ($moyenEleve < 10)  { $moyenEleve="0".$moyenEleve; }
	$moyenEleveaff=substr($moyenEleve,0,4);
}else{
...
if ($uneseuledecimale == "oui") $moyenClasseGenaff=number_format($moyenClasseGenaff,1,'.','');
...
if ($uneseuledecimale == "oui") $moyenClasseMinaff=number_format($moyenClasseMinaff,1,'.','');
...
if ($uneseuledecimale == "oui") $moyenClasseMaxaff=number_format($moyenClasseMaxaff,1,'.','');
