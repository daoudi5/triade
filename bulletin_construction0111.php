...
if ($typeexamen == "bull0111") { $typeexamen="Bulletin ".ucwords($textTrimestre); $typecommentaire="";    $examen=""; }  // BAC Blanc
...
for($j=$dep;$j<$nbEleveT;$j++) {  // premiere ligne de la creation PDF
	...
	$appreciation2="Appréciation d'ensemble : ";
...
//	$pdf->WriteHTML(ucwords(trim($textTrimestre)));
	...
	$pdf->WriteHTML("Note");
	$pdf->SetX(125);
	$pdf->WriteHTML("Appréciations des professeurs");
	// fin des titres
...
$pdf->MultiCell($LargeurMG,20,'',1,'L',0);
$pdf->SetXY(10,$YmoyenneGeneral+1);
$pdf->MultiCell($largeurMat,3,"TOTAL : " ,0,'R',0);
$pdf->SetY($YmoyenneGeneral+5.5);
$pdf->MultiCell($largeurMat,3,"MOYENNE : " ,0,'R',0);
$pdf->SetY($YmoyenneGeneral+10);
$pdf->MultiCell($largeurMat,3,"Moyenne de la classe : " ,0,'R',0);
$pdf->SetXY($largeurMat+13,$YmoyenneGeneral+10);
$pdf->MultiCell(10,3,"$moyenClasseGenaff" ,0,'L',0);
$pdf->SetY($YmoyenneGeneral+15);
...
$pdf->SetXY($largeurMat+36,$YmoyenneGeneral+15);
$tt=$moyenClasseMax*$moyenCoeffGenaff;
$pdf->MultiCell(10,3,"$tt" ,0,'L',0);

rsort($classement,SORT_NUMERIC);
foreach ($classement as $key => $val) {	
	if ($val == $moyenEleve) { $val1 = $key; }
}
$val1++;
if ($val1 == 1) { $rang="1 er"; }else{ $rang="$val1 ème"; }
$pdf->SetXY($largeurMat+15+32+13,$YmoyenneGeneral+1);
$pdf->MultiCell(30,3,"$rang",0,'L',0);
$pdf->SetXY($largeurMat+15+32+13,$YmoyenneGeneral+5);
...
$pdf->SetFillColor(220);
$pdf->MultiCell(15,20,'',1,'L',0);
$pdf->SetXY($XMoyCL,$YMoyGE);
$pdf->MultiCell(32,20,'',1,'L',0);
