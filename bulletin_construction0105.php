...
for($j=0;$j<count($eleveT);$j++) {  // premiere ligne de la creation PDF
    ...
    $infoeleve2=LANGELE4." : $classe_nom";
    ...
    // adresse de l'élève
    // elev_id,nomtuteur, prenomtuteur, num_adr1,   adr1, code_post_adr1, commune_adr1, num_adr2, adr2, code_post_adr2, commune_adr2, numeroEleve, class_ant, date_naissance, regime
    ...
    for($ik=0;$ik<=count($dataadresse);$ik++) {
        ...
        $regime=$dataadresse[$ik][14];
        $class_ant=trunchaine($dataadresse[$ik][12],20);
        ...
        //$pdf->SetXY(98,36);
        $pdf->SetXY(110,36);
        $chaine="Employeur : ".trim(strtoupper($nomtuteur))." ".trim(ucwords(strtolower($prenomtuteur)));
        $pdf->WriteHTML(trunchaine($chaine,36));
        $pdf->SetXY(110,42);
        $chaine=trim($adr1);
        $pdf->WriteHTML(trunchaine($chaine,36));;
        $pdf->SetXY(110,48);
    ...
    for($i=0;$i<count($ordre);$i++) {
        ...
        $pdf->MultiCell(10,$hauteurMatiere,'',0,'L',0);
    ...
    $pdf->SetFont('Arial','',7);
    $pdf->SetXY($Xcom,$Ycom);
    $pdf->MultiCell(87,3,$commentaireeleve,'','','L',0);
