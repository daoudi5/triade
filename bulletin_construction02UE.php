...
if ($_POST["typetrisem"] == "semestre") {
	...
	if ($_POST["saisie_trimestre"] == "trimestre3" ) { $textTrimestre="Annuel"; $triabsconet=""; $sem='12'; $tricoef=''; }
}
...
if ($_POST["saisie_trimestre"] == "trimestre3" ) {
	$dateDebut=$dateDebutS1;
	$dateFin=$dateFinS2;
...
for($j=$dep;$j<$nbEleveT;$j++) {  // premiere ligne de la creation PDF
	...
	if (file_exists($logo)) {
		$pdf->Image($logo,80,3,50,20);	
	}
	$logo="./image/banniere/UPA_carre.jpg";
	if (file_exists($logo)) {
		$pdf->Image($logo,180,3,20,20);	
	}
	...
	$pdf->SetXY($X+15,$Y);
	$pdf->MultiCell(70,3,"  $classe_nom",0,'L',0);
	$pdf->SetFont('Arial','',8);
	$pdf->SetXY($X,$Y+=5);
...
// ----------------------------------------------------
	...
	for($f=0;$f<count($recupUE);$f++) {
		...
		if ($nbmatiere*$hauteurMatiere+$Y > 230) {
			$pdf->SetFont('Arial','',6);
			$pdf->SetXY(3,260);
			$pdf->MultiCell(200,3,"L'Université Professionnelle d'Afrique est une association à vocation francophone présente sur les site de Brazzaville-Kinshasa-Pointe-Noire.\nPartenaire du Groupe ESG - www.univpro-afrique.com \n\nAucun duplicata de ce document ne pourra être édité. Veillez à faire des copies.",0,'C',0);
...
if (($_POST["affdocattestation"] == "oui") && ($ectsGeneral >= 60)) {
	...
	$logo="./image/banniere/upa.jpg";
	if (file_exists($logo)) {
		$pdf->Image($logo,$X,$Y,60,20);	
	}
