...
for($i=0;$i<count($data);$i++) {
	$em=""; $emF=""; $bgcolor="";
	if (ereg_replace("-",'',$data[$i][3]) < dateYMD() ) {  $em="<i>"; $emF="</i>"; $bgcolor="bgcolor='#E4E4E4'"; }
	$id=$data[$i][0];
	...
	if ((LAN == "oui") && (AGENTWEB == "oui")) {
		...
		$mess="<iframe width=100 height=100 src=\'${http}www.triade-educ.com/agentweb/agentmel.php?inc=5&mess=$vocal\'  MARGINWIDTH=0 MARGINHEIGHT=0 HSPACE=0 VSPACE=0 FRAMEBORDER=0 SCROLLING=no align=left ></iframe>".$data[$i][6] ;
	}
	...
	if (ereg_replace("-",'',$data[$i][3]) >= dateYMD() ) {
