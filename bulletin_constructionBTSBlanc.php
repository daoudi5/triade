...
// Relevé de Travaux d'Application et de Synthèse Par Philippe GISCLON
// Version 1.0 (12/01/2010) - Modif par l'équipe TRIADE
...
$typeexamen=$_POST["examen"]; // inutile

$typeexamen="BTS BLANC"; // nom affiché en en-tête
$typecommentaire="btsblanc";
$examen="BTS Blanc"; // type des notes enregistrées dans la base
...
for($j=$dep;$j<$nbEleveT;$j++) {  // premiere ligne de la creation PDF
	...
	$duplicata="Relevé à conserver précieusement" . " - $urlsite - $mail";
...
######
	...
	$photo=recup_photo_bulletin_idsite($idsite);
	if (count($photo) > 0) {
...
######
	...
	$pdf->SetXY($xcoor0+58,$ycoor0+45);
	$pdf->SetFont('Arial','',16);
	$pdf->WriteHTML("<B>".trim($typeexamen)."</B>");
	// fin titre
	...
	$pdf->WriteHTML(trim(trunchaine($classe_nom,30))." ($Pdate)");
	// fin d'insertion
	...
	for($ik=0;$ik<=count($dataadresse);$ik++) {
		...
		$pdf->SetFont('Arial','',10);
		$chaine="M ou Mme ".trim(strtoupper($nomtuteur))." ".trim(ucwords(strtolower($prenomtuteur)));
	...
	$pdf->WriteHTML("Note");
	$pdf->SetX(130);
	$pdf->WriteHTML("Commentaires éventuels");
	// fin des titres
	...
	for($i=0;$i<count($ordre);$i++) {
		...
		$noteaff1=$noteaff;
		if ($noteaff1 != "") {$noteaff1=round($noteaff1,2);}
		if (($noteaff1 < 10) && ($noteaff1 != "")) { $noteaff1="0".$noteaff1; }
		if (strlen($noteaff1) == 2) {$noteaff1 = $noteaff1.".00";}
		if (strlen($noteaff1) == 4) {$noteaff1 = $noteaff1."0";}
 ...
 */
	...
	$noteaveccoef="";
	if ($noteaff1 != "") {
		$noteaveccoef=$coeffaff*$noteaff1;
		$noteaveccoef=round($noteaveccoef,2);
	}
...
// $pdf->WriteHTML($moyenCoeffGenaff); // total des coef même ceux sans notes
$pdf->WriteHTML($coefEleG); // total des coef mais uniqueent ceux avec notes
$moyenCoeffGenaff="";
...
if ((file_exists("./data/image_pers/logo_signature$idsite.jpg"))){
	$pdf->SetFont('Arial','',7);
	$pdf->SetXY(165,$YmoyenneGeneralT+48); //$pdf->SetXY(120,$YmoyenneGeneralT+6.5);
	//$pdf->WriteHTML("[ <I>Signature du directeur</I> ]");
	$taille = getimagesize("./data/image_pers/logo_signature$idsite.jpg");
	$logox=$taille[0]/12; //$logox=$taille[0]/25;
	$logoy=$taille[1]/12; //$logoy=$taille[1]/25;
	$pdf->Image("./data/image_pers/logo_signature$idsite.jpg","160",$YmoyenneGeneralT+15,$logox,$logoy);
	//$pdf->Image("./data/image_pers/logo_signature.jpg","150",$YmoyenneGeneralT-6,$logox,$logoy);
	$pdf->WriteHTML("[ <I>Signature du directeur</I> ]");
}
...
// Affichage de la mention d'admission
	$pdf->SetFont('Arial','',14);
	$pdf->SetXY(140,$YmoyElGenValue);
	if ($moyenEleveaff <10) {
		$pdf->WriteHTML('<font color="#FF0000"><B>NON ADMIS(E)</B></font>');
	} else {
		$pdf->WriteHTML('<B>ADMIS(E)</B>');
	}
$noteMoyEleG=0; // pour la moyenne de l'eleve general
$coefEleG=0; // pour la moyenne de l'eleve general
...
$pdf->WriteHTML("<u>Commentaires</u>");
...
$fichier="./data/pdf_bull/$classe_nom/bulletin_BTS_BLANC_".$nomEleve."_".$prenomEleve."_".$_POST["saisie_trimestre"].".pdf";
@unlink($fichier); // destruction avant creation
$pdf->output($fichier);
$pdf->close();
bulletin_archivage("BTS BLANC ".$_POST["saisie_trimestre"],$anneeScolaire,$fichier,$idEleve,$classe_nom,$nomEleve,$prenomEleve);
