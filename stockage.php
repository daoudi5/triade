...
function is_editable($fichier)
	{
	$retour=0;
	if(eregi("\.txt$|\.sql$|\.php$|\.php$|\.phtml$|\.htm$|\.html$|\.cgi$|\.pl$|\.js$|\.css$|\.inc$",$fichier)) {$retour=1;}
	return $retour;
	}
...
function barre_outil($revenir)
	...
	//echo "<a href=\"http://support.triade-educ.com/stockage/index.php?inc=".GRAPH."&URL=".URLSITE."\"><img src=\"image/stockage/hdd.gif\" alt=\"Stockage +\" border=\"0\"></a>&nbsp;&nbsp;\n";
...
$racine=WEBROOT."/".ECOLE."/data/stockage/".$_SESSION["membre"]."/";
if (!is_dir($racine)) {
	mkdir($racine,0755);
}
...
$racine=WEBROOT."/".ECOLE."/data/stockage/".$_SESSION["membre"]."/".$id_pers."/";
if (!is_dir($racine)) {
	mkdir($racine,0755);
}
...
echo "<TEXTAREA NAME=\"code\" rows=\"30\" cols=\"160\" wrap=\"OFF\">\n";
...
if (!VerifInodeTotal($racine)) {
	...
	if($nomfic=="") {
		...
		if(eregi("\.html$",$nomfic)||eregi("\.html$",$nomfic))	{
...
if(!connecte($id))
	{
	echo "<a href=\"http://www.webjeff.org/langages/php_scripts.html\" target=\"_blank\">\n";
	echo "<img src=\"image/stockage/logo.gif\" width=\"133\" height=\"77\" border=\"0\"></a><br><br>\n";
