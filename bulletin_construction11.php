...
for($j=0;$j<count($eleveT);$j++) {  // premiere ligne de la creation PDF
	...
	$appreciationbis="($nbretard retard(s) / $nbabs demi-journée(s) d'absence(s) / $nbheureabs heure(s) d'absence(s) ) " ;
...
//	$pdf->WriteHTML($infoeleve2);
	...
	for($ik=0;$ik<=count($dataadresse);$ik++) {
		...
		$pdf->SetXY($Xv1,41); 
		$pdf->SetFont('Arial','',8);
		$pdf->WriteHTML("N°: $numero_eleve - Régime: $regime ");
		$pdf->SetXY($Xv1,46);
	...
	$pdf->SetFont('Arial','',9);
	$pdf->SetFillColor(220);
	$pdf->SetXY(15,62); //  placement  cadre titre
	...
	$pdf->SetFont('Arial','',$confPolice[0]+0.8);
	$pdf->SetXY($Xcom,$Ycom+0.3);
...
if ($YmoyenneGeneral > 210) {
	$pdf->AddPage();
	$YmoyenneGeneral=20;
}
$LargeurMG=$largeurMat;
$YmoyenneGeneralT=$YmoyenneGeneral + 0.5;
...
$pdf->SetFont('Arial','',10);
$pdf->WriteHTML("Avis du chef d'établissement : ".$directeur);
$pdf->SetFont('Arial','',9);
