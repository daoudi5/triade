...
for($j=0;$j<count($eleveT);$j++) {  // premiere ligne de la creation PDF
	...
	$logo="./image/banniere/banniere-createc.jpg";
	if (file_exists($logo)) {
		$xlogo=80;
		$ylogo=30;
...
//	$pdf->WriteHTML($coordonne2);
	...
	$pdf->SetXY(5,40); // placement du cadre du nom de l eleve
	...
	$pdf->SetFont('Arial','',9);
	$pdf->SetFillColor(220);
	$pdf->SetXY(5,62); //  placement  cadre titre
	...
	for($i=0;$i<count($ordre);$i++) {
		...
		$largeurMat=40;
		$hauteurMatiere=11; // taille du cadre matiere
		...
		// Mise en place des 3 notes
		$pdf->SetFont('Arial','',8);
		$pdf->SetXY($Xmat+$largeurMat,$Ymat);
		$pdf->MultiCell(20,$hauteurMatiere,'',1,'L',0);
		$pdf->SetXY($Xmat+$largeurMat,$Ymat+0.5);
		$recupNote=recupNoteBull(4,$idEleve,$ordre[$i][0],$dateDebut,$dateFin,$idprof,$idgroupe); // recupe les 3 premieres notes
		$pdf->MultiCell(20,3.5,"$recupNote",0,'L',0);
		//$pdf->WriteHTML("$recupNote");
	


		$Ymat=$Ymat + $hauteurMatiere;

		// mise en place de la colonne cap blanc
		$pdf->SetFont('Arial','',8);
		...
		$pdf->SetFont('Arial','',9);
		$pdf->SetXY($XcoeffVal+9,$YcoeffVal);
		$examen="CAP Blanc";
		if ($idgroupe == "0") {
			$noteCAPBlanc=moyenneEleveMatiereExamen($idEleve,$ordre[$i][0],$dateDebut,$dateFin,$idprof,$examen);
		}else{
			$noteCAPBlanc=moyenneEleveMatiereGroupeExamen($idEleve,$ordre[$i][0],$dateDebut,$dateFin,$idgroupe,$idprof,$examen);
		}
		$pdf->WriteHTML($noteCAPBlanc);
	...
	$pdf->SetFont('Arial','',$confPolice[0]+1.5);
	$pdf->SetXY($Xcom+10,$Ycom);
	$pdf->MultiCell(87,$confPolice[1]+1,$commentaireeleve,'','','L',0);
	...
	$pdf->SetFont('Arial','',6);
	$pdf->SetXY($XprofVal-10,$YprofVal);
	...
	// pour le calcul de la moyenne general de l'eleve CAP BLANC
	if ( $noteCAPBlanc != "" ) {
	        $noteMoyEleGTempo = $noteCAPBlanc * $coeffaff;
                $noteMoyEleGCAP=$noteMoyEleGCAP + $noteMoyEleGTempo;
                $coefEleGCAP=$coefEleGCAP + $coeffaff;
	}
}
...
// Note Vie Scolaire
if ((MODNAMUR0 == "oui") && ($_POST["affnoteviescolaire"] == "oui")){
...
//	$pdf->MultiCell(10,$hauteurMatiere,'',1,'L',0);
	...
	$coeffaff=$coefBull;
	$pdf->SetFont('Arial','',9);
	$pdf->SetXY($XcoeffVal+10,$YcoeffVal);
...
// affichage Moyenne CAP BLANC
$moyenEleveCAP=moyGenEleve($noteMoyEleGCAP,$coefEleGCAP);
$pdf->SetXY($XmoyElValue-2,$YmoyElGenValue);
$pdf->WriteHTML("$moyenEleveCAP");
$moyenEleveCAP="";$noteMoyEleGCAP="";$coefEleGCAP="";
