...
for($j=$dep;$j<$nbEleveT;$j++) {  // premiere ligne de la creation PDF
	...
	$appreciationbis="- $nbretard retard(s) ";
	$appreciationbis2="- $nbabs demi-journée d'absence(s) / $nbheureabs heure(s) d'absence(s)";
	...
	$pdf->SetFont('Arial','',9);
	$pdf->SetFillColor(220);
	$pdf->SetXY(15,51); //  placement  cadre titre
	$pdf->MultiCell(184,11,'',1,'C',1);
	$pdf->SetXY(25,53); // placement contenu titre
 ...
 */
	...
	$pdf->SetFont('Arial','',8);
	$pdf->SetXY(112,$YmoyenneGeneral);
	$pdf->MultiCell(87,10,'',1,'L',0);
	$pdf->SetXY(113,$YmoyenneGeneralT-1);
	$pdf->WriteHTML("$appreciationbis");
	$pdf->SetXY(113,$YmoyenneGeneralT+3);
	$pdf->WriteHTML("$appreciationbis2");
...
$pdf->SetXY(16,$YcomP1);
$pdf->WriteHTML("Appréciation du chef d'établissement : ");
$pdf->SetFont('Arial','',8);
//$pdf->WriteHTML($appreciationbis);
