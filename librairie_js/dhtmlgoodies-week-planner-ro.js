...
var instantSave = false;	// Save items to server every time something has been changed (i.e. moved, resized or text changed) -
...
var externalSourceFile_save = '';	// File called by ajax when changes are made to an element
var externalSourceFile_delete = '';	// File called by ajax when an element is deleted. Input to this file is the variable "eventToDeleteId=<id>"
var popupWindowUrl = '';	// Called when double clicking on an event. use false if this option is disabled.

var txt_deleteEvent = '';	// Text in dialog box - confirm before deleting an event
...
function editEventWindow(e,inputDiv)
{
	...
	var editEvent = window.open(popupWindowUrl + '?id=' + inputDiv.id,'editEvent','width=600,height=550,status=no');
	editEvent.focus();
}
	...
	for(var no=1;no<items.length;no++){
		...
		if(itemsToBeCreated[no]['id']){
			...
			if (itemsToBeCreated[no]['idgroupe'] > 0) {
                                ...
                                currentAppointmentDiv = createNewAppointmentDiv((el_x - appointmentsOffsetLeft)+decal,topPos,(newAppointmentWidth-(appointmentMarginSize*2))/2,itemsToBeCreated[no]['description'],elHeight);
                        }else{
...
function displayPreviousProf2()
{
	idProf=document.form.profID.value;
//	updateHeaderDates();	
	clearAppointments();
	getItemsFromServer();
}
...
function displayPreviousClasse2()
{
	var val=document.form.classeID.options.selectedIndex
	idClasse=document.form.classeID.options[val].value;
	if (idClasse == "-1") {
...
function displayPreviousClasse3()
{
	idClasse=document.form.classeID.value;
	idProf="";
//	updateHeaderDates();	
	clearAppointments();
	getItemsFromServer();
