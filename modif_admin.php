...
if(isset($_POST["create"])){
	// requete ? prenom2 ?
	$cr=modif_personnel($_POST["id_pers"],$_POST["saisie_creat_nom"],$_POST["saisie_creat_prenom"],$_POST["saisie_intitule"],$_POST["saisie_creat_adr"],$_POST["saisie_creat_code"],$_POST["saisie_creat_tel"],$_POST["saisie_creat_mail"],$_POST["saisie_creat_commune"],$_POST["saisie_creat_tel_port"],0,$_POST['saisie_pays'],$_POST["saisie_indice_salaire"]);
	if($cr == 1){
		alertJs(LANGMODIF14);
	...
	$saisie_id=$_GET["saisie_id"];
	  ...
	  $passage_argument="oui"; // pour le JavaScript
          ...
          // pers_id,nom,prenom,mdp,civ,email,adr,code_post,commune,tel,tel_port,identifiant,offline,id_societe_tuteur,pays
...
<tr><td align=right width=40%><font class="T2"><?php print LANGNA3?>&nbsp;:&nbsp;</font></td><td><input type=button onclick="open('modif_pers_pass.php?id=<?php print $saisie_id;?>&type=ADM','pass','width=400,height=300')" value='<?php print LANGPER30 ?>'  class="bouton2" > </td></tr>
</table>
</fieldset>
<br><br><br>
...
<script language=JavaScript>buttonMagicSubmit("<?php print LANGMODIF13 ?>","create"); //text,nomInput</script>
	<script language=JavaScript>buttonMagic("<?php print LANGBT8?>","list_admin.php","_parent","","");</script><br><br>
<?php if ($offline == 0) { ?>
	<script language=JavaScript>buttonMagicSubmit("<?php print LANGMESS398 ?>","offline"); //text,nomInput</script>
<?php } else {?>
