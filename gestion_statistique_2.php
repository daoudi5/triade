...
$choix=$_POST["saisie_nature"];
	...
	$worksheet3->write(0, 2, "Nb heure réalisé", $header);
	$worksheet3->write(0, 3, "Coût programmé", $header);
	$worksheet3->write(0, 4, "Coût réalisé", $header);
	...
	for($i=0;$i<count($data);$i++) {
		...
		for($k=0;$k<count($tabtmp);$k++) {
			...
			list($nbheure,$nbmin,$nbsec)=split(':',$tabtmp[$k][0]);
	...
	for($i=0;$i<count($data);$i++) {
		...
		if ($idclasse > 0) {
			...
			$worksheet2->write($j, 2, "$Nb_heure_realise", $center);
			$worksheet2->write($j, 3, "$Cout_programme", $center);
			$worksheet2->write($j, 4, "$Cout_realise",$center);
			$j++;
		}
	}
	...
	$j=1;
	for($i=0;$i<count($data);$i++) {
		$nomprenom=ucwords($data[$j][2])." ".ucwords($data[$j][3]);
		$idpers=$data[$j][0];
		...
		if ($idpers > 0) {
			...
			$worksheet3->write($j, 2, "$Nb_heure_realise", $center);
			$worksheet3->write($j, 3, "$Cout_programme", $center);
			$worksheet3->write($j, 4, "$Cout_realise",$center);
			$j++;
		}
	}
...
<br>
<center>
<input type=button onclick="open('visu_document.php?fichier=<?php print $fichier?>','_blank','');" value="<?php print "Récupération des statistiques" ?>"  class="bouton2">
<br /></center><br><br>
