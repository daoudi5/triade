...
//alertJs("EN MAINTENANCE MERCI DE REVENIR PLUS TARD, SERVICE TRIADE");
validerequete("2");
$cnx=cnx();
$debut=deb_prog();
...
$ue=gestion_ue($idClasse,$ue_semestre,true); 
...
for($g=0;$g<count($eleveT);$g++) {
	...
	for ($nb_ue=0;$nb_ue<count($ue);$nb_ue++) { //  a.code_ue, a.num_ue, a.nom_ue, m.code_ue_detail, m.code_matiere, a.semestre
		...
		if ($ecrit==$ue[$nb_ue][2]) {
			...
			if ($ecrit!='' ) {
			$id_ue = $ecrit;
			unset($som_coef_partiel_euS1);
                        unset($som_coef_partiel_euS2);
                        unset($somme_coefS1);
                        unset($somme_coefS2);
	...
	$notepartiel = recupNotepartiel($idEleve,$idMatiere,$dateDebutS1,$dateFinS1,$idClasse);
        ...
        for ($nb_note=0;$nb_note<count($notepartielS1);$nb_note++) {
                 if ($notepartielS1[$nb_note][0]>=0 ) {
                        ...
                        $som_coef_partiel_euS1+=$notepartielS1[$nb_note][6];
                        $moyenne_ueS1+=$notepartielS1[$nb_note][6]*$notepartielS1[$nb_note][0];
                        $somme_coefS1+=$notepartielS1[$nb_note][6];
                        $affich_coef_partielS1=$notepartielS1[$nb_note][6];
                        $moyenne_gen_partielS1=$moy_partiel_euS1;
                        $coef_gen_partielS1+=$notepartielS1[$nb_note][6];

			$moyenne_ueG+=$notepartielS1[$nb_note][6]*$notepartielS1[$nb_note][0];
			$coef_ueG+=$notepartielS1[$nb_note][6];
                 }else{
                        $affich_coef_partielS1="";
                 }
        }
        ...
        for ($nb_note=0;$nb_note<count($notepartielS2);$nb_note++) {
                 if ($notepartielS2[$nb_note][0]>=0 ) {
                        ...
                        $som_coef_partiel_euS2+=$notepartielS2[$nb_note][6];
                        $moyenne_ueS2+=$notepartielS2[$nb_note][6]*$notepartielS2[$nb_note][0];
                        $somme_coefS2+=$notepartielS2[$nb_note][6];
                        $affich_coef_partielS2=$notepartielS2[$nb_note][6];
                        $moyenne_gen_partielS2=$moy_partiel_euS2;
                        $coef_gen_partielS2+=$notepartielS2[$nb_note][6];

			$moyenne_ueG+=$notepartielS2[$nb_note][6]*$notepartielS2[$nb_note][0];
			$coef_ueG+=$notepartielS2[$nb_note][6];
                 }else{
                        $affich_coef_partielS2="";
                 }
         }
...
if ($coef_gen_partielS1 > 0) { $moyenne_ueS1=$moyenne_gen_partielS1/$coef_gen_partielS1;  $nbMoyenPartiel++; }
if ($coef_gen_partielS2 > 0) { $moyenne_ueS2=$moyenne_gen_partielS2/$coef_gen_partielS2;  $nbMoyenPartiel++; }
...
//print $moyenne_ueS1." ".$moyenne_ueS2." ==> ";
if ($nbMoyenPartiel > 0) $moyenne_gen_partiel=($moyenne_ueS1+$moyenne_ueS2) / $nbMoyenPartiel;
$moyenne_ue+=$notepartielS1[$nb_note][6]*$notepartielS1[$nb_note][0];
$coef_ue+=$notepartielS1[$nb_note][6];

if ($coef_ueG  > 0) $moyenne_gen_partiel=$moyenne_ueG/$coef_ueG;

unset($moyenne_ueG);
unset($coef_ueG);

unset($nbMoyenPartiel);
unset($nbMoyenPartielS1);
unset($nbMoyenPartielS2);
unset($coef_gen_partielS1);
unset($coef_gen_partielS2);
unset($som_coef_partiel_euS1);
unset($som_coef_partiel_euS2);
unset($somme_coefS1);
unset($somme_coefS2);
$moyenne_gen_partiel=number_format($moyenne_gen_partiel,'2','.','');
//print $moyenne_gen_partiel.'<br>';
...
//print_r($moyenne_par);
//print ($moyenne_par[0].'---'.end($moyenne_par))."<br>";
//print ('<br>----<br>');
sort($moyenne_per);
//print "<hr>";
//print_r($moyenne_per);
...
// premiere ligne
	...
	$pdf->MultiCell(80,6,'MOYENNES',1,'C',0);
...
// deuxieme ligne
	...
	$pdf->SetXY($Xmat+$largeurMat,$Ymat);
	$pdf->SetFillColor(198,209,229);  // couleur du cadre de l'eleve
	$pdf->MultiCell(10,6,'Coef.',1,'C',1);
	$pdf->SetXY($Xmat+$largeurMat+10,$Ymat);
	$pdf->MultiCell(10,6,'ECTS',1,'C',1);
	$pdf->SetFillColor(198,209,229);  // fond blanc
	$Xmat1=$Xmat+$largeurMat+20;
	$pdf->SetXY($Xmat1,$Ymat);
	$pdf->MultiCell(13,6,'Partiel 1',1,'C',1);
	$Xmat1=$Xmat1+13;
	$pdf->SetXY($Xmat1,$Ymat);
	$pdf->MultiCell(13,6,'Partiel 2',1,'C',1);
	$Xmat1=$Xmat1+13;
	$pdf->SetXY($Xmat1,$Ymat);
	$pdf->MultiCell(14,6,'Moyenne',1,'C',1);
	$Xmat1=$Xmat1+14;
	$pdf->SetXY($Xmat1,$Ymat);
	$pdf->MultiCell(20,6,'Moyenne 1',1,'C',0);
	$Xmat1=$Xmat1+20;
	$pdf->SetXY($Xmat1,$Ymat);
	$pdf->MultiCell(20,6,'Moyenne 2',1,'C',0);
	$Xmat1=$Xmat1+20;
	$pdf->SetXY($Xmat1,$Ymat);
	$pdf->MultiCell(20,6,'Moyenne ',1,'C',0);
	$Xmat1=$Xmat1+20;
	$pdf->SetXY($Xmat1,$Ymat);
	$pdf->MultiCell(20,6,'Moyenne G.',1,'C',0);
...
for ($nb_ue=0;$nb_ue<count($ue);$nb_ue++) { // boucle pour chaque UE
	...
	//		if ($noteperiode[0][0]!=0 && $noteperiode[0][0]!='') {$nb_mat+=1;}
		...
		if ($ecrit!='') {
			...
			$pdf->SetFont('Arial','',7);
			$pdf->SetXY($Xmat,$Ymat);
			$pdf->MultiCell(40,6,' Moyenne min.:'.format_moyenne($partiel[$id_ue]['min']).' - max:'.format_moyenne($partiel[$id_ue]['max']),'T','C',1);
			$pdf->SetFont('Arial','',7);
			$pdf->SetXY($Xmat+40,$Ymat);
			$pdf->MultiCell(60,6,''.format_moyenne($moyenne_ue).'',1,'C',1);
...
//			print ('nbre2 : '.$nb_mat);
		...
		//	$moyenne_periode_ue=$moyenne_periode_ueGG;
...
//			print('$moyenne_periode_ue:'.$moyenne_periode_ue. '--$moyenne_periode_gen : ' .$moyenne_periode_gen);
			$moyenne_periode_generale="";
			$coeff_periode_generale="";
			$moyenne_GG+=$moyenne_periode_ue;
			$moyenne_periode_ue='';
			$moyenne_periode_ueGG='';
			$nb_mat=1;
			$Ymat+=5;
			$nb_note_periode_ue=0;
			$nb_mat_per=0;
...
//		$pdf->WriteHTML(''.$nom_ue.'');
	...
	////	$moyenne_ue=$moyenne_ue/$somme_coef;
		...
		$pdf->SetXY($Xmat+$largeurMat,$Ymat);
		$moy_coef_partiel="";
		$moy_coef_partiel=recupCoeff($idMatiere,$idClasse,$ordre_recup);
		$coeff_gg=$moy_coef_partiel;
		$pdf->MultiCell(10,$hauteurMatiere,$moy_coef_partiel,'LR','C',1);

		$pdf->SetXY($Xmat+$largeurMat+10,$Ymat);
		$pdf->MultiCell(10,$hauteurMatiere,$ects,'LR','C',1);
...
//print($nb_ue.'coef_pa'.$notepartiel[0][6].' ;');
		...
		$pdf->MultiCell(13,$hauteurMatiere,$affiche_note,'LR','C',1);
		$moyenPartiel2=$affiche_note;
//print ('$moy_partiel_eu ='.$moy_partiel_eu .' $affiche_note ='.$affiche_note .' $moy_coef_partiel ='.$moy_coef_partiel .' $idEleve='.$idEleve.' $idMatiere= '.$idMatiere .'<br>');	
//print('note_pa'.$notepartiel[0][0].'; ');	
//		if ($idgroupe == "0") {
...
//		}
		...
		$pdf->SetXY($Xmat+$largeurMat+20+13,$Ymat);
		$pdf->MultiCell(13,$hauteurMatiere,$affiche_note,'LR','C',1);		
		$moyenPartiel1=$affiche_note;

		// ------------------------------------------
		$moyenne_partiel=$moyenneMatPartiel;
		if ($moyenPartiel1 != "") $nbMoyPartiel++;
		if ($moyenPartiel2 != "") $nbMoyPartiel++;
		$moyenneMatPartiel=($moyenPartiel2+$moyenPartiel1)/$nbMoyPartiel;
		if ($moyenneMatPartiel != "") {
			$moyenneMatPartielAff=number_format($moyenneMatPartiel,2,'.','');
			$moyenne_partiel=$moyenneMatPartielAff;
		}
		$pdf->SetXY($Xmat+$largeurMat+20+13+13,$Ymat);
		$pdf->MultiCell(14,$hauteurMatiere,$moyenneMatPartielAff,'LR','C',1);
		unset($nbMoyPartiel);
		unset($moyenneMatPartiel);
		unset($moyenneMatPartielAff);
...
/*		$noteperiode= recupNoteperiode($idEleve,$idMatiere,$dateDebutS1,$dateFinS2);
		...
		if ($moyenne_periode >=0 && is_numeric($moyenne_periode) && $nb_note_periode>0) { 
			...
			$moyenne_periode=$moyenMadridP2+$moyenMadridP1;


			$moyenne_periode=number_format($moyenne_periode,2,'.','');
			$pdf->MultiCell(20,$hauteurMatiere,$moyenne_periode,'R','C',0);
		
			$moyenne_periode_generale+=$moyenne_periode*$coeff_gg;
			$coeff_periode_generale+=$coeff_gg;

		
		} else {
*/
		if ($moyenP1 != "") $nbmoyen_periode++;
		if ($moyenP2 != "") $nbmoyen_periode++;
		$moyenne_periode=($moyenP1+$moyenP2)/$nbmoyen_periode;
		unset($nbmoyen_periode);
		if ($moyenne_periode != "") $moyenne_periode_aff=number_format($moyenne_periode,2,'.','');
		$pdf->SetFont('Arial','',7);
		$pdf->SetXY($Xmat+$largeurMat+40+20+20+20,$Ymat);
		$pdf->MultiCell(20,$hauteurMatiere,"$moyenne_periode_aff",'R','C',0);	

		if  ($moyenne_periode != "") {
			$nb_note_periode_ue++;
			$nb_mat_per++;
			$moyenne_periode_ue+=$moyenne_periode_aff;

		}

		if ($moyenne_periode != "") {
			$moyenne_general+=$moyenne_periode*1;
			$moyenGenNb+=1;
		}
		if ($moyenne_partiel != "") {
			$moyenne_general+=$moyenne_partiel*$coeff_gg;
			$moyenGenNb+=$coeff_gg;
		}
		if ($moyenGenNb > 0) $moyenne_general=$moyenne_general/$moyenGenNb;
		if ($moyenne_general != "") $moyenne_general_aff=number_format($moyenne_general,2,'.','');
		$pdf->SetXY($Xmat+$largeurMat+40+20+20+20+20,$Ymat);
                $pdf->MultiCell(20,$hauteurMatiere,"$moyenne_general_aff",'R','C',0);      // MOYENNE GENERALE

		unset($moyenne_periode);
		unset($moyenGenNb);
		unset($moyenne_general_aff);
		unset($moyenne_general);
		unset($moyenne_periode_aff);
...
// fin de la mise en place des matieres
	...
	$moyenne_periode_ueGG=$moyenne_periode_generale/$coeff_periode_generale;
} // fin boucle UE
...
//print_r ($periode);
			...
			if ($YmatAv >= 230) { $pdf->AddPage(); $Ymat=10; }
...
// periode
		// print ("moy: ".$moyenne_periode_gen."nb: ".$nb_mat_gen."<br>");
			$moyenne_periode_gen=$moyenne_GG / ($nb_mat_gen);
			unset($moyenne_GG);
			$pdf->SetFont('Arial','',8);
...
$commentairedirection=ereg_replace("\n"," ",$commentairedirection);
