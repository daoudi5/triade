...
<tr id='coulBar0' ><td height="2"><b><font   id='menumodule1' ><?php print LANGTITRE34 ?></font></b></td></tr>
<tr id='cadreCentral0' >
<td ><br>
...
if (isset($_POST["create"])) {
	if ($_FILES['rtf']['name'] != "") {
		...
		if ( (!empty($fichier)) &&  ($size <= 8000000)) {
			if  (($type == "application/msword") || (ereg('.rtf$',$fichier) ))  {
				@unlink("./data/parametrage/courrier_rtd.rtf");
				move_uploaded_file($tmp_name,"./data/parametrage/courrier_rtd.rtf");
				print "<BR><center><font class='T2'>"."Fichier enregistré"."</font></center>";
				config_param_ajout("{FICHIERRTFRTD}",'param_retard');
				$texte="{FICHIERRTFRTD}";
		...
		$texte=ereg_replace("\\\'","'",$texte);
		config_param_ajout($_POST["suite"],'param_retard');
	}
...
<?php
	...
	$data=config_param_visu('param_retard');
	print LANGPARAM2;
	if (trim($data[0][0]) != "{FICHIERRTFRTD}") {
		$textes=$data[0][0];
	}else{
...
<textarea id="editor" style="height: 48em; width: 100%;" name=suite><?php print $data[0][0] ?>
</textarea><br><br>
