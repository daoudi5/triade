...
if (!isset($_GET["ideleve"])) {
	...
	if (count($data) > 1) {
		...
		for($i=0;$i<count($data);$i++) {
			...
			print "<td width='5%'><input type='button' onclick=\"open('compta_consulte2.php?ideleve=".$data[$i][0]."','_parent','')\" class='button' value='Sélectionner' /></td>";
			print "</tr>";
		}
		print "</table>";
	}else{
...
<?php
	...
	for($j=0;$j<count($dataV);$j++) {
		...
		list($anneeversement,$moisverserment,$jourversement) = split('-',$dateversement);
		...
		$dateduJour=date("Ymd");
		$dateVersOr=ereg_replace("-","",$dateVersOr);
