...
if (count($valeur)) {
	...
	if ($_POST["typetriseman"] == "semestre") {
	...
	if ($_POST["saisie_trimestre"] == "trimestre1" ) $dateFin=$dateFinT1;
	if ($_POST["saisie_trimestre"] == "trimestre2" ) $dateFin=$dateFinT2;
	if ($_POST["saisie_trimestre"] == "trimestre3" ) $dateFin=$dateFinT3;
...
while(count($ordre)) {
	...
	while (list($key, $nom_matiere) = each($tabmatiere)) {
		...
		if (trim($nomSousMatiere) != "") {
			$nom_matiere=trunchaine(ucfirst($nom_matiere),5)." ".trunchaine($nomSousMatiere,5);
		}else{
			$nom_matiere=eregi_replace('education','educ',$nom_matiere);
	...
	for($j=0;$j<count($eleveT);$j++) {  // premiere ligne de la creation PDF
		...
		$pdf->SetFont('Arial','',8);
		$pdf->SetXY($xnomeleve,$ynomeleve);
		$pdf->WriteHTML(trunchaine(ucwords($nomEleve." ".$prenomEleve),21));
		...
		if (($afficheMoyen) && ($nbaffichematiere > count($ordre))) {
			...
			if (MODNAMUR0 == "oui") {
				$nombreviescol = 0;
       				$noteaff=calculNoteVieScolaireJusquauTrimestre($idEleve,$coefProf,$coefVieScol,$_POST["saisie_trimestre"]);
				if ( $noteaff != "" ) {
			...
			foreach ($tabmatiere3 as $key => $value)  {
				list($nom_matiere,$num_ordre,$idMatiere)=split("##",$value);
				$matiere=$nom_matiere;
				//$idMatiere=$key;
...
?>
Information : les moyennes g&eacute;n&eacute;rales sont r&eacute;alis&eacute;es via les moyennes des sous-mati&egrave;res et non la moyenne des moyennes des mati&egrave;res
<!-- // fin  -->
</td></tr></table>
