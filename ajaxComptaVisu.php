...
if (isset($_POST["id"])) {
	...
	$dateduJour=date("Ymd");
	$dateVersOr=ereg_replace("-","",$_POST["dateversor"]);


	$nbj=ereg_replace("-","",dateFormBase($dateVers))-$dateduJour;
	...
	if ($dateduJour >= ereg_replace("-","",dateFormBase($dateVers)))  {
	...
	if (($montantVers >= $_POST["montantavers"] ) && ($dateduJour >= ereg_replace("-","",dateFormBase($dateVers)))   ) {
