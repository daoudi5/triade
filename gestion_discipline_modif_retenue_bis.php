...
if (isset($_POST["idmodifeleve"])) {
	$cr=modifRetenue($_POST["idmodifeleve"],$_POST["date_de_la_retenue"],$_POST["heure_de_la_retenue"],$_POST["saisie_sanction"],$_POST["saisie_motif"]);
	if ($cr) {
...
<TD bgcolor='yellow' align=center width=5%>&nbsp;<?php print "Info."?>&nbsp;</td>
</TR>
<?php
...
?>
	<TR  class="tabnormal" onmouseover="this.className='tabover'" onmouseout="this.className='tabnormal'">
        <form method=POST name="formulaire<?php print $j ?>" action="gestion_discipline_modif_retenue_bis.php" >
	...
	<TD valign=top>
	&nbsp;<?php print ucwords(LANGDISC20) ?>: <select name=saisie_sanction onchange="searchRequest(this,'sanction','rien','formulaire<?php print $j ?>','saisie_motif')"  >
	<option id="select0" value="<?php print $data_2[$j][5] ?>"><?php print rechercheCategory($data_2[$j][5]) ?></option>
	<?php
	select_category();
	?>
	</select> <br />
	&nbsp;<?php print ucwords(LANGPARENT15) ?>: <b><select name="saisie_motif">
	<option id='select0' valu="<?php print  $data_2[$j][7] ?>" ><?php print  $data_2[$j][7] ?></option>
	</select></b><br />
	...
	<TD valign=top>
	<a href='#'  onMouseOver="AffBulle('<?php print "Attribué par : <b>".$qui?></b> <br> le  : <?php print dateForm($data_2[$j][3]) ?> ');"  onMouseOut="HideBulle()";><img src="image/commun/show.png" border='0' /></a>&nbsp;&nbsp;&nbsp;<a href='#' onclick='document.formulaire<?php print $j?>.submit();' title="Enregistrer" ><img src="image/commun/export.png" border='0' /></a>&nbsp;
	<input type="hidden" name="idmodifeleve" value="<?php print $data_2[$j][0] ?>" />
	<input type="hidden" name="date_de_la_retenue" value="<?php print $data_2[$j][1] ?>" />
	<input type="hidden" name="heure_de_la_retenue" value="<?php print $data_2[$j][2] ?>" />
