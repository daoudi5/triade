...
print "<font class=T1><ul>";
	...
	print "<br><br><br>";
	//id_serial,nom,contact,adresse,code_p,ville,secteur_ac,activite_prin,tel,fax,email,info_plus,bonus
...
?>
		...
		<div style="width:368px;" class="dhtmlgoodies_contentBox" id="box<?php print $i ?>">
			...
			<font class=T2>
			<?php
			$siren=($data[$i][16] == "Choix...") ? "" : $data[$i][16] ;
			$siret=($data[$i][17]  == "Choix...") ? "" : $data[$i][17] ;
			$naf=($data[$i][21] == "Choix...") ? "" : $data[$i][21] ;
			?>	
			...
			<?php print "Type Organisation" ?> : <b><?php print $data[$i][22] ?></b>  <br><br>
			...
			Plan : <a href="#" onclick="open('http://support.triade-educ.com/support/google-map-triade.php?etablissement=<?php print  urlencode($data[$i][1])?>&adresse=<?php print urlencode($data[$i][3]) ?>&ville=<?php print urlencode($data[$i][5]) ?>&pays=<?php print urlencode($data[$i][14])?>','_blank','width=450,height=350')" /><img src="image/commun/loupe.png" border="0" /></a><br><br><br>
