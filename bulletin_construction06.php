...
$data=aff_grp_bull_bonifacio("litteraire");
$idliste=$data[0][1];
$idliste=ereg_replace("[{}]",'',$idliste);
$tabliterraire=explode(",",$idliste);
...
//-------------------------
	...
	for($g=0;$g<count($eleveT);$g++) {
		...
		if (trim($moyenEleve2) != "") {
			$moyenEleve2=ereg_replace(',','.',$moyenEleve2);
...
//$nbheureabs="___";
//
$appreciation="Bilan Assiduité: $nbretard  retard(s) & $nbabs  demi-journée(s) d'absence(s) (dont ___ non justifiée(s)) et $nbheureabs heure(s) d'absence.                                  ";
...
$duplicata="<b>ATTENTION:Ce bulletin est l'original, il doit être conservé par la famille <b> - suivi d'un élève sur http://www.bonifacio.triade-educ.com";
$signature="Le responsable pédagogique de la classe: Professeur Principal  $profp /    La Principale: Mme CHASSAIN";
// FIN variables
...
// Debut création PDF
	...
	$pdf->SetXY(15,40); // placement du cadre du nom de l eleve
	...
	$Xv1=20;
	$Xv11=111;
	$pdf->SetXY($Xv1,41); // placement du nom de l'eleve
	...
	$pdf->SetFont('Arial','',9);
	$pdf->SetFillColor(220);
	$pdf->SetXY(15,65); //  placement  cadre titre
	$pdf->MultiCell(184,11,'',1,'C',1);
	$pdf->SetXY(25,67); // placement contenu titre
...
//print $effbordure." $libelleMatiere <br>";
		...
		$libelleMatiere=ereg_replace('0$','',$libelleMatiere);
		$pdf->WriteHTML('<B>'.trunchaine(strtoupper(sansaccent(strtolower($libelleMatiere))),19).'</B>');

		if ($sousmatiere != "") {
			$pdf->SetXY($Xmat+($largeurMat/2)+10,$Ymat+2.5);
			$pdf->SetFont('Arial','',7);
			$sousmatiere=ereg_replace('0$','',$sousmatiere);
		...
		if (($sousmatiere != "0") && ($sousmatiere != "")){
			...
			if ( trim($matiereEnCours) != trim($matiereSuivante)) {
				...
				// -----------------------
				foreach($tabliterraire as $value) {
					if ($idMatiere == $value) {
					//	print "Note : $noteaff1 - matiere : ".chercheMatiereNom($idMatiere)." - Coef : $coefsoustotalgroup - Tabscient : ".chercheMatiereNom($value)." <br>";
						if (trim($noteaff1) != "") {
							$moyenLiterraire+= $noteaff1 * $coefsoustotalgroup;
							$nbnoteLiterraire+=$coefsoustotalgroup;
						}
					}
				}
				...
				foreach($tabscientifique as $value) {
					if ($idMatiere == $value) {
						if (trim($noteaff1) != "") {
							$moyenScientifique+= $noteaff1 * $coefsoustotalgroup;
							$nbnotescientifique+=$coefsoustotalgroup;
						}
					}
				}
				// ------------------------
			...
			foreach($tabliterraire as $value) {
				if ($idMatiere == $value) {
					if (trim($noteaff1) != "") {
						...
						$moyenLiterraire+= $noteaff1 * $coeffaff;
						$nbnoteLiterraire+=$coeffaff;
					}
				}
			}
...
if (MODNAMUR0 == "oui") {
	...
	$pdf->SetXY($XmoyMatGVal,$YmoyMatGVal-3);
	$moyeMatGenaff=$moyeMatGen1;
	$noteCPE=cherche_note_scolaire_eleve_cpe($idEleve,"-10",$idClasse,$_POST["saisie_trimestre"],"-1");
	$noteEns=calculNoteVieScolaireEns($idEleve,$_POST["saisie_trimestre"]);
	$pdf->WriteHTML("CPE: $noteCPE/20");
	$pdf->SetXY($XmoyMatGVal,$YmoyMatGVal);	
	$pdf->WriteHTML("ENS: $noteEns/20");
...
//	$pdf->WriteHTML($moyeMatGenMaxaff);
	...
	$pdf->SetFont('Arial','',$confPolice[0]);
	$pdf->SetXY($Xcom,$Ycom);
	$pdf->MultiCell(87,$confPolice[1],$persVieScolaire." : ".$commentaireeleve,'','','L',0);
...
if ($moyenLiterraire !== ""){
	$noteLitteraire = $moyenLiterraire / $nbnoteLiterraire;
	$noteLitteraire=number_format($noteLitteraire,2,'.','');
	$noteLitteraire=ereg_replace('\.',',',$noteLitteraire);
}
if ($moyenScientifique !== ""){
	...
	$noteScientifique=ereg_replace('\.',',',$noteScientifique);
}
...
$pdf->SetXY($LargeurMG+25,$YMoyGE); // cadre note generale
$pdf->MultiCell(15,10,'',1,'L',0);
$pdf->SetXY($XMoyCL+5+17+2,$YMoyGE+2);  // placement du mot 
//$pdf->SetFont('Arial','',11);
//$pdf->WriteHTML("<b>$moyenEleve</b>");

// fin du cadre moyenne generale

$pdf->SetXY($XMoyCL+12+5,$YMoyGE);
$pdf->MultiCell(32,10,'',1,'L',0);  // cadre litteraire
$pdf->SetXY($XMoyCL+12+5+4,$YMoyGE+2);  // placement du mot 
$pdf->SetFont('Arial','',9);
$pdf->WriteHTML("<b>LITTERAIRE*</b>");

$pdf->SetXY($XMoyCL+17+32,$YMoyGE); // cadre note litteraire
$pdf->MultiCell(15,10,'',1,'L',0);
$pdf->SetXY($XMoyCL+17+32+2,$YMoyGE+2);  // placement du mot 
$pdf->SetFont('Arial','',11);
$pdf->WriteHTML("<b>$noteLitteraire</b>");

$pdf->SetXY($XMoyCL+55+15,$YMoyGE); // cadre scientifique
$pdf->MultiCell(32,10,'',1,'L',0);  
$pdf->SetXY($XMoyCL+55+15+4,$YMoyGE+2); // placement du mot
$pdf->SetFont('Arial','',9);
$pdf->WriteHTML("<b>SCIENTIFIQUE**</b>");


$pdf->SetXY($XMoyCL+55+15+32,$YMoyGE); // cadre note scientifique
$pdf->MultiCell(15,10,'',1,'L',0);
$pdf->SetXY($XMoyCL+55+15+32+2,$YMoyGE+2); // placement du mot
$pdf->SetFont('Arial','',11);
...
$moyenClasseGen=ereg_replace('\.',',',$moyenClasseGen);
$pdf->SetFont('Arial','',5);
...
$pdf->SetFont('Arial','',6);
$pdf->WriteHTML("* moy.des matières suivantes: ".$liste_litteraire." / ** moy.des matières suivantes: ".$liste_scientifique);
$pdf->SetFont('Arial','',8);
$pdf->SetXY(16,$YduplicaSign +8);//+0
