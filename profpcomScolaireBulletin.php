...
if( count($data) > 0 ) {
	...
	for($i=0;$i<count($data);$i++) {
		...
		print "<br>";
		print "<textarea cols=60 rows=5 name='commentaire_$i' onkeypress=\"compter(this,'$nbcar', this.form.CharRestant)\" >$commentaire</textarea>";
		$nbtexte=strlen($commentaire);
		print "&nbsp;<input type=text name='CharRestant' size=3 disabled='disabled' value='$nbtexte' />";
		print "</td></tr>";
	}
	$nbeleve=count($data);
