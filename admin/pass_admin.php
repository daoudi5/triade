...
if (isset($_POST["create"])) {
	...
	if (( crypt(md5($_POST["saisie_ancien"]),"T2") == $PASSWORD) && ($_POST["saisie_new"] == $_POST["saisie_renew"] ) ) {
		$mp=crypt(md5($_POST["saisie_new"]),"T2");
		$fichier=fopen("../common/lib_acces_inc.php","w");
		$donne="<?php\n";
		$donne.="\$LOGIN=\"administrateur\";\n";
		$donne.="\$PASSWORD=\"$mp\";\n";
		$donne.="?>\n";
		...
		$modif="non";
		$cnx=cnx();
		modifpassedokeos($_POST["saisie_new"]);
...
<tr><td align="right"><font class=T2>Ancien mot de passe</font> :</td><td><input type=password  name=saisie_ancien></td></tr>
