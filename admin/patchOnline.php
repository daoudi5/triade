...
if (eregi("\.\.",$patch_ftp)) { exit; }
if (!eregi("\.zip$",$patch_ftp)) { exit; }
...
if (file_exists($fichier_orig)) {
	$fichier=ereg_replace("\.zip","",$patch_ftp);
	$rep_patch=ereg_replace("\.zip","",$patch_ftp);
...
if (file_exists($fichier_info)) {
	...
	foreach ($lines as $line_num => $line) {
		...
		$repecole=REPECOLE."/";
		$c1=$repecole.$motif;
		if (ereg("$c1",$line)) { $line=trim(ereg_replace("admin/",$motif,$line)); }
		if (!is_dir("../".trim($line))) {
...
if (file_exists($fichier_info)) {
	...
	foreach ($lines as $line_num => $line) {
		if(ereg(":",$line)){
			list($tab0,$tab1)= split (":", $line, 2);
			$tab0=trim($tab0);
			$source="../data/patch/$rep_patch/product/".trim($tab0);
			$dest="../".$tab1;
			$motif=REPADMIN."/";
			if (!ereg("livreor/admin/",$dest)) {
				$dest=trim(ereg_replace("admin/",$motif,$dest));
			}
			if ( !copy("$source","$dest") ) {
				$ok=0;
			}
...
if (file_exists($fichier_sql)) {
	...
	foreach($tab as $value){
		if (trim($value) != "") {
			global $prefixe;
			$value=ereg_replace('PREFIXE',$prefixe,$value);
			$value=ereg_replace('TYPETABLE',$typetable,$value);
