...
//	header('Expires: 0');
	...
	while ($row = mysql_fetch_row($result))
	{
		...
		# Si le contenu de la table est demandé
		...
		/* Si insertion complete demandée, on recupère le nom des champs */
		$reqcomplete = ' (';
			for($i = 0; $i < $nb; $i++)
			{
				$reqcomplete .= '`'.mysql_field_name($requete, $i).'`';
...
if (DBTYPE == "mysql") {
	...
	if (file_exists($fichier)) {
		...
		fclose($fichier);
	?>
		Dernière sauvegarde le : <?php print $donnee ?><br><br><b>Compatible Windows</b><br><br>
	<img src="./image/on1.gif" align='center' width='8' height='8' /> <a href='recup_mysql_base.php?id=base_2'><b>Récupérer la base de données, cliquez ici <--</b></a><br /><br />
...
<table><tr><td>
<font class=T2>Accès à la sauvegarde (Linux)  : </font></td><td>
