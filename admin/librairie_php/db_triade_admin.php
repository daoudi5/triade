...
function execSql($sql) {
	...
	global $ERROR;
        ...
        if(DB::isError($res))
        {
			if (eregi('restobase',$_SERVER['SCRIPT_NAME'])) {
				//Pgclose();
				//return ;
			}
...
function verifpatchinstall($idpatch) {
	...
	global $prefixe;
	$idpatch=MyAddSlashes($idpatch);
	$idpatch=eregi_replace('.zip','',$idpatch);
 	$sql="SELECT idpatch,info FROM ${prefixe}patch WHERE idpatch='$idpatch' ";
    	$res=execSql($sql);
	$data=chargeMat($res);
    	unset($sql);
...
function verif_fichier($repadmin) {
	...
	if (count($data) > 0) {
		for($i=0;$i<count($data);$i++) {
			...
			$fichier=ereg_replace("\./admin","./$repadmin",$fichier);
			...
			if ($val == $sum) {
				...
				if ( (eregi('\.log$',$fichier)) || 
					(eregi('css$',$fichier)) ||  
					(eregi('^.*/common/.*$',$fichier)) ||  
					(eregi('htaccess$',$fichier))  ||
					(eregi('htpasswd$',$fichier))  ||
					(eregi('intra-msn-triade.zip$',$value))  ||
					(eregi('^.*/installation/.*$',$fichier))  ||
					(eregi('^.*/data/.*$',$fichier)) ||
					(eregi('^.*/moodle/.*$',$fichier)) ||
					(eregi('^.*/dokeos/.*$',$fichier)) ||
					(eregi('^.*/pmb/.*$',$fichier)) ||
					(eregi('^.*/cache/.*$',$fichier)) ||
					(eregi('^.*/librairie_js/menudepart.js$',$fichier)) ||
					(eregi('^.*/librairie_js/menuadmin.js$',$fichier)) ||
					(eregi('^.*/librairie_js/menueleve.js$',$fichier)) ||
					(eregi('^.*/librairie_js/menuparent.js$',$fichier)) ||
					(eregi('^.*/librairie_js/menuprof.js$',$fichier)) ||
					(eregi('^.*/librairie_js/menuscolaire.js$',$fichier)) ||
					(eregi('^.*/dump-dist.php$',$fichier)) ||
					(eregi('^.*/robots.txt$',$fichier)) ||
					(eregi('^.*/alert.triade$',$fichier)) ||
					(eregi('^.*/triade.alert$',$fichier)) ||
					(eregi('^.*/lib_error.php$',$fichier)) ||
					(eregi('^.*/conf_error.php$',$fichier)) ||
					(eregi('^.*/log/.*$',$fichier)) ||
					(eregi('^.*/messenger/IntraMessengerClient/options.ini$',$fichier)) ||
					(eregi('^.*/messenger/public/intra-msn-triade.zip$',$fichier)) ||
					(eregi('^.*/livreor/identif/logins.php$',$fichier))
				) {
...
function verifDbb() {
	...
	while ($row = mysql_fetch_row($result))
	{
		...
		if (eregi('statutilisateur',$row[0])) {	continue; }
		$res = mysql_fetch_array($req);
		$res[1]=eregi_replace($prefixe,'',$res[1]);
		$res[1]=eregi_replace("ENGINE=.*$",'',$res[1]);
		$res[1]=eregi_replace("collate [A-Za-z0-9_]+",' ',$res[1]);
		$res[1]=eregi_replace("[ \t]",'',$res[1]);
