...
if(isset($_POST["create"])) {
	...
	$gid=$cgrp[1];
	$mid=$_GET["sMat"];
	if (anneeScolaireViaIdClasse($cid) == $anneeScolaire) { 
...
<br>
&nbsp;&nbsp;&nbsp;<font class='T2'>Année Scolaire : <?php print $anneeScolaire ?></font><br><br>
<table border=0>
<tr><td>	
<form method='post' >
...
<?php 
if (trim($choix_tri_text) != "") { 
	print "<option value='$choix_tri' STYLE=\"color:#000066;background-color:#FCE4BA\">".ucfirst($choix_tri_text)."</option>";
}else{
	print "<option value='' STYLE=\"color:#000066;background-color:#FCE4BA\">".LANGCHOIX."</option>";
}
?>
...
<?php if ($_SESSION["membre"] == "menuadmin") { ?>
	<a href="visu_pdf_admin.php?id=<?php print $fichier?>" target="_blank"><img src="image/commun/print.gif" border=0 align=center></a>
<?php }elseif ($_SESSION["membre"] == "menupersonnel") { ?>
	<a href="visu_pdf_personnel.php?id=<?php print $fichier?>" target="_blank"><img src="image/commun/print.gif" border=0 align=center></a>
...
<?php }else{  ?>
	<a href="visu_pdf_scolaire.php?id=<?php print $fichier?>" target="_blank"><img src="image/commun/print.gif" border=0 align=center></a>
<?php } ?>
...
if (($_SESSION["membre"] == "menuadmin") || ($_SESSION["membre"] == "menupersonnel")) {
	...
	print "<input type='hidden' name='saisie_pers' value='".$_GET['saisie_pers']."' /></form><br><br>";
}	 
?>
<br>
...
if ($HPV[gid]) {
	if ($triEleve == "nomeleve") { ?>
		<tr><td align="center">[ <a href="visunoteadmin.php?sMat=<?php print $mid ?>&sClasseGrp=<?php print $cgrp1 ?>&order=classe "><?php print LANGNNOTE2 ?></a> ]</td></tr>
<?php } ?>

<?php if ($triEleve == "classe") { ?>
	<tr><td align="center">[ <a href="visunoteadmin.php?sMat=<?php print $mid ?>&sClasseGrp=<?php print $cgrp1 ?>&order=nomeleve "><?php print LANGNNOTE3 ?></a> ]</td></tr>
<?php } 
}
...
//	$sql="(SELECT e.elev_id, CONCAT( upper(trim(e.nom)),' ',trim(e.prenom) ) ,e.classe  FROM ${prefixe}eleves e WHERE e.classe='$cid' AND e.compte_inactif != 1 $order  ) UNION ( SELECT e.elev_id,  CONCAT( upper(trim(e.nom)),' ',trim(e.prenom) ) ,e.classe  FROM ${prefixe}eleves e , ${prefixe}eleves_histo h WHERE h.idclasse='$cid' AND e.compte_inactif != 1 AND e.elev_id=h.ideleve  ) $order";
	...
	$sql=" SELECT s.* FROM ( SELECT elev_id,CONCAT(upper(trim(nom)),' ',trim(prenom)),classe FROM ${prefixe}eleves , ${prefixe}classes  WHERE classe='$cid' AND code_class=classe  AND annee_scolaire='$anneeScolaire' UNION ALL SELECT e.elev_id,CONCAT( upper(trim(e.nom)),' ',trim(e.prenom) ),e.classe FROM ${prefixe}eleves e ,${prefixe}classes c, ${prefixe}eleves_histo h WHERE h.idclasse='$cid' AND e.elev_id=h.ideleve AND h.idclasse=c.code_class AND h.annee_scolaire='$anneeScolaire') s  ORDER BY 2";
...
//$worksheet1->set_selection('A0');
	...
	$texte="Note des devoirs de : $libel ";
	...
	for($i=0;$i<count($mat);$i++){
		...
		$h=0;
		if (($triEleve == "classe") && ($table == "eleves"))  {
			...
			if ($idClasseA != $afficheClasse) {
				...
				$affi=ereg_replace(' ','&nbsp;',$affi);
				print "<tr><td colspan=2 ><font class='T2' color='blue' >&nbsp;Classe&nbsp;:&nbsp;<b>".$affi."</b></font></td></tr>";
				$pdf->SetXY($xcoor,$ycoor);
		...
		if ((VIESCOLAIRENOTEENSEIGNANT == "oui") || ($_SESSION["membre"] != "menupersonnel")) {
			$data_note=recherche_note_pour_scolaire($mat[$i][0],$sql2,$mid,$_SESSION["id_pers"]);
		}else{
			$data_note=recherche_note_pour_admin($mat[$i][0],$sql2,$mid,$_SESSION["id_pers"]);
		}
		...
		for($t=0;$t<count($data_note);$t++){
			...
			if ($okmoy == 1) {
				...
				if (($sujetaff == 0) && ($moyenneeleve > 0)) {
					$pdf->SetFont('Arial','',9);
					$xsujet3=$xsujet2+10;
					$pdf->TextWithRotation($xsujet3,$ysujet,"Moyenne",45,-45);
					$xsujet4=$xsujet3+4;
					$pdf->TextWithRotation($xsujet4,$ysujet,"Elève",45,-45);

					$worksheet1->write(2, $yxls+=1 ,"Moyenne", $center);
...
$pdf->SetXY(5,$ycoor+5);
$moyenneclasse="Moyenne de classe : ".number_format($moyenClasse,2,'.','')." (Max : $moyenmax) (Min : $moyenmin)";
$pdf->MultiCell(90,5,$moyenneclasse,0,'',0);


print "<script>document.getElementById('MoyenClasse').innerHTML='<font class=\'T2\'>&nbsp;&nbsp;Moyenne de classe : <font color=\'blue\'>";
print number_format($moyenClasse,2,'.','')."</font>";
print " <font class=\'T1\'>&nbsp;(Max : $moyenmax) (Min : $moyenmin)</font>";
print "'</script>";
