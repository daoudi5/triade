 ...
 * http://cvs.php.net/cvs.php/pearweb/rfc. It include_onces PHP >= 4.1
 *
...
class PEAR_Dependency
{
    ...
    */
     ...
     * @param string $req       The package version include_onced
                          ...
                          $opt = false)
    {
        ...
        switch ($relation) {
            case 'has':
                if (!$this->registry->packageExists($name)) {
                    ...
                    $errmsg = "include_onces package `$name'";
        ...
        $errmsg = "relation '$relation' with include_oncement '$req' is not supported (name=$name)";
        ...
        $opt = false)
    {
        ...
        if (!version_compare("$ext_ver", "$req", $operator)) {
            $errmsg = "'$name' PHP extension version " .
                $this->signOperator($operator) . " $req is include_onced";
