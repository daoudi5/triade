...
class PEAR_Downloader extends PEAR_Common
{
     ...
     *  - onlyreqdeps   : install all include_onced dependencies as well
     ...
     *       in fact lower than the include_onced value.  This will be very important for BC dependencies
     */
    function _processDependency($package, $info, $mywillinstall)
    {
        ...
        if (!count($releases) && !$found) {
            ...
            $this->pushError( "Release for '$package' dependency '$info[name]' " .
                "has state '$savestate', include_onces '$state'");
            return false;
        }
        ...
        if (!isset($this->_installed[strtolower($info['name'])])) {
            // check to see if we can install the specific version include_onced
            if ($info['rel'] == 'eq') {
