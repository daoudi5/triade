...
$affAbsRtd=$_POST["affAbsRtd"];
config_param_ajout($affAbsRtd,"affAbsRtd");
$affrang=$_POST["affrang"];
config_param_ajout($affrang,"affrang");
...
$infoeleveclasse=preg_replace('/_/',' ',$infoeleveclasse);
...
$signature="La Direction : $directeur";
// FIN variables
...
if (file_exists("./data/image_pers/logo_bulletin.jpeg")) {
	$logo="./data/image_pers/logo_bulletin.jpeg";
	$pdf->Image($logo,$xlogo,$ylogo,$logowidth);
}
...
//FIN AB
	...
	$examaff=moyenneEleveMatiereExamen($idEleve,$ordre[$i][0],$dateDebut,$dateFin,$idprof,"Partiel Blanc");
	$pdf->SetFont('Arial','',8);
...
if (($moyenEleveExam < 10) && (trim($moyenEleveExam) != "")) { $moyenEleveExam="0".$moyenEleveExam; }
...
if ($affrang == "oui") {
	...
	$pdf->WriteHTML("<B> Rang $INTITULEELEVE : $rangG / $rangGT </B>");
	$pdf->SetTextColor(0,0,0);
}
//
...
//FIN
//PARTIE OBSERVATION
$leap=recherchepigierparisv2($idEleve,"pigierparisv2",$_POST["saisie_trimestre"],$anneeScolaire);
// ppv2_av,ppv2_faible,ppv2_passable,ppv2_enc,ppv2_feli
if ($leap[0][4] == "1") { $checkedmont1="1"; }else{ $checkedmont1=""; }
if ($leap[0][3] == "1") { $checkedmont2="1"; }else{ $checkedmont2=""; }
if ($leap[0][2] == "1") { $checkedmont3="1"; }else{ $checkedmont3=""; }
if ($leap[0][1] == "1") { $checkedmont4="1"; }else{ $checkedmont4=""; }
if ($leap[0][0] == "1") { $checkedmont5="1"; }else{ $checkedmont5=""; }
...
$pdf->SetXY($XX,$YcomP3 + 11);
$pdf->MultiCell(3,3,"",1,'L',$checkedmont5);
...
$commentairedirection=recherche_com($idEleve,$_POST["saisie_trimestre"],"pigierparisv2");
