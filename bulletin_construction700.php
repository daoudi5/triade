...
for($j=$dep;$j<$nbEleveT;$j++) {  // premiere ligne de la creation PDF
	...
	for($i=0;$i<count($ordre);$i++) {
		...
		$moyenExam=moyenneEleveMatiereExamen($idEleve,$idMatiere,$dateDebut,$dateFin,$idprof,"ISP");
		$pdf->SetFont('Arial','',12);
		$pdf->SetXY($XnotVal+15,$YnotVal);
		$moyenExam1=$moyenExam;
		if ($moyenExam1 != "") {
			$moyenExamTotal+=$moyenExam1;
			$nbExamNote++;
		}
		if (($moyenExam1 < 10) && ($moyenExam1 != "")) { $moyenExam1="0".$moyenExam1; }
		$pdf->WriteHTML($moyenExam1);
	...
	$pdf->SetXY($XmoyMatGenMinVal+15,$YmoyMatGVal);
	...
	$pdf->SetXY($XmoyMatGenMaxVal+15,$YmoyMatGVal);
...
if ($affichemoyengeneral == "oui") {
	...
	if ($nbExamNote > 0) {
		$moyenExamTotal=$moyenExamTotal/$nbExamNote;
		if (($moyenExamTotal < 10) && ($moyenExamTotal != "")) { $moyenExamTotal="0".$moyenExamTotal; }
		$moyenExamTotal=number_format($moyenExamTotal,'2',',','');
		$pdf->SetFont('Arial','',10);
		$pdf->SetXY($XmoyClasseGValue+2,$YmoyClasseGValue);
		$pdf->WriteHTML($moyenExamTotal);
	}
