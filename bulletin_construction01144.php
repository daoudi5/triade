...
if (file_exists("./image/banniere/banniere-agr.jpg")) {
	$logo="./image/banniere/banniere-agr.jpg";
...
$pdf->WriteHTML("Diplôme Certifié Niveau II");
$pdf->SetFont('Arial','',8);
...
// elev_id,nomtuteur, prenomtuteur, adr1, code_post_adr1, commune_adr1, adr2, code_post_adr2, commune_adr2, numero_eleve, class_ant, date_naissance, regime, civ_1, civ_2,nom,prenom,nom_resp_2,prenom_resp_2,lieu_naissance
		...
		$pdf->MultiCell(90,3,"$chaine",0,'L',0);
                ...
                $chaine=trim($num_adr1)." ".ucwords(trim(strtolower($adr1)));
...
$pdf->SetXY($X+17,$Y+11); // placement du prenom de l'eleve
...
$pdf->SetTextColor(0,0,0);
$pdf->SetFillColor(230,230,255);
$pdf->SetXY(50,$Y+15); //  placement  cadre titre
$pdf->MultiCell(158,7,'',1,'C',1);
$pdf->SetXY(52,$Y+16); // placement contenu titre
$pdf->WriteHTML($titrenote1);
if ($affmoyclasse == "oui") {
	$pdf->SetX(98+20);
	$pdf->WriteHTML("Sujets et notes des devoirs");
	$pdf->SetX(169);
	$pdf->WriteHTML("Moy. Elève");
	$pdf->SetXY(187.5,$Y+24);
	$pdf->MultiCell(20,3,"Moy. Classe",0,'C',0);
}else{
        $pdf->SetX(100+30);
        $pdf->WriteHTML("Sujets et notes des devoirs");
        $pdf->SetXY(187.5,$Y+17);
...
// fin des titres
	...
	$appreciationretard="ASSIDUITE : $nbretard retard(s) / $nbabsNJ demi-journée d'absence(s) non justifiée(s) / $nbabs demi-journée d'absence(s) justifié(s) " ;
	unset($nbabsNJ);
	unset($nbabs);
	unset($nbretard);
	//---------------------------------//
...
//	print $datasousmatiere;
	...
	for($b=0;$b<count($note);$b++) {
		...
		if ($ordre[$i][0] == $note[$b][2]) {
			...
			//	$noteaff=ereg_replace(".50",".5",$noteaff);
...
$pdf->MultiCell(158,35,'',1,'C',0);
$pdf->SetXY($Xmat+1,$Ymat+3);
$pdf->MultiCell(158,4,"$appreciationretard",0,'L',0);
$pdf->SetXY($Xmat+1,$Ymat+9+3);
$pdf->WriteHTML("Observations générales : ");
...
$pdf->MultiCell(150,$confPolice[1],$commentairedirection,'','','L',0); // commentaire de la direction (visa)
