...
$data_2=affAbsNonJustif22Limit($_GET["filtre"],$_GET["depart"],$_GET["nbaff"]);
	...
	for($j=0;$j<count($data_2);$j++) {
			...
			$classe=ereg_replace(" ","&nbsp;",$classe[0][1]);
			$dureee=$data_2[$j][4]." J";
			if ($data_2[$j][4] == -1) { $dureee=preg_replace('/\./','H',$data_2[$j][7]) ; }
?>
	<TR>
	<td id=bordure valign="top" bgcolor='#CCCCCC' >&nbsp;<?php print strtoupper(recherche_eleve_nom($ideleve))." ".ucwords(strtolower(recherche_eleve_prenom($ideleve))) ?></td>
	<td id=bordure valign="top"  bgcolor="#A0A0A0" >&nbsp;<?php print ucwords($classe) ?>&nbsp;</td>
	...
	<td bgcolor="#CCCCCC"  valign="top"  id=bordure>
	...
	<td bgcolor="#A0A0A0"  valign="top" align=center id=bordure >
	<?php print dateForm($data_2[$j][1])?>
	</td>

	<td bgcolor="#CCCCCC" valign="top"  align=left id=bordure >
	<?php list($cre,$dC,$fC)=split("#",$data_2[$j][12]);
