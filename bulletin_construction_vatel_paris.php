...
//			if ($notepartiel[$nb_note][0]> -1  && $notepartiel[$nb_note][0]!='' ||  $notepartiel[$nb_note][0]==0  ) {
			if (($notepartiel[$nb_note][0]>=0 ) && ($notepartiel[$nb_note][0] != '')) {
				$nb_mat+=1;
...
//	$pdf->MultiCell(25,10,"$Trimestre Année scolaire $anneeScolaire",1,'C',0);
	...
	$pdf->MultiCell(25,24,"",1,'C',0);
			...
			if ($Trimestre  == "1er  SEMESTRE") $Trimestre="1ère Période" ;
			if ($Trimestre  == "2eme SEMESTRE") $Trimestre="2ème Période" ;
...
// premiere ligne
	...
	$pdf->MultiCell(40,6,'PARTIELS',1,'C',1);
	$pdf->SetFillColor(255);  // fond blanc
	$Xmat1=$Xmat+$largeurMat+40;
	$pdf->SetXY($Xmat1,$Ymat);
	$pdf->MultiCell(100,6,'CONTROLE CONTINU',1,'C',0);
...
// deuxieme ligne
	...
	$pdf->SetXY($Xmat1,$Ymat);
	$pdf->MultiCell(20,6,'Note',1,'C',1);
//	$Xmat1=$Xmat1+20;
	//$pdf->SetXY($Xmat1,$Ymat);
//	$pdf->MultiCell(20,6,'Coef',1,'C',0);
	$Xmat1=$Xmat1+20;
	$pdf->SetXY($Xmat1,$Ymat);
	$pdf->MultiCell(80,6,'Notes ( Coef. 1 )',1,'C',0);
	$Xmat1=$Xmat1+80;
	$pdf->SetXY($Xmat1,$Ymat);
...
for ($nb_ue=0;$nb_ue<count($ue);$nb_ue++) { // boucle pour chaque UE
	...
	//		if ($noteperiode[0][0]!=0 && $noteperiode[0][0]!='') {$nb_mat+=1;}
		...
		if ($ecrit!='') {
			...
			$pdf->MultiCell(40,6,''.format_moyenne($moyenne_ue).' '.$somme_coef.'',1,'C',1);
...
//---------------------------------------------------------------------
		...
		for ($nb_note=0;$nb_note<count($notepartiel);$nb_note++) { 
			if ($notepartiel[$nb_note][0]>=0 && is_numeric($notepartiel[$nb_note][0])) {
				...
				$somme_coef+=$notepartiel[$nb_note][6];
				$som_coef_partiel_eu+=$notepartiel[$nb_note][6];
 // print  "$matiere  $somme_coef ".$notepartiel[$nb_note][6]."<br>";
				$affich_coef_partiel =$notepartiel[$nb_note][6];
			} else {
				$affich_coef_partiel="";
			}
		...
		$pdf->SetFont('Arial','',7);
		$pdf->SetXY($Xmat+$largeurMat,$Ymat);
		if (($appliPratique == "oui") && ($moy_coef_partiel == "") && (trim($matiereVerif) == "APPLICATIONS PRATIQUES") ) {
			$moy_coef_partiel=recupCoefPartielApplicationPratique($idClasse,$_POST["saisie_trimestre"],$idMatiere);
...
// FIN Mise en place des moyenens general partiel et periode
	...
	$pdf->WriteHTML("<b>Absences période :               - Absences total cursus :               - niveau d'avertissement / 3</b>");
	//---------------------------------//
