...
for($j=$dep;$j<$nbEleveT;$j++) {  // premiere ligne de la creation PDF
	...
	$pdf->SetFont('Arial','',8);
	$pdf->SetXY($xcoor0,$ycoor0+5+5);
	$pdf->WriteHTML($coordonne1);
	$pdf->SetXY($xcoor0,$ycoor0+10+5);
	$pdf->WriteHTML($coordonne2);
	$pdf->SetXY($xcoor0,$ycoor0+15+5);
	$pdf->WriteHTML($coordonne3);
	$pdf->SetXY($xcoor0,$ycoor0+20+5);
	...
	$pdf->SetXY(15,43); // placement du cadre du nom de l eleve
	$pdf->MultiCell(77,31,'',1,'L',1);
	...
	$pdf->SetXY($Xv1,45); // placement du nom de l'eleve
	...
	$prenomtuteur=ucwords($dataadresse[0][2]);
	$contact="$civ $nomtuteur $prenomtuteur";
	$adr1=preg_replace('/,/',"\n",$dataadresse[0][3]);
	if (strlen($dataadresse[0][4]) != 5) { 
		$code_post_adr1="0".$dataadresse[0][4];
	}else{
		$code_post_adr1=$dataadresse[0][4];
	}
	$commune_adr1=ucwords($dataadresse[0][5]);
		


	$civ2=civ($dataadresse[0][14]);
	$nomtuteur2=strtoupper($dataadresse[0][17]);
	$prenomtuteur2=ucwords($dataadresse[0][18]);
	$contact2="$civ2 $nomtuteur2 $prenomtuteur2";
	$adr2=preg_replace('/,/',"\n",$dataadresse[0][6]);
	if (strlen($dataadresse[0][7]) != 5) { 
		$code_post_adr2="0".$dataadresse[0][7];
	}else{
		$code_post_adr2=$dataadresse[0][7];
	}
	$commune_adr2=ucwords($dataadresse[0][8]);

	if ($divorceAFaire == 1) {
		$contact=$contact2;
		$adr1=$adr2;
		$code_post_adr1=$code_post_adr2;
		$commune_adr1=$commune_adr2;		
		$divorceAFaire=0;
	}else{
		if (	(trim($nomtuteur2) != "") && 
			(trim($commune_adr2) != "") &&
...
//		$pdf->RoundedRect(94,40, 100, 40, 4.5, 'DF');
		$pdf->SetXY(87,26);
                $pdf->MultiCell(100,32,"",0,'L',0);
		$pdf->SetFont('Arial','',11);
		$pdf->SetXY(104,45);
//		print $contact."<br>";
		$adr1=trim($adr1);
		$code_post_adr1=trim($code_post_adr1);
	        $commune_adr1=trim($commune_adr1);
                $pdf->MultiCell(205,4,"$contact\n\n$adr1\n$code_post_adr1 $commune_adr1",0,'L',0);
	...
	$pdf->SetFont('Arial','',9);
	$pdf->SetFillColor(220);
	$pdf->SetXY(15,60+$nb); //  placement  cadre titre
	$pdf->MultiCell(184,11,'',1,'C',1);
	$pdf->SetXY(25,62+$nb); // placement contenu titre
...
$pdf->WriteHTML("Le président du conseil");
$pdf->SetFont('Arial','',5);
