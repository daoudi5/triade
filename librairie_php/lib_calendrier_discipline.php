...
for($i=0;$i<count($data);$i++)
{
	...
	if ($annee == $date_recup_annee) {
		$nomprenomeleve=recherche_eleve($data[$i][0]);
		$nomprenomeleve=ereg_replace('"',"&quot;",$nomprenomeleve);
		$nomprenomeleve=ereg_replace('\'','\\\\\'',$nomprenomeleve);
                $passe=1;
 		if ($date_deja == $date_recup_jour_mois ) {
                        $dstT.="<BR><B>".trim($nomprenomeleve)."</B><font size=1> à ".timeForm($data[$i][2])." durée ".timeForm($data[$i][10])."</font>";
                }else{
                        if ($txt_fini == 1) {
                                $dstT.="\"";
                        }
                        ...
                        $dstT.=",\"<B>".trim($nomprenomeleve)."</B><font size=1> à ".timeForm($data[$i][2])." durée ".timeForm($data[$i][10])."</font>";
                        $txt_fini=1;
                }
       }
}
...
print <<<EOF
        ...
        for(var i=0;i<6;i++) {
                ...
                for (j=0;j<7;j++) {
					...
					if ( j == 2 ) {
						disp("<TD $Ferieap $FerieMatin width='10' bgcolor='#FFFFFF'  align='center'><FONT face='Arial' size='-1' color='#0000CC'>"+aff_j+"</font></TD>");
					}else{
						if (j == 5) 
					           disp("<TD $FerieSaap $FerieSaMatin width='10' bgcolor='#FFFFFF'  align='center'><FONT face='Arial' size='-1' color='#0000CC'>"+aff_j+"</font></TD>"); 
						else
		                                    disp("<TD width='10' bgcolor='#FFFFFF' align='center'><FONT face='Arial' size='-1' color='#0000CC'>"+aff_j+"</FONT></TD>");
