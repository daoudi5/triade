	...
	$nomsalle=$data2[0][1];
		...
		if (!ereg("h",$data[$i][5])) { $data[$i][5]="0${heure}h00 mn"; }
 		if ($date_deja == $date_recup_jour_mois ) {
                        $dstT.="<hr>";
                        $dstT.= LANGPER25.": ".trim($data[$i][3])."<BR>".LANGPER17.": ".trim($data[$i][2]). " à ".timeForm($data[$i][4])." (".$data[$i][5].")" ;
			$dstT.="<br>";
	  ...
	  var d_jour=new Date();
        ...
        for(var i=0;i<6;i++) {
                ...
                for (j=0;j<7;j++) {
					...
					if (j==2) {
 						disp("<TD $Ferieap $FerieMatin width='10' bgcolor='#FFFFFF' align='center'><FONT face='Arial' size='-1' color='#000000'>"+aff_j+"</FONT></TD>");
					}else{

						if (j == 5) 
 							disp("<TD $FerieSaap $FerieSaMatin width='10' bgcolor='#FFFFFF' align='center'><FONT face='Arial' size='-1' color='#000000'>"+aff_j+"</FONT></TD>");

						else
	                                    disp("<TD width='10' bgcolor='#FFFFFF' align='center'><FONT face='Arial' size='-1' color='#000000'>"+aff_j+"</FONT></TD>");
					}
				}
                        }
                        else
