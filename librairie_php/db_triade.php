	...
	if(DB::isError($res)) {
		...
		if (file_exists("./data/parametrage/analyse.triade")) {  // activation du l'analyse de triade 
			...
			$fichier=fopen("./data/parametrage/analyse.log","a");
       	    fwrite($fichier,"- ".dateDMY()." ‡ ".dateHIS()."ERROR : ".$_SERVER['PHP_SELF']." -> ".$sql."\n");
       	    fclose($fichier);
		}
		if (file_exists("../data/parametrage/analyse.triade")) {  // activation du l'analyse de triade 
			turnOverLog("../data/parametrage/analyse.log",100000000); // 100Mo
		...
		if (file_exists("./data/parametrage/analyse.triade")) { // activation du l'analyse de triade 
			turnOverLog("./data/parametrage/analyse.log",100000000); // 100Mo
                        $fichier=fopen("./data/parametrage/analyse.log","a");
                        fwrite($fichier,"- ".dateDMY()." ‡ ".dateHIS()." -- ".$_SERVER['PHP_SELF']." -- ".$sql."\n");
                        fclose($fichier);
                }
		if (file_exists("../data/parametrage/analyse.triade")) { // activation du l'analyse de triade 
			turnOverLog("../data/parametrage/analyse.log",100000000); // 100Mo
 ...
 * PostÈ par Jerome Neuveglise
 * Cette fonction permet de coder un texte afin que les caractËres spÈciaux soient
 * lisibles sur un Ècran de portable WAP. Par exemple, un È doit se voit coder en
 * &#233;
*/
 ...
 * PostÈ par Ben
 * Si vous voulez donner un peu plus de fonctionnalitÈs ‡ votre moteur de
 * recherche, proposez aux utilisateurs la mÈthode approximative.
 * Il suffit de transformer chaque mot ‡ chercher de la maniËre suivante :
 * "phpinfo" devient "p%h%p%i%n%f%o".
 * Donc votre requÍte devient :
...
function accent_import($chaine)	{
	...
	$chaine=str_replace("È","[eacute]",$chaine);
	$chaine=str_replace("Ë","[egrave]",$chaine);
	$chaine=str_replace("˘","[ugrave]",$chaine);
	$chaine=str_replace("‚","[acirc]",$chaine);
	$chaine=str_replace("Í","[ecirc]",$chaine);
	$chaine=str_replace("Ó","[icirc]",$chaine);
	$chaine=str_replace("Ù","[ocirc]",$chaine);
	$chaine=str_replace("˚","[ucirc]",$chaine);
	$chaine=str_replace("‰","[auml]",$chaine);
	$chaine=str_replace("Î","[euml]",$chaine);
	$chaine=str_replace("Ô","[iuml]",$chaine);
	$chaine=str_replace("ˆ","[ouml]",$chaine);
	...
	$chaine=str_replace('í',"[Quote2]",$chaine);
	$chaine=str_replace('Á',"[ccdile]",$chaine);

	
	$chaine=str_replace('Û',"[ocute]",$chaine);
	$chaine=str_replace('·',"[acute]",$chaine);
	$chaine=str_replace('Ò',"[ntild]",$chaine);
	$chaine=str_replace('Ì',"[icute]",$chaine);
	$chaine=str_replace('˙',"[ucute]",$chaine);
	$chaine=str_replace('—',"[Ntild]",$chaine);
...
function accent_export($chaine)	{
	...
	$chaine=str_replace("[egrave]","Ë",$chaine);
	$chaine=str_replace("[ugrave]","˘",$chaine);
	$chaine=str_replace("[acirc]","‚",$chaine);
	$chaine=str_replace("[ecirc]","Í",$chaine);
	$chaine=str_replace("[icirc]","Ó",$chaine);
	$chaine=str_replace("[ocirc]","Ù",$chaine);
	$chaine=str_replace("[ucirc]","˚",$chaine);
	$chaine=str_replace("[auml]","‰",$chaine);
	$chaine=str_replace("[euml]","Î",$chaine);
	$chaine=str_replace("[iuml]","Ô",$chaine);
	$chaine=str_replace("[ouml]","ˆ",$chaine);
	$chaine=str_replace("[uuml]","¸",$chaine);
	$chaine=str_replace("[Egrave]","»",$chaine);
	$chaine=str_replace("[Eacute]","…",$chaine);
	$chaine=str_replace("[Ecirc]"," ",$chaine);
	...
	$chaine=str_replace("[ByQuote]","`",$chaine);
	$chaine=str_replace("[DblQuote]",'"',$chaine);
	$chaine=str_replace("[Quote2]",'í',$chaine);
	$chaine=str_replace("[ccdile]",'Á',$chaine);
	$chaine=str_replace("[ocute]",'Û',$chaine);
	$chaine=str_replace("[acute]",'·',$chaine);
	$chaine=str_replace("[ntild]",'Ò',$chaine);
	$chaine=str_replace("[icute]",'Ì',$chaine);
	$chaine=str_replace("[ucute]",'˙',$chaine);
	$chaine=str_replace("[Ntild]",'—',$chaine);
	$chaine=str_replace("[stdin]",'<',$chaine);
	$chaine=str_replace("[stdout]",'>',$chaine);
...
function txt_vers_html($chaine)	{
	...
	$chaine=str_replace(" ","&Ecirc;",$chaine);
	$chaine=str_replace("«","&Ccedil;",$chaine);
	$chaine=str_replace("Á","&ccedil;",$chaine);
	$chaine=str_replace(" ","&nbsp;",$chaine);
	return $chaine;
}
...
function html_vers_text($chaine)	{
	...
	$chaine=str_replace("&uuml;","¸",$chaine);
	$chaine=str_replace("&nbsp;"," ",$chaine);
	$chaine=str_replace("&ccedil;","Á",$chaine); 
...
// chargement du rÈsultat d une requÍte SQL dans une matrice (tableau bi-dimensionnel)
...
// chargement du rÈsultat d une requÍte SQL dans une matrice (tableau)
...
function create_personnel_via_admin($nom,$pren,$mdp,$tp,$civ,$pren2='') {
	...
	if (SECURITE == 3) {
		if ( (strlen($mdp) < 8) || (!ereg('[a-z]',$mdp)) || (!ereg('[A-Z]',$mdp)) || (!ereg('[0-9]',$mdp)) ) {
			return false ;
		}
	}

	if (SECURITE == 2) {
		if ( (strlen($mdp) < 8) || (!ereg('[a-z]',$mdp)) || (!ereg('[0-9]',$mdp)) ) {
			return false ;
		}
	}
...
//-------------------------------------------------------------------------//
function create_devoirscolaire($idclsorgrp,$idmatiere,$date,$pourle,$travail,$clsorgrp,$number,$idprof,$tempsestime) {
    	global $cnx;
	global $prefixe;
	$heure=dateHIS();
	...
	$data=chargeMat($res);
	if (count($data) > 0) {
		$sql="UPDATE ${prefixe}devoir_scolaire SET date_saisie='$date',  heure_saisie='$heure', texte='$travail', tempsestimedevoir='$tempsestime' , number='$number' WHERE id_class_or_grp='$idclsorgrp' AND date_devoir='$pourle' AND matiere_id='$idmatiere' AND classorgrp='$clsorgrp' AND idprof='$idprof'";	
		return(execSql($sql));		
	}else{
		delete_devoirscolaire($pourle,$clsorgrp,$idmatiere,$idclsorgrp,$idprof);
		$date=dateDMY2();
		$sql="INSERT INTO ${prefixe}devoir_scolaire(id_class_or_grp,matiere_id,date_saisie,heure_saisie,date_devoir,texte,classorgrp,number,idprof,tempsestimedevoir)VALUES ('$idclsorgrp','$idmatiere','$date','$heure','$pourle','$travail','$clsorgrp','$number','$idprof','$tempsestime')";
	        return(execSql($sql));
	}
}

function create_cahiertexteContenu($idclsorgrp,$idmatiere,$date,$datecontenu,$clsorgrp,$number,$idprof,$contenu) {
    	global $cnx;
	global $prefixe;
	$heure=dateHIS();
	$date=dateDMY2();
	...
	$data=chargeMat($res);
	if (count($data) > 0) {
		$sql="UPDATE ${prefixe}cahiertexte SET date_saisie='$date',  heure_saisie='$heure', contenu='$contenu' ,number='$number' WHERE id_class_or_grp='$idclsorgrp' AND date_contenu='$datecontenu' AND matiere_id='$idmatiere' AND classorgrp='$clsorgrp' AND idprof='$idprof'";
		return(execSql($sql));	
	}else{
		delete_cahiertexte($datecontenu,$clsorgrp,$idmatiere,$idclsorgrp,$idprof);
		$date=dateDMY2();
		$sql="INSERT INTO ${prefixe}cahiertexte(id_class_or_grp,matiere_id,date_saisie,heure_saisie,date_contenu,classorgrp,number,idprof,contenu) VALUES ('$idclsorgrp','$idmatiere','$date','$heure','$datecontenu','$clsorgrp','$number','$idprof','$contenu')";
	        return(execSql($sql));
	}
}
...
function create_cahiertexteObjectif($idclsorgrp,$idmatiere,$date,$datecontenu,$clsorgrp,$number,$idprof,$objectif) {
    	global $cnx;
	global $prefixe;
	$heure=dateHIS();
	$date=dateDMY2();
	...
	if (count($data) > 0) {
		$sql="UPDATE ${prefixe}cahiertexte SET date_saisie='$date',  heure_saisie='$heure', objectif='$objectif' $sqlSuite WHERE id_class_or_grp='$idclsorgrp' AND date_contenu='$datecontenu' AND matiere_id='$idmatiere' AND classorgrp='$clsorgrp' AND idprof='$idprof'";
		return(execSql($sql));	
	}else{
		delete_cahiertexte($datecontenu,$clsorgrp,$idmatiere,$idclsorgrp,$idprof);
		$sql="INSERT INTO ${prefixe}cahiertexte(id_class_or_grp,matiere_id,date_saisie,heure_saisie,date_contenu,classorgrp,number_obj,idprof,objectif) VALUES ('$idclsorgrp','$idmatiere','$date','$heure','$datecontenu','$clsorgrp','$number','$idprof','$objectif')";
	        return(execSql($sql));
	}
}
...
function recherche_devoir_scolaire_2($date_contenu,$clsorgrp,$idmatiere,$idclsorgrp,$idprof) {
	...
	$sql="SELECT id,id_class_or_grp,matiere_id,date_saisie,heure_saisie,date_devoir,texte,classorgrp,number,idprof,tempsestimedevoir  FROM ${prefixe}devoir_scolaire WHERE id_class_or_grp='$idclsorgrp' AND matiere_id='$idmatiere' AND date_devoir='$date_contenu' AND classorgrp='$clsorgrp' AND idprof='$idprof' ";
	$res=execSql($sql);
	$data=chargeMat($res);
	return $data;
}
...
function recherche_contenu_scolaire_($date_contenu,$clsorgrp,$idmatiere,$idclsorgrp,$idprof) {
	...
	global $prefixe;
	$date_contenu=dateFormBase($date_contenu);
	$sql="SELECT id,id_class_or_grp,matiere_id,date_saisie,heure_saisie,classorgrp,number,contenu,objectif,date_contenu,idprof,number_obj,blocnote FROM ${prefixe}cahiertexte WHERE id_class_or_grp='$idclsorgrp' AND matiere_id='$idmatiere' AND date_contenu='$date_contenu'  AND classorgrp='$clsorgrp' AND idprof='$idprof' ";
	$res=execSql($sql);
	$data=chargeMat($res);
	return $data;
}
...
function create_discipline_retenue($id_eleve,$date_retenue,$heure_retenue,$date,$user,$sanction,$qui,$motif,$duree_retenue,$devoir=NULL,$description_fait,$courrierparent=NULL,$daterepport=NULL) {
	...
	$message="Retenue pour $qui <br> Motif : $motif <br> Description : $description_fait <br> Sanction : $sanction <br> Retenue le ".dateForm($date_retenue)." ‡ ".timeForm($heure_retenue);
...
function create_discipline_sanction($id_eleve,$motif,$sanction,$date,$user,$qui,$devoir,$description_fait) {
	...
	if ($qui == "") $attribue="non indiquÈ"; 
	$message="Sanction pour l'ÈlËve : $nomeleve $prenomeleve <br>Sanction : $motif<br>AttribuÈ par : $qui<br>Description : $description_fait<br>CatÈgorie : $libellesanction <br>Devoir ‡ faire : $devoir";
	for($i=0;$i<count($data);$i++) {
		$destinataire=$data[$i][0];
		envoi_messagerie($_SESSION["id_pers"],$destinataire,$objet,Crypte($message,$number),$date,$heure,$type_personne,'ENS',$number,'',0);
	}
...
// function pour le passage de la table elevessansclasse ‡ eleves
...
function create_update_eleve_scolnet($nomE,$prenomE,$datenaissance,$params,$ascii,$champsvide,$affectpasswd) {
	...
	if (ereg("/",$datenaissance)) { $datenaissance=dateFormBase($datenaissance); }
...
function purge_rep_membre($path) {
	$O = dir($path);
    	...
    	// !!!! il faut bien appeler 2 fois la mÈthode close() !!!
    	$O -> close();
    	$O -> close();
	rmdir($path);
}
...
function create_suppleant($nom,$pren,$mdp,$remp,$date_entree,$date_sortie,$civ,$pren2='') {
        global $prefixe;
	// cette fonction nÈcessite une transaction
	// pour Ítre fiable
...
function affRetarddujour3($date) {
        global $cnx;
	global $prefixe;
	if (ereg('/',$date)) { $date=dateFormBase($date); }
...
function affdevoirScolaireParent($idClasse,$date,$ref) {
	...
	global $prefixe;
	$date=dateFormBase($date);
	$sql="SELECT id_class_or_grp, matiere_id, date_saisie, heure_saisie, date_devoir, texte, classorgrp, id, number, idprof,tempsestimedevoir FROM ${prefixe}devoir_scolaire WHERE id_class_or_grp='$idClasse' AND $ref='$date'  ORDER BY date_devoir,heure_saisie ";
	$res=execSql($sql);
	$data_2=chargeMat($res);
	return $data_2;
}
...
function affdevoirScolaireProf($idProf,$date,$ref,$classorgrp) {
	...
	global $prefixe;
	$date=dateFormBase($date);
	$sql="SELECT id_class_or_grp, matiere_id, date_saisie, heure_saisie, date_devoir, texte, classorgrp, id, number,idprof,tempsestimedevoir FROM ${prefixe}devoir_scolaire WHERE idprof='$idProf' AND $ref='$date' AND classorgrp='$classorgrp'  ORDER BY date_devoir,heure_saisie ";
	$res=execSql($sql);
	$data_2=chargeMat($res);
	return $data_2;
}
...
function affcontenuScolaireProf($idProf,$date,$ref,$classorgrp) {
	...
	global $prefixe;
	$date=dateFormBase($date);
	$sql="SELECT id_class_or_grp, matiere_id, date_saisie, heure_saisie, date_contenu, contenu, classorgrp, id, number, idprof FROM ${prefixe}cahiertexte WHERE idprof='$idProf' AND $ref='$date'  AND classorgrp='$classorgrp'  ORDER BY date_contenu,heure_saisie ";
	$res=execSql($sql);
	$data_2=chargeMat($res);
	return $data_2;
}
...
function affobjectifScolaireProf($idProf,$date,$ref,$classorgrp) {
	...
	global $prefixe;
	$date=dateFormBase($date);
	$sql="SELECT id_class_or_grp, matiere_id, date_saisie, heure_saisie, date_contenu, objectif, classorgrp, id, number, idprof FROM ${prefixe}cahiertexte WHERE idprof='$idProf' AND $ref='$date'  AND classorgrp='$classorgrp'  ORDER BY date_contenu,heure_saisie ";
	$res=execSql($sql);
	$data_2=chargeMat($res);
	return $data_2;
}
...
function affcontenuScolaireParent($idClasse,$date,$ref) {
	...
	global $prefixe;
	$date=dateFormBase($date);
	$sql="SELECT id_class_or_grp, matiere_id, date_saisie, heure_saisie, date_contenu, contenu, classorgrp, id, number, idprof FROM ${prefixe}cahiertexte WHERE id_class_or_grp='$idClasse' AND $ref='$date' ORDER BY date_contenu,heure_saisie";
	$res=execSql($sql);
	$data_2=chargeMat($res);
	return $data_2;
}
...
function affobjectifScolaireParent($idClasse,$date,$ref) {
	...
	global $prefixe;
	$date=dateFormBase($date);
	$sql="SELECT id_class_or_grp, matiere_id, date_saisie, heure_saisie, date_contenu, objectif, classorgrp, id, number_obj, idprof FROM ${prefixe}cahiertexte WHERE id_class_or_grp='$idClasse' AND $ref='$date' ORDER BY date_contenu,heure_saisie ";
	$res=execSql($sql);
	$data_2=chargeMat($res);
	return $data_2;
}
...
// recherche le nombre d'absence non justifiÈ
...
function affAbsNonJustif2bis($trie,$idclasse="tous"){
	...
	$sql="SELECT a.elev_id, a.date_ab, a.date_saisie, a.origin_saisie, a.duree_ab , a.date_fin, a.motif, a.duree_heure, a.id_matiere, a.time, e.nom, e.elev_id, e.classe, a.courrierenvoyer,e.email,e.email_resp_2 FROM ${prefixe}absences a , ${prefixe}eleves e WHERE (a.duree_ab='0' OR a.motif='inconnu' OR a.justifier!='1') AND a.elev_id = e.elev_id $suiteSql AND a.courrierenvoyer = '0' ORDER BY $trie ";
	$res=execSql($sql);
	$data_2=chargeMat($res);
	return $data_2;
}
...
function affAbsence4($datej){
        global $cnx;
	global $prefixe;
	if (ereg('/',$datej)) { $datej=dateFormBase($datej); }
...
function anneeScolaireViaIdClasse($idClasse="") {
        ...
        global $prefixe;
        if ($idClasse != "") {
                $idSite=chercheIdSite($id_classe);
...
//	print $civ."<br>";
	...
	if (trim($civ) == "ÿßŸÑÿ¢ŸÜÿ≥ÿ©") { return 25; }
	if (trim($civ) == "ÿßŸÑÿ≥ŸäÿØ") { return 26; }
	if (trim($civ) == "ÿßŸÑÿ≥ŸäÿØÿ©") { return 27; }
...
function civ3($civ) {
	...
	if ($civ == "ÿßŸÑÿ¢ŸÜÿ≥ÿ©") { return 25; }
	if ($civ == "ÿßŸÑÿ≥ŸäÿØ") { return 26; }
	if ($civ == "ÿßŸÑÿ≥ŸäÿØÿ©") { return 27; }
...
function listingCiv() {
        ...
        if (CHARSET == "UTF-8") {
                print "<option value='25' id='select1' >ÿßŸ<84>ÿ¢Ÿ<86>ÿ≥ÿ©</option>";
                print "<option value='26' id='select1' >ÿßŸ<84>ÿ≥Ÿ<8a>ÿØ</option>";
                print "<option value='27' id='select1' >ÿßŸ<84>ÿ≥Ÿ<8a>ÿØÿ©</option>";
        }
        if (CIVARMEE == "oui") {
...
function create_matiere($nom_matiere) {
	...
	$matTmp=explode(" ",$mat);
	$mat='';// espace indispensable pour la base de donnÈes et l'affichage
...
function recupDateTrimByIdclasse($trim,$idclasse,$anneeScolaire='') {
	...
	if (trim($anneeScolaire) != "") {
        ...
        if ($trim == "annee") {
                $sql="SELECT date_debut FROM  ${prefixe}date_trimestrielle WHERE trim_choix='trimestre1' AND idclasse='$idclasse' $sqlsuite ";
                $res=execSql($sql);
                $data=chargeMat($res);
                $dateDebut1=$data[0][0];
                $sql="SELECT date_fin FROM  ${prefixe}date_trimestrielle WHERE trim_choix='trimestre2' AND idclasse='$idclasse' $sqlsuite ";
                $res=execSql($sql);
                $data=chargeMat($res);
                $dateFin2=$data[0][0];
                $sql="SELECT date_fin FROM  ${prefixe}date_trimestrielle WHERE trim_choix='trimestre3' AND idclasse='$idclasse' $sqlsuite ";
                $res=execSql($sql);
                $data=chargeMat($res);
                $dateFin3=$data[0][0];
                ...
                // VÈrification si dans la table il y a une idclasse en 0 ==> donc toutes les classes
                $sql="SELECT date_debut,date_fin,trim_choix FROM  ${prefixe}date_trimestrielle WHERE trim_choix='$trim' AND idclasse='$idclasse' $sqlsuite  ";
                $res=execSql($sql);
                $data=chargeMat($res);
                if (count($data) > 0) {
                        return $data;
...
function recupDateDebutAnnee($anneeScolaire='') {
        ...
        if ($anneeScolaire != "") {
                list($anneeDebut,$anneeFin)=split(" - ",$anneeScolaire);
...
//---------------------------------------------------------------------------//
//---------------------------------------------------------------------------//
// function config retard mise ‡ jour
...
function modif_retard2($elev_id,$heure_ret,$date_ret,$duree,$motif,$date_de_saisie,$user,$justifier,$heuredoriginsaisie,$date_ret_origine) {
	...
	$motif=strip_tags($motif);
	$heureS=dateHIS();
	if (ereg('/',$date_ret_origine)) { $date_ret_origine=dateFormBase($date_ret_origine); }
	if (ereg('/',$date_ret)) { $date_ret=dateFormBase($date_ret); }
...
function modif_absence($elev_id,$date_ab,$date_de_saisie,$user,$motif,$duree,$time,$idmatiere,$justifier,$heuredoriginsaisie,$saisie_date_ret_origine,$heuredabsence,$refRattrapage) {
	...
	if (ereg('/',$date_ab)) { $date_ab=dateFormBase($date_ab); }
...
// affichage date trimestrielle mise ‡ jour
...
// suppression de devoir scolaire ‡ la maison
...
function suppression_retard($saisie_eleve_id,$saisie_heure_ret,$saisie_date_ret) {
	global $cnx;
	global $prefixe;
	if (ereg('/',$saisie_date_ret)) { $saisie_date_ret=dateFormBase($saisie_date_ret); }
...
function select_matiere2() {
	...
	global $prefixe;
		...
		$sousmat=ereg_replace("^0$","",$sousmat);	
...
//------------------------------------------------------------//
//------------------------------------------------------------//
// reÁoit une date au format aaaa-jj-mm
...
// reÁoit une heure au format hh:mm:ss
...
// reÁoit une date au format jj/mm/aaaa
...
// reÁoit une heure de type hh:mm
// verifie si vrai
function checkTime($heure) {
	$elements1=split(":",$heure);
...
// reÁoit une date au format aaaa-mm-jj
...
function modif_pers_passe($id,$pass,$type,$envoiMail="non",$email="") {
        ...
        if (SECURITE == 3) {
                if ( (strlen($pass) < 8) || (!ereg('[a-z]',$pass)) || (!ereg('[A-Z]',$pass)) || (!ereg('[0-9]',$pass)) ) {
                        return 0 ;
                }
        }

        if (SECURITE == 2) {
                if ( (strlen($pass) < 8) || (!ereg('[a-z]',$pass)) || (!ereg('[0-9]',$pass)) ) {
                        return 0 ;
                }
        }
...
function modif_eleve_sconet($params){
	...
	if (count($liste) > 0) {
		if ($responsable == "1") {
			...
			if ($sqlsuite != "") {
				$sqlsuite=ereg_replace(',$','',$sqlsuite);
...
for($i=0;$i<count($data);$i++)
	...
	print($nom."[".$i."]=new Array();\n");
		...
		{
		$data[$i][$j]=ereg_replace("'","\'",$data[$i][$j]);
...
function create_absent($duree,$date,$saisie_pers,$date_saisie,$user,$motif,$idmatiere,$justifier,$heuredabsence,$idprof,$creneaux,$refRattrapage) {
	...
	if ($motif == "") { $motif="inconnu"; }
	if ($duree == "????") { $duree=0; }
	if ((ereg('H',$duree))  || ($duree == "heure")) {
		if ($duree == "heure") {
			$dureeH=$heuredabsence;
		}else{
			$dureeH=ereg_replace("H",".",$duree);
		}
		$duree="-1";
		$datefin=$date;
...
function smsauto($ideleve) {
	...
	$res=execSql($sql);
	$data=chargeMat($res);	
	$listeClasse=ereg_replace("\{","",$data[0][0]);
	$listeClasse=ereg_replace("\}","",$listeClasse);		
...
function enrAlertAbsRtd($idEleve,$type) {
	...
	if (count($data) > 0) {
		...
		if (($nb >= $nbmax) && ($signaler == '0') && (trim($nbmax) != "") ) {
			...
			$date=dateDMY2();
			$heure=dateHIS();
			if ( $type=="alertNbAbs" ) { $type2="d'absence autorisÈe"; }
			if ( $type=="alertNbRtd" ) { $type2="de retard autorisÈ"; }
			$message="L'ÈlËve <b>$nomPrenomEleve</b> vient de dÈpasser le nombre  $type2 <br> <br> <i>Message automatique</i><br><br>";
...
function enrAlertAbsRtdBis($idEleve,$type,$matabs) {
	...
	if (count($dataa) > 0) {
		...
		if (($nb >= $nbmax) && ($signaler == '0') && (trim($nbmax) != "") ) {
			...
			if ( $type=="alertNbAbs" ) { $type2="d'absence autorisÈe"; }
			if ( $type=="alertNbRtd" ) { $type2="de retard autorisÈ"; }
				$message="L'ÈlËve <b>$nomPrenomEleve</b> vient de dÈpasser le nombre   $type2 pour la matiÈre $matabs, nombre Abs : $nb / $nbmax<br> <br> <i>Message automatique</i><br><br>";
			for($j=0;$j<count($dataLi);$j++) {
				$idpers=$dataLi[$j];
				$type_personne_dest=$type_personne=recherche_type_personne($idpers);
				$emetteur=$destinataire=$idpers;
...
function createAffectationCopy($idclasseSource,$anneeScolaireSource,$idclasseDestination,$anneeScolaireDest) {
	...
	$sql="DELETE FROM ${prefixe}affectations WHERE code_classe='$idclasseSource' AND  annee_scolaire='$anneeScolaireDest'";
	execSql($sql);
...
for ($i=0;$i<=$rows;$i++) {
	...
	$varSql[$i][anneeScolaire]=$anneeScolaire;
		// inutile ??? versus requÍte sur matieres champ ''
	if($matTmp[1]):
		$varSql[$i][smat]='true';
	else:
		$varSql[$i][smat]='false';
...
// crÈation du tableau des requÍtes insert
for($i=0;$i<count($varSql);$i++){
...
function calculDateFin($date,$duree) {
	...
	$duree = $duree * 86500 ;
	if ($dur >= 1) {
		$duree = $duree - 86500 ; // on enleve 1 journÈe
	}
	$rdate=$rdate + $duree;
...
function chercheIdMatiere($nom_matiere) {
	global $cnx;
	global $prefixe;
	$nom_matiere=ereg_replace("'","\'",$nom_matiere);
...
function chercheMatiereNom($id_matiere) {
	...
	if ($id_matiere < 0) {
		$id_matiere=ereg_replace('-','',$id_matiere);
...
EOF;
   ...
   $res=execSql($sql);
   $data=chargeMat($res);
   $valeur=civ($data[0][3])." ".sansaccentmajuscule(strtoupper(trim($data[0][1])));
   return $valeur;
   }
}
...
function visu_affectation_detail($id_classe) {
	global $cnx;
	global $prefixe;
...
// verification si le prof est affecte ‡ une classe
...
function nombre_absJustifieMaladie($id_eleve,$dateDebut,$dateFin) {
        global $cnx;
        global $prefixe;
        $sql="SELECT elev_id, date_ab, date_saisie, origin_saisie, duree_ab ,date_fin, motif, duree_heure,justifier FROM ${prefixe}absences WHERE elev_id='$id_eleve' AND date_ab >='$dateDebut' AND date_ab <= '$dateFin' AND justifier = '1' AND lower(motif) = 'certificat mÈdical' AND duree_ab >= '0.5' ";
        $res=execSql($sql);
        $data_2=chargeMat($res);
        $nb=0;
        for($i=0;$i<count($data_2);$i++) {
		...
		foreach($listeid as $idunique) {
			...
			if (verifPresenceId($idunique,'PERS')) {
				...
				if ($brouillon == 0) {
					...
					envoiMessageIntraMSN("Vous avez reÁu un message \"$objet\"  sur votre compte Triade",$loginMSN);
				}
...
function circulaireAffProf($okprof,$tri,$filtre) {
	...
	$sql="SELECT id_circulaire, sujet, refence, file, date, enseignant, classe FROM	${prefixe}circulaire WHERE enseignant='1'  $sqlsuite ORDER BY $tri ";
	$res=execSql($sql);
        $data=chargeMat($res);
        return $data;
}
...
function circulaireAffPersonnel($tri,$filtre) {
	...
	if ($tri == "date") $tri="date DESC";
	if ($filtre != "") $sqlsuite=" AND categorie='$filtre'";
	$sql="SELECT id_circulaire, sujet, refence, file, date, enseignant, classe FROM	${prefixe}circulaire WHERE comptepersonnel='1' $sqlsuite ORDER BY $tri ";
	$res=execSql($sql);
        $data=chargeMat($res);
        return $data;
}
...
function circulaireAffVieScolaire($tri,$filtre) {
	...
	if ($tri == "date") $tri="date DESC";
	if ($filtre != "") $sqlsuite=" AND categorie='$filtre'";
	$sql="SELECT id_circulaire, sujet, refence, file, date, enseignant, classe FROM	${prefixe}circulaire WHERE compteviescolaire='1' $sqlsuite ORDER BY $tri ";
	$res=execSql($sql);
        $data=chargeMat($res);
        return $data;
}
...
function circulaireAffTuteurdeStage($tri,$filtre) {
         ...
         if ($tri == "date") $tri="date DESC";
         if ($filtre != "") $sqlsuite=" AND categorie='$filtre'";
         $sql="SELECT id_circulaire, sujet, refence, file, date, enseignant, classe FROM ${prefixe}circulaire WHERE comptetuteurdestage='1' $sqlsuite ORDER BY $tri ";
         $res=execSql($sql);
         $data=chargeMat($res);
         return $data;
}
...
function circulaireAffAdmin($tri,$filtre) {
         ...
         $sql="SELECT id_circulaire,sujet,refence,file,date,enseignant,classe FROM  ${prefixe}circulaire $sqlsuite $ORDERBY $tri ";
         $res=execSql($sql);
         $data=chargeMat($res);
         return $data;
 }
...
function circulaireAffParent($idClasse,$tri,$filtre) {
	...
	global $prefixe;
	if ($filtre != "") $sqlsuite="WHERE categorie='$filtre'";
	$sql="SELECT  id_circulaire, sujet, refence, file, date, enseignant, classe FROM ${prefixe}circulaire  $sqlsuite ORDER BY $tri DESC";
	$res=execSql($sql);
        $data=chargeMat($res);
        return $data;
}
...
$val=$options[$i][0];
$lib=$options[$i][1];
$lib=ereg_replace("0", " ", $lib); // j'ai rajouter mais si la matiere contient un zero il sera remplace par un blanc
$libTitle=$lib;
...
// ------------------------------------------------------------//
// $options -> matrice de listes ‡ deux ÈlÈments (0:value du option ,1:libelle du option)
...
for($cpt=0;$cpt<count($groupes);$cpt++){
	...
	$liste_eleves=ereg_replace(',,',',',$liste_eleves);
	$liste_eleves=ereg_replace("\{,","{",$liste_eleves);
	$liste_eleves=ereg_replace(",\}","}",$liste_eleves);
        ...
        for($i=0;$i<count($mat);$i++){
                ...
                for($j=0;$j<count($mat[$i]);$j++){
                        ...
                        if ($j == 5) {
                                ...
                                if (!ereg("[0-9]0$",trim($matiere))) {
                                        $matiere=ereg_replace("^0$","",trim($matiere));  // supprime le ZERO des sous matieres
                                        $matiere=ereg_replace("0$","",trim($matiere));  // supprime le ZERO des sous matieres
                                }
...
function htmlTrMatCopy($mat){
                for($i=0;$i<count($mat);$i++){
                        ...
                        for($j=0;$j<count($mat[$i]);$j++){
                                ...
                                if ($j == 4) $matiere='<font color="green"><i>non&nbsp;rÈcupÈrÈ</i></font>';
                                if ($j == 5) $matiere='<font color="green"><i>non&nbsp;rÈcupÈrÈ</i></font>';
                                print "\t<td valign=top ><font class=T2>&nbsp;".$matiere."</font></td>\n" ;
                        }
                        print("</tr>\n");
                }
...
// fonction de dev-dÈbogue principalement
...
// supprime toutes les entrÈes de GEP_classe
...
function verifMatiereAvecGroupeCarnetDeNote($idMatiere,$idEleve,$idClasse,$anneeScolaire='') {
        ...
        for($i=0;$i<count($tab_idGr);$i++) {
                ...
                $res=execSql($sql);
                $data=chargeMat($res);
                $liste_eleves=ereg_replace("\{","",$data[0][1]);
                $liste_eleves=ereg_replace("\}","",$liste_eleves);
	...
	$Array = array(); $dir = opendir($rep);
  	...
  	while ($File = readdir($dir)){
		if($File != "." && $File != ".." && $File != ".htaccess") {
			if (ereg('^tableaupp',$File)) {
				$fichier="$rep/$File";
      				@unlink("$fichier");
			}
			if (ereg('^periode',$File)) {
				$fichier="$rep/$File";
      				@unlink("$fichier");
			}
			if (ereg('jpg$',$File)) {
...
// history des opÈrations ÈffectuÈ
	...
	if (trim($id_eleve) != "") {
		...
		if (count($data) > 0) {
			for($i=0;$i<count($data);$i++) {
				...
				$liste_eleves="";
				if ($idgrp != "") {
					$liste_eleves=ereg_replace("\{","",$data[$i][1]);
					$liste_eleves=ereg_replace("\}","",$liste_eleves);
...
function listeEleveDansGroupe($idgroupe) {
	global $cnx;
	global $prefixe;
	$sql="SELECT group_id,liste_elev FROM ${prefixe}groupes WHERE  group_id='$idgroupe' ";
	$res=execSql($sql);
	$data=chargeMat($res);
	$liste_eleves=ereg_replace("\{","",$data[0][1]);
	$liste_eleves=ereg_replace("\}","",$liste_eleves);
...
function TextNoAccent($Text){
	 Return (strtr($Text, "¿¡¬√ƒ≈‡·‚„‰Â“”‘’÷ÿÚÛÙıˆ¯»… ÀÈËÍÎ«ÁÃÕŒœÏÌÓÔŸ⁄€‹˘˙˚¸ˇ—Ò","AAAAAAaaaaaaOOOOOOooooooEEEEeeeeCcIIIIiiiiUUUUuuuuyNn"));
 }
...
// ----------------------------------------- //
// Module de gestion prÈcÈdent et suivant //
...
function suivant0($fichier,$table,$limit,$nbaff,$requete) {
	...
	$get="";
	if (ereg('#',$fichier)) {
		list($fichier,$get)=split('#',$fichier);
		$get="&".$get;
	}
...
function precedent0($fichier,$table,$limit,$nbaff,$requete) {
	...
	if (ereg('#',$fichier)) {
		...
		$get="&".$get;
       	...
       	if ($limit >= 0) {
             $url="$fichier?limit=$limit&nba=${nbaff}$get&id=$id";
             print "<input type=button class=BUTTON value='<-- PrÈcÈdent' onclick=\"open('".$url."','_parent','')\"> ";
       	}
}
...
function blacklisteenr($nomuser,$prenomuser,$date,$ip,$navig_user,$membre,$fichier) {
	...
	$ins=execSql($sql);
	if ($ins) {
		if (MAILBLACKLIST == "oui") { mailAdmin("Un utilisateur est BlackListÈ"); }
	}
	return $ins;
 }
...
function stage_ajout($num,$debutdate,$findate,$idclasse,$nomstage,$jourdesemaine) {
	...
	$listejour=ereg_replace(',$','',$listejour);
...
function checkbox_stage($id) {
	...
	for($i=0;$i<count($data);$i++)
	{
		...
		list($nom_stage,$null)=split('\|',$data[$i][3]);
...
function create_eleve_stage($ideleve,$iddent,$lieu,$ville,$idprof,$date,$loge,$nourri,$xservice,$raison,$info,$iddatestage,$codepostal,$tuteur,$tel,$alternance,$dateDebutAlternance,$dateFinAlternance,$jourstage,$compte_tuteur_stage,$horairedebutjournalier,$horairefinjournalier,$date2,$idprof2,$service,$indemnitestage,$pays,$fax,$responsable2,$langue,$trim,$idtuteur2=0) {
	...
	$jourstages=ereg_replace(',$','',$jourstages);
...
function recherche_stage_historique($ideleve) {
	global $cnx;
	global $prefixe;	 
	$sql="SELECT e.nom,s.nomprenomeleve,s.classeeleve,s.periodestage,s.trimestre,s.langue,s.service FROM ${prefixe}stage_history s, ${prefixe}stage_entreprise e WHERE e.id_serial=s.identreprise AND s.ideleve='$ideleve' GROUP BY s.periodestage ";
	$res=@execSql($sql);
	$data=chargeMat($res);
	return($data);
}
...
function valide_equip($valide,$id,$idpers,$servername) {
	...
	if ($valide == 1) {
		$text="Bonjour, <br /><br /> Votre rÈservation de : $equip est accordÈe.<br><br> ";
	...
	if ($valide == 2) {
        $text="Bonjour, <br /><br /> Votre rÈservation de : $equip n\'est pas accordÈe.<br><br> ";
		$text.="<i>Informations : le $date du $heure_depart au $heure_fin <br> $info </i>";
		$text.="<br><br>Pour tout renseignement nous contacter. <br><br>";
		$text.="Triadement Votre, ";
...
function create_resa2($equipement,$date,$idpers,$heure1,$heure2,$info,$confirm=0,$jusquau,$tabJours,$id_edt_seances) {
	...
	list($h,$m,$s)=split(":",$heure1);
	list($h2,$m2,$s2)=split(":",$heure2);	
...
function renvoiMembreFormatePersonne($membre) {
	...
	if ($membre == "menueleve") { $membre="ElËve"; } 
...
function envoi_mail_forward($nom,$prenom,$texte,$email,$lien,$emetteur,$number,$objet,$idpers) {
  ...
  // recuperation des coordonnÈes de l'etablissement
  $data=visu_param();
  for($i=0;$i<count($data);$i++) {
...
puis 'Valider votre accrÈditation ‡ une centrale de stage'<br>
<br>
Une fois le mot de passe enregistrÈ vous pourrez acceder a la central des stages.<br>
<br>
<hr>
...
function liste_idpers_grp_mail($liste_id) {
	global $cnx;
	global $prefixe;
	$liste_id=ereg_replace("\{","",$liste_id);
	$liste_id=ereg_replace("\}","",$liste_id);
...
function verifGroupBis($idEleve) {
	...
	$data=chargeMat($res);
	for ($i=0;$i<count($data);$i++) {
		$liste_eleves=ereg_replace("\{","",$data[$i][0]);
       		$liste_eleves=ereg_replace("\}","",$liste_eleves);
		$listeEleve=preg_split ("/,/", $liste_eleves);
		foreach ($listeEleve as $valeur) {
			if ($idEleve == $valeur) {
...
function fin_prog($debut) {
	global $cnx;
    	global $prefixe;
	// calcul du temps d'ÈxÈcution et affichage
	$fin=deb_prog();
	$total = $fin - $debut;
...
function enr_config_note_usa($libelle,$min,$max) {
    ...
    $data=ChargeMat(execSql($sql));
    if (count($data) > 0) {
    	print "<ul><font color=red>Cette note est dÈj‡ affectÈe.</font></ul>";
    	return;
    }
	$sql="SELECT libelle,min,max FROM ${prefixe}config_note_usa WHERE (min <= '$min' AND max >= '$min') OR (max >= '$max' AND min <= '$max' )";
	$data=ChargeMat(execSql($sql));
	if (count($data) > 0) {
	    	print "<ul><font color=red>Cette intervalle fait dÈj‡ partie de la note <b>".$data[0][0]."</b>.</font></ul>";
	    	return;
    }
...
function verifImageJpg($type) {
	if ((eregi($type, 'jpg')) || (eregi($type, 'pjpeg')) || (eregi($type, 'jpeg')) )  {
		return true;
	}else{
		return false;
	}
...
function verifPdf($type) {
	if ((eregi($type, 'pdf')) || (eregi($type, 'PDF')) )  {
		return true;
	}else{
		return false;
	}
...
function sansaccent($chaine) {
   return strtr($chaine,'‡‚‰Â„·¬ƒ¿≈√¡Ê∆Á«ÈËÍÎ… À»ÔÓÏÌœŒÃÕÒ—ˆÙÛÚı”‘÷“’˘˚¸˙‹€Ÿ⁄ˇ','aaaaaaaaaaaaaacceeeeeeeeiiiiiiiinnoooooooooouuuuuuuuy');
}


function sansaccentmajuscule($chaine) {
   return strtr($chaine,'¬ƒ¿≈√¡«… À»œŒÃÕ—”‘÷“’‹€Ÿ⁄','AAAAAACEEEEIIIINOOOOOUUUU');
}
...
function recupNbrEleveRegime() {
	...
	// De 0 (pour Dimanche) ‡ 6 (pour Samedi)
...
function liste_eleve_entreprise() {
	global $cnx;
	global $prefixe;
	$datedujour=dateDMY2();
...
function iso2ascii($s) {
	$iso="·ËÔÈÏÌÂµÚÛ¯πª˙˘˝æ‰Îˆ¸¡»œ…ÃÕ≈•“”ÿ©´⁄Ÿ›ÆƒÀ÷‹";
...
function create_rss($koi,$nom,$prenom,$descriptionItem) {
	...
	if ($koi == "actu") { $titre="Etablissement - ActualitÈ - News"; }
	...
	$image->link = "http://www.triade-educ.com"; 
...
";
				$messageTEXT=ereg_replace("&nbsp;"," ",$message);
                                $to = $data[$i][3] ;
...
<hr><br>
<br>
S.A.R.L.  T.R.I.A.D.E.  http://www.triade-educ.com / Gestion Scolaire via Internet<br>
<br>
";
...
---> S.A.R.L.  T.R.I.A.D.E.  http://www.triade-educ.com / Gestion Scolaire via Internet <----
";
	...
	if (trim($nom_expediteur) == "") { $nom_expediteur=$email_expediteur; }
        ...
        $message_texte=ereg_replace("<[^>]*>", "", $message_texte);
...
//        $message_html=ereg_replace('src=',$serveur,$message_html);
//        $message_html=ereg_replace('"/','/',$message_html);
	...
	if (preg_match('/,/',$destinataire)) {
		...
		history_cmd("MAILING","MAILING","Email envoyÈ ‡ $destinataire");
	}else{
...
function arrondi($note) {
	if (ereg("50$",$note)) { return $note; }	
	if (ereg("00$",$note)) { return	number_format($note,0,'',''); }	
	list($dix,$cent)=split('\.',$note);
	if (($cent >= 01) && ($cent <= 25)) { return  floor(number_format($note,0,'','')); } // xx.01 xx.25
	if (($cent >= 26) && ($cent < 50))  { return  ereg_replace("(.*\.)[0-9][0-9]","\\150",$note); } // xx.26 xx.49
	if (($cent > 50) && ($cent <= 75))  { return  ereg_replace("(.*\.)[0-9][0-9]","\\150",$note); } // xx.51 xx.75
	if (($cent <= 99) && ($cent >= 76))   { return  ceil(number_format($note,0,'','')); }  // xx.76 xx.99
	return $note;
}

function arrondiaudixieme($valeur) {
	if (ereg(',',$valeur)) { $valeur=ereg_replace(',','.',$valeur); }
	$valeur=round($valeur,1);
	$valeur=sprintf("%01.1f",$valeur);
	list($unite,$dixaine)=split("\.",$valeur);
/*	if ($dixaine == "00") {
		$dixaine=0;
	}elseif ($unite == 20) {
		$dixaine=0;
...
function select_classe_profp($idprof) {
	...
	$curs=execSql($sql);
	$data=chargeMat($curs);
	// patch pour problËme sous-matiËre ‡ 0
	for($i=0;$i<count($data);$i++){
...
function recherche_com($ideleve,$tri,$type_bulletin,$anneeScolaire="") {
        ...
        global $prefixe;
        if ($anneeScolaire == "") $anneeScolaire=$_COOKIE["anneeScolaire"];
        $sql="SELECT commentaire FROM  ${prefixe}bulletin_direction_com WHERE ideleve='$ideleve' AND trimestre='$tri' AND type_bulletin='$type_bulletin' AND annee_scolaire='$anneeScolaire' ";
        $curs=execSql($sql);
        $data=chargeMat($curs);
        if (count($data) > 0) {
                return html_vers_text($data[0][0]);
...
function listEdt($startOfWeek,$endOfWeek,$idclasse,$idprof,$idRessource,$accesEDT){
	global $cnx;
	global $prefixe;
	list($startOfWeek,$null)=split(" ",$startOfWeek);
	list($endOfWeek,$null)=split(" ",$endOfWeek);
	...
	$datefinEDT=ereg_replace("-","",dateFormBase($datefinEDT[0][1]));
	$datedebutEDT=ereg_replace("-","",dateFormBase($datedebutEDT[0][1]));

	$dateDeb=ereg_replace("-","",$startOfWeek);
	$dateFi=ereg_replace("-","",$endOfWeek);
...
function ajoutEdt($CODE,$ENSEIGNEMENT,$dateDebut,$dateFin,$bgcolor) {
	...
	list($DATE,$HEURE)=split(" ",$dateDebut);
	list($null,$heure2)=split(" ",$dateFin);
...
function miseAJourEdt($id,$eventDescription,$color,$idclasse,$idprof,$jusquau,$prestation,$tabJours,$recursive,$idmatiere,$coursannule,$docdst,$reportle,$reporta,$validecreation,$emargement,$idressource,$dureehoraire,$debuthoraire,$emargementeval,$emargementpedago,$idgroupe,$semainesurdeux) {
	...
	$eventDescription=ereg_replace("\n"," ",$eventDescription);
	$eventDescription=ereg_replace("\r"," ",$eventDescription);
...
function create_carnet($nom_carnet,$code_lettre,$code_chiffre,$code_couleur,$code_note,$section,$nbPeriode,$code_julesverne,$code_commentaire) {
	...
	$liste_section.="}";
	$liste_section=ereg_replace(",\}","}",$liste_section);
...
function selectTypeNotation($idcarnet){
	...
	$sql="SELECT code_lettre,code_chiffre,code_couleur,code_note,code_julesverne,code_commentaire FROM ${prefixe}carnet_suivi  WHERE id='$idcarnet' ";
	$data=ChargeMat(execSql($sql));
	if ($data[0][0] == 1) { print "<option id='select1' value='lettre'>en Lettre (A ‡ D)</option>\n"; }
	if ($data[0][1] == 1) { print "<option id='select1' value='chiffre'>en Chiffre (1 ‡ 4)</option>\n"; }
	if ($data[0][2] == 1) { print "<option id='select1' value='couleur'>en Couleur (Vert, Bleu ,Orange, Rouge)</option>\n"; }
	if ($data[0][3] == 1) { print "<option id='select1' value='note'>en Note (0 ‡ 10 ou 0 ‡ 20)</option>\n"; }
...
function selectPeriodeCarnet($idcarnet) {
	...
	$nb=$data[0][0];
	for($i=1;$i<=$nb;$i++) {
		print "<option id='select1' value='$i'>PÈriode $i</option>\n";
	}
...
function chercheSectionClasse($idsection) {
	...
	$data=ChargeMat(execSql($sql));
	$data=ereg_replace("\{","",$data[0][0]);
	$data=ereg_replace("\}","",$data);
	$tab_section=split(",",$data);
	return $tab_section;
}
...
function rechercheEvalutionEleveBultin($idEleve,$idcarnet,$iddescriptif,$idcompetence,$periode,$listeidclasse) {
	global $cnx;
	global $prefixe;
	$listeidclasse=ereg_replace("\{","",$listeidclasse);
	$listeidclasse=ereg_replace("\}","",$listeidclasse);
...
function ajoutModifCaracVieScolaire($bullpers,$coef_bulletin,$coefProf,$coefScolaire,$idclasse,$anneeScolaire) {
	...
	$data=ChargeMat(execSql($sql));
	$coef_bulletin=ereg_replace(',','.',$coef_bulletin);
	$coefProf=ereg_replace(',','.',$coefProf);
	$coefScolaire=ereg_replace(',','.',$coefScolaire);
	if (count($data) > 0) {
...
function eregist_planclasse($idclasse,$newcoord) {
	...
	$newcoord=ereg_replace(",$","","$newcoord");
	$tab=explode(",",$newcoord);
	$sql="DELETE FROM ${prefixe}planclasse  WHERE  idclasse='$idclasse' ";
	execSql($sql);
	foreach($tab as $key=>$value) {
		if (ereg("E",$value)) {
			list($idEleve,$coor)=split('\(',$value,2);
			$idEleve=ereg_replace("E","",$idEleve);
			$coor=ereg_replace('\)',"",$coor);
			list($x,$y)=split('\;',$coor,2);
		}
		if (ereg("B",$value)) {
			list($idEleve,$coor)=split('\(',$value,2);
			$idEleve=ereg_replace("B","",$idEleve);
			$idEleve="-".$idEleve;
			$coor=ereg_replace('\)',"",$coor);
			list($x,$y)=split('\;',$coor,2);
		}
...
function create_piecejointe($fichier,$fichierMD5,$idpiecejointe,$etat) {
	global $cnx;
	global $prefixe;
	// Etat=1 c'est ok , sinon ‡ dÈtruire
...
function selectStage($eid) {
	...
	foreach($tabEntr as $key=>$value) {
	        list($id,$idnum)=split('#',$key);
	        $num=rechercheNumStage($idnum);
	       	if (trim($num) != "") $num="$num)";
	        print "<option id='select1' value='$id#$idnum' title=\"$value\" > $num ".trunchaine($value,30)."</option>";
	}
}		
...
function enrConfigBrevet($tab,$idclasse) {
	...
	execSql($sql);
	foreach($tab as $key => $value) {
		list($libelle,$idmatiere)=split("\|",$value);
		if ($libelle != "") {
...
function modifOrdreaffectation($idclasse,$tablist,$tri,$anneeScolaire) {
	...
	foreach($tablist as $ordreNew => $value) {
		list($ordre,$idMatiere,$idProf,$coef,$idGrp,$idLang,$avecSousMatiere,$visubull,$ects,$ue)=split(":",$value);
		$sql="INSERT INTO ${prefixe}affectations (ordre_affichage,code_matiere,code_prof,code_classe,coef,code_groupe,langue,avec_sous_matiere,visubull,trim,ects,id_ue_detail,annee_scolaire) VALUES ('$ordreNew','$idMatiere','$idProf','$idclasse','$coef','$idGrp','$idLang','$avecSousMatiere','$visubull','$tri','$ects','$ue','$anneeScolaire')";
		$cr=execSql($sql);
	}
	return 1;
...
function cumulEntretien($eid,$nomclasse) {
	...
	$data=ChargeMat($res);
	for($i=0;$i<count($data);$i++) {
		list($heure,$minute,$seconde)=split(":",$data[$i][2]);
		$secondeH1=($heure*60*60) + $seconde + ($minute*60);
		list($heure2,$minute2,$seconde2)=split(":",$data[$i][3]);
...
function calculduree($h1,$h2) {
	list($heure,$minute,$seconde)=split(":",$h1);
	$secondeH1=($heure*60*60) + $seconde + ($minute*60);
	list($heure2,$minute2,$seconde2)=split(":",$h2);
	$secondeH2=($heure2*60*60) + $seconde2 + ($minute2*60);
	$temps=$secondeH2-$secondeH1;
...
function nbHeureVacationParIdMatiere($idMatiere,$dateDebut,$dateFin) {
	...
	$dateDebut=dateFormBase($dateDebut);
	$dateFin=dateFormBase($dateFin);
	$sql="SELECT SEC_TO_TIME(SUM(s.duree)) FROM ${prefixe}edt_seances s, ${prefixe}vacation_config v WHERE v.id = s.prestation AND s.idmatiere='$idMatiere' 	AND s.date >= '$dateDebut' AND s.date <= '$dateFin' AND s.coursannule != '1' ";
	$res=execSql($sql);
	$data=ChargeMat($res);
	return($data[0][0]);
}
...
function chercheSigneAstro($jour,$mois) {
	/*
	Horoscope des BÈliers BELIER du 21 mars au 20 avril  
   	Horoscope des Taureaux TAUREAU du 21 avril au 20 mai 
   	Horoscope des GÈmeaux GEMEAUX du 21 mai au 20 juin 
...
function enrgPresent($saisie_heure,$datedepart,$idmatiere,$idprof,$ideleve) {
	...
	global $prefixe;
	$datedepart=dateFormBase($datedepart);
	list($libelle,$heureD,$heureF)=split('#',$saisie_heure);
...
function enrConfigVersement($idclasse,$nameversement,$montantversement,$dateversement,$anneescolaire) {
	...
	$montantversement=ereg_replace(',','.',$montantversement);
...
function  selectVersementAjout($idclasse,$ideleve,$anneeScolaire=' ') {
	...
	for($i=0;$i<count($data);$i++) {
		if (verifcomptaExclu($data[$i][0],$ideleve)) {
			$option="optgroup title=\"".$data[$i][2]."\" label=\"".trunchaine($data[$i][2],20)." (".affichageFormatMonnaie($data[$i][3])." $unite) exonÈrÈ.\"";
		}elseif (verifVersement($data[$i][0],$ideleve)) {
			$option="optgroup title=\"".$data[$i][2]."\" label=\"".trunchaine($data[$i][2],20)." (".affichageFormatMonnaie($data[$i][3])." $unite) dÈj‡ versÈ.\"";
		}else{
	...
	for($i=0;$i<count($data2);$i++) {
		if (verifVersement($data2[$i][0],$ideleve)) {
			$option="optgroup title=\"".$data2[$i][2]."\"  label=\"".trunchaine($data2[$i][2],20)." (".affichageFormatMonnaie($data2[$i][3])." $unite) dÈj‡ versÈ.\"";
		}else{
			$option="option id='select1' ";
		}
...
function listingPreinscription($idclasse,$decision,$annee_scolaire) {
	...
	$sqlsuite=ereg_replace('^AND','',$sqlsuite);
...
function envoiMessageIntraMSN($message,$loginMSN) { // loginMSN au format nom.prÈnom
        global $cnx;
	global $prefixe;
...
function verifPdfProfVersement($fic,$id_pers) {
	global $cnx;
	global $prefixe;
	$fic=ereg_replace("./data/comptaenseignant/","",$fic);
	$fic=ereg_replace(".pdf","",$fic);
...
function infoBulleEleve($idEleve) {
	...
	$mess.="<br /> TÈl resp 1 : <font color=blue>".$data[0][4]." / ".$data[0][6]."</font>" ; 
	$mess.="<br /> TÈl resp 2 : <font color=blue>".$data[0][5]."</font>" ; 
	$mess.="<br /> TÈl ElËve : <font color=blue>".$data[0][7]."</font>"; 
...
function recupAdrEleve($idClasse) {
	global $cnx;
	global $prefixe;
	$sql="SELECT nom,prenom,adr_eleve,ccp_eleve,commune_eleve,sexe,pays_eleve,numero_eleve FROM ${prefixe}eleves WHERE classe='$idClasse' AND compte_inactif != 1 ORDER BY nom,prenom";
	$curs=execSql($sql);
...
function recupAdrParent1($idClasse) {
	global $cnx;
	global $prefixe;
	$sql="SELECT nomtuteur,prenomtuteur,adr1,code_post_adr1,commune_adr1,civ_1,pays_eleve,numero_eleve FROM ${prefixe}eleves WHERE classe='$idClasse' AND compte_inactif != 1 ORDER BY nom,prenom";
	$curs=execSql($sql);
	$liste=chargeMat($curs);
	unset($curs);
	return $liste;
...
function accesNonReserve() {
	print "<br><font class='T2' id='color3'><center><img src='image/commun/img_ssl.gif' align='center' /> AccËs non autorisÈ.</center></font>";
}
...
function creationStageCentralSouhait($datedemande,$identreprise,$sexe,$service,$observation,$idperiode,$nbdemande,$salaire,$logement) {
	global $cnx;
	global $prefixe;
	$datedemande=dateFormBase($datedemande);
	$sql="INSERT INTO ${prefixe}centralstagesouhait  (datedemande,identreprise,sexe,service,observation,idperiode,nbdemande,salaire,logement) VALUE ('$datedemande','$identreprise','$sexe','$service','$observation','$idperiode','$nbdemande','$salaire','$logement')";
	return(execSql($sql));
...
function recupInfoCentralStage($id) {
	...
	global $prefixe;
	$date=dateDMY2();
	$sql="SELECT datedemande,identreprise,sexe,service,observation,nbdemande,idperiode,null,salaire,logement FROM ${prefixe}centralstagesouhait WHERE id='$id'";
	$data=ChargeMat(execSql($sql));
	return $data;
}
...
function verifAccesCentrale($productid,$p) {
	...
	if (count($data) > 0) {
		...
		print "<br><br><font class='T2'>Merci de contacter votre centrale de stage <br> afin de valider votre Ètablissement.</font>";
		print "</center>";
		exit;	
	}
}
...
function libelleTrimestre($saisie_tri) {
	$libelle="Toute l'annÈe";
	if ($saisie_tri == "tous" ) { $libelle="Toute l'annÈe"; }
...
function import_affectation_IPAC($idMatiere,$idClasse,$coef,$numero_semestre,$ects,$idDetailUE,$ordre,$specif,$anneeScolaire="") {
  	global $cnx;
	global $prefixe;
	if ($specif == "oui") {
	...
	if (trim($anneeScolaire) == "") $anneeScolaire=$_COOKIE["anneeScolaire"];
	$sql="INSERT INTO ${prefixe}affectations (code_matiere,code_classe,coef,trim,ects,id_ue_detail,specif_etat,ordre_affichage,langue,nb_heure,code_groupe,annee_scolaire) VALUE ('$idMatiere','$idClasse','$coef','$numero_semestre','$ects','$idDetailUE','$specif','$ordre','','','0','$anneeScolaire')";
	execSql($sql);
}

function import_affectation_IPAC_UPDATE($idMatiere,$idClasse,$coef,$numero_semestre,$ects,$idDetailUE,$ordre,$specif,$anneeScolaire="") {
	global $cnx;
        global $prefixe;
        if ($specif == "oui") {
                $specif="etudedecasipac";
	...
	if (trim($anneeScolaire) == "") $anneeScolaire=$_COOKIE["anneeScolaire"];
	$sql="UPDATE  ${prefixe}affectations SET id_ue_detail='$idDetailUE' , specif_etat='$specif' ,   ects='$ects' , coef='$coef'  
...
Madame, Monsieur<br>
<br>
Vous avez contactÈ le service ENT Vatel parce que vous avez ÈgarÈ votre mot de passe.<br>
...
function verifEmailEnregistrer($idpers,$membre,$tuteur) {
	...
	if (trim($sql) != "") {
	        $res=execSql($sql);
	        $data=chargeMat($res);
		$mail_valide =  ereg("([A-Za-z0-9]|-|_|\.)*@([A-Za-z0-9]|-|_|\.)*\.([A-Za-z0-9]|-|_|\.)*",$data[0][0]);
...
function suppressionRattrapage($idsupp,$nompers) {
	...
	if ($ref_id_absrtd != "") {
		list($nomeleve,$ideleve)=split('#',rechercheEleveViaRef_id_absrtd($ref_id_absrtd));
...
/*
 * CRYPT_CKEY et CRYPT_CIV sont ‡ changer pour votre projet
 */
...
function vatel_liste_ueT($semestre,$idclasse,$anneeScolaire) {  // fonction utilisÈe pour afficher la liste de toute des UE
   	global $cnx;
	global $prefixe;
	if ($semestre == 'tous') { 
...
"S" => "28","T" => "29","U" => "30","V" => "31","W" => "32","X" => "33","Y" => "34","Z" => "35");
        ...
        // DÈplacement des 4 premiers caractËres vers la droite et conversion des caractËres
        $tmpiban = strtr(substr($iban,4,strlen($iban)-4).substr($iban,0,4),$charConversion);
 
        // Calcul du Modulo 97 par la fonction bcmod et comparaison du reste ‡ 1
        return (intval(bcmod($tmpiban,"97")) == 1);
}

function isValideIBAN ($s_IBAN) {
        // VÈrification que le numÈro IBAN est bien dÈfini
        if(empty($s_IBAN)) return false;
 
        // Nettoyage des caractËres de formatage et mise en Capital
        $s_IBAN = strtoupper(trim($s_IBAN));
        /* VÈrification de l'IBAN par rapport au modËle :
                - Ne comporte pas ' espace , . / - ? : ( ) , " +
                - Suppression des caractËres IBAN en dÈbut de phrase si prÈsent
                - DÈplacement des 4 premiers caractËres (2 lettres et 2 chiffres) ‡ la fin de la chaÓne
                - Remplacement des caractËres alphabÈtiques comme suit : A->10, B->11 C->12... Z->35
                - VÈrifie que le modulo 97 donne 1
        */
...
function bulletin_archivage($trimestre,$anneeScolaire,$fichiersource,$idEleve,$classe_nom,$nomEleve,$prenomEleve) {
        ...
        $classe_nom=ereg_replace("/","_",$classe_nom);
...
function  ajaxEnvoiMailCentral($productid,$id,$idcentralestage,$email,$nomprenometudiant,$infocontenu,$numbermail) {
	 ...
	 list($nom_ent,$contact,$nometablissement,$villeetablissement,$paysetablissement,$periode,$departement)  = split('\|\|',$infocontenu);
...
function recupMatiereUE2($nom_ue,$idclasse) {
        ...
        $anneeScolaire=$_COOKIE["anneeScolaire"];
        $sql="SELECT u.code_matiere,m.libelle,u.code_enseignant,a.ordre_affichage, a.visubull , a.langue , a.specif_etat , e.semestre, a.ects
...
function select_MatiereNonAffecter($len) {
        ...
        for($i=0;$i<count($data);$i++) {
                ...
                print "<option id='select1' value='".$data[$i][0]."' title=\"".trim($data[$i][1])." ".trim($data[$i][2])."\" > $ok ".trunchaine(trim($data[$i][1]),$len);
                if(trim($data[$i][2]) != "0") {
                        print trunchaine(" ".trim($data[$i][2]),$len);
                }
                print "</option>\n";
...
function verifAnneeScolaireFuture($anneeScolaire,$idClasse) {
        ...
        list($annee,$anneep)=split(" - ",$anneeScolaireEnCours);
        list($anneef,$anneeff)=split(" - ",$anneeScolaire);
