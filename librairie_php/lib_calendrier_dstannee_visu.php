	...
	$nomsalle=$data2[0][1];
		...
		if (!ereg("h",$data[$i][5])) { $data[$i][5]="0${heure}h00 mn"; }
 		if ($date_deja == $date_recup_jour_mois ) {
                        $dstT.="<hr>";
                        $dstT.="Classe: ".trim($data[$i][3])."<BR>Matière: ".trim($data[$i][2]). " à ".timeForm($data[$i][4])." (".$data[$i][5].")" ;
			$dstT.="<br>";
...
$FerieSamedi="";
$FerieMercredi="";
if (defined("CALSAMEDI")) {
	if (CALSAMEDI == "oui") { $FerieSamedi=" || (j==5)"; }
        ...
        for(var i=0;i<6;i++) {
                ...
                for (j=0;j<7;j++) {
                        ...
                        if ((7*i+j>=j1-1)&&(jour<=nb_jour)) {
                                if ((j==6)||(estFerie(jour,m)) || (j==5) || (estConseil(jour,m)) $FerieSamedi $FerieMercredi ) {
