	...
	$date_recup=dateMoisAnnee($data[$i][3]);
        ...
        if ($annee == $date_recup_annee) {
        	$equip=recherche_equip($data[$i][1]);
                $equip=ereg_replace('"',"&quot;",$equip);
                $equip=ereg_replace('\'',"&rsquo;",$equip);
                $equip=ereg_replace('"',"&quot;",$equip);
                $equip=ereg_replace('\'',"&rsquo;",$equip);
                $passe=1;
 		if ($date_deja == $date_recup_jour_mois ) {
                        $dstT.="<HR>";
                        $dstT.="Equipement: ".trim($equip)."<BR>de  ".trim(timeForm($data[$i][4]))." à ".trim(timeForm($data[$i][5]));
                }else{
                        if ($txt_fini == 1) {
                                $dstT.="\"";
                        }
        ...
        for(var i=0;i<6;i++) {
			...
			aff_jj=aff_j;
					...
					if (estConseil(jour,m)) {
                                     ...
                                     disp("<TD width='10' bgcolor='pink'  align='center'><FONT face='Arial' size='-1' color='#0000CC'><A href='#' onMouseOver=\"AffBulle('<font face=Georgia, Times New Roman, Times, serif>"+dstT[ii]+" </FONT>');\"  onMouseOut='HideBulle()' onclick=\"PopupCentrer('calendrier_config_equip.php?saisiejour="+aff_j+"&saisiemois="+mois[d.getMonth()]+"&saisieannee="+a+"','500','350','scrollbars=yes','calendrier');\" return true; >"+aff_jj+"</A></FONT></TD>");
					ii++;
                                        }else {
				   ...
				   if (j==2) {
 					disp("<TD $Ferieap $FerieMatin width='10' bgcolor='#FFFFFF' align='center'><FONT face='Arial' size='-1' color='#0000CC'><A href='#' onclick=\"PopupCentrer('calendrier_config_equip.php?saisiejour="+aff_j+"&saisiemois="+mois[d.getMonth()]+"&saisieannee="+a+"','500','350','','calendrier')\";>"+aff_j+"</A></FONT></TD>");
				   }else{
					if (j == 5) 
					disp("<TD $FerieSaap $FerieSaMatin width='10' bgcolor='#FFFFFF' align='center'><FONT face='Arial' size='-1' color='#0000CC'><A href='#' onclick=\"PopupCentrer('calendrier_config_equip.php?saisiejour="+aff_j+"&saisiemois="+mois[d.getMonth()]+"&saisieannee="+a+"','500','350','','calendrier')\";>"+aff_j+"</A></FONT></TD>");
					else
					    disp("<TD width='10' bgcolor='#FFFFFF' align='center'><FONT face='Arial' size='-1' color='#0000CC'><A href='#' onclick=\"PopupCentrer('calendrier_config_equip.php?saisiejour="+aff_j+"&saisiemois="+mois[d.getMonth()]+"&saisieannee="+a+"','500','350','','calendrier')\";>"+aff_j+"</A></FONT></TD>");
				   }
				}
                        }
                        else
