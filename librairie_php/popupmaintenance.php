...
$disabled="";
if (file_exists($fic)) {
	$fichier=fopen("./data/maintenance.txt","r");
	$message=fread($fichier,"100");
	list($date,$time1,$time2)= split (":", $message, 3);
	...
	list($heure1,$minute1)=split ("h", $time1,2);
	list($heure2,$minute2)=split ("h", $time2,2);

	if (($date == dateDMY()) && (dateH() >= $heure1  ) && ( dateH() <= $heure2) ) {
		$disabled="disabled";
	}
?>
...
<?php } ?>
 if(document.getElementById)
     ...
     document.write('<DIV ID=ejs_al_box2 STYLE="border-radius: 5px 5px 5px 5px; box-shadow: 0px 0px 10px 4px rgba(119, 119, 119, 0.75); moz-box-shadow: 0px 0px 10px 4px rgba(119, 119, 119, 0.75); -webkit-box-shadow: 0px 0px 10px 4px rgba(119, 119, 119, 0.75); position:absolute;visibility:hidden;'+ejs_al_classe+';width:'+ejs_al_box2+';height:'+ejs_al_hauteur+';z-index:5"');
     if(document.all)
