...
// modif ambis 9/12/08				$moyenne_periode=$moyenne_periode/$nb_note_periode;
			...
			if ($moyenne_periode_ue>=0) { $moyenne_periode_ue= $moyenne_periode_ue/$nb_mat_per;}
			$moyenne_periode_gen += $moyenne_periode_ue;
...
for ($nb_ue=0;$nb_ue<count($ue);$nb_ue++) { // boucle pour chaque UE
	...
	//		if ($noteperiode[0][0]!=0 && $noteperiode[0][0]!='') {$nb_mat+=1;}
		...
		if ($ecrit!='') {
			...
			if ($moyenne_ue>=0) {$moyenne_gen_partiel+=$moyenne_ue;$moyenne_ue=$moyenne_ue/$somme_coef;$somme_coef_gen_partiel+=$somme_coef;}
...
//	print_r ($noteperiode);
		...
		$som_coef_periode=0;
		if ($moyenne_periode>0) { $moyenne_periode_ue+=$moyenne_periode;}
		if ($nb_note_periode>0) {$nb_note_periode_ue++;$nb_mat_per++;}
